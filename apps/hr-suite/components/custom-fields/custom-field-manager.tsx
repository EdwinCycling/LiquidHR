...
import { localizedCustomFieldLabel } from '@/lib/custom-fields/locale-label'
...
interface Labels {
  entity: string; employeeEntity: string; documentEntity: string; selectOption: string
...
function PreviewControl({ fieldType, options, labels, locale }: { fieldType: FieldType; options: OptionDraft[]; labels: Labels; locale: Locale }): ReactNode {
  ...
  if (fieldType === 'SELECT' || fieldType === 'MULTI_SELECT') return <DropdownSelect disabled multiple={fieldType === 'MULTI_SELECT'} aria-label={labels.previewValue} placeholder={labels.selectOption}><option value={options[0]?.value ?? ''}>{options[0] ? localizedCustomFieldLabel(options[0].labelNl, options[0].labelEn, locale) : labels.selectOption}</option></DropdownSelect>
...
export function CustomFieldManager({ definitions, entityType, labels, locale }: { definitions: CustomFieldDefinition[]; entityType: 'EMPLOYEE' | 'DOCUMENT'; labels: Labels; locale: Locale }) {
  ...
  return <div className="space-y-6">
    ...
    <div>
      ...
      <EntityList
        ...
        items={definitions.map((definition) => ({
          id: definition.id,
          primary: <Button className="justify-start px-0 text-left font-semibold" onClick={() => openEdit(definition)} size="sm" type="button" variant="ghost">{localizedCustomFieldLabel(definition.labelNl, definition.labelEn, locale)}</Button>,
    ...
    >
      ...
      {editor.mode === 'create' ? <FormField control={<DropdownSelect aria-label={labels.fieldType} name="fieldType" onChange={(event) => updateDraft({ fieldType: event.target.value as FieldType })} placeholder={labels.selectOption} value={editor.draft.fieldType}>{TYPES.map((type) => <option key={type} value={type}>{labels.types[type]}</option>)}</DropdownSelect>} label={labels.fieldType} /> : null}
      <div className="grid gap-4 sm:grid-cols-3">
        <FormField control={<DropdownSelect aria-label={labels.hrAccess} name="hrAccess" onChange={(event) => updateDraft({ hrAccess: event.target.value as Access })} placeholder={labels.selectOption} value={editor.draft.hrAccess}>{accessOptions(labels)}</DropdownSelect>} label={labels.hrAccess} />
        <FormField control={<DropdownSelect aria-label={labels.managerAccess} name="managerAccess" onChange={(event) => updateDraft({ managerAccess: event.target.value as Access })} placeholder={labels.selectOption} value={editor.draft.managerAccess}>{accessOptions(labels)}</DropdownSelect>} label={labels.managerAccess} />
        <FormField control={<DropdownSelect aria-label={labels.selfAccess} name="selfAccess" onChange={(event) => updateDraft({ selfAccess: event.target.value as Access })} placeholder={labels.selectOption} value={editor.draft.selfAccess}>{accessOptions(labels)}</DropdownSelect>} label={labels.selfAccess} />
      </div>
    ...
    <Surface className="p-5 sm:p-6" variant="subtle"><p className="eyebrow text-primary">{labels.preview}</p><h2 className="mt-1 text-lg font-semibold">{editor ? localizedCustomFieldLabel(editor.draft.labelNl, editor.draft.labelEn, locale) || labels.previewEmpty : labels.previewEmpty}</h2><p className="mt-1 text-sm text-muted-foreground">{labels.types[editor?.draft.fieldType ?? 'TEXT']}</p><div className="mt-5 max-w-xl"><PreviewControl fieldType={editor?.draft.fieldType ?? 'TEXT'} options={parseOptions(editor?.draft.options ?? '')} labels={labels} locale={locale} /></div></Surface>
  </div>
}
