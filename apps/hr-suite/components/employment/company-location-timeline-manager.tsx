...
interface Labels {
  title: string; description: string; company: string; companyAddress: string; locations: string; current: string; history: string; future?: string; active: string; notRecorded: string; readOnly: string; noLocations: string; location: string; locationSearch: string; locationSearchPlaceholder: string; noLocationResults: string; add: string; edit: string; save: string; cancel: string; effectiveOn: string; failed: string; saving: string; changeSaved: string; singleLocationMode: string
...
export function CompanyLocationTimelineManager({ company, locations, assignments, employmentId, canWrite, labels, today = new Date().toISOString().slice(0, 10) }: { company: { name: string; single_location: boolean; address: CompanyAddress | null }; locations: CompanyLocationOption[]; assignments: CompanyLocationAssignment[]; employmentId: string; canWrite: boolean; labels: Labels; today?: string }) {
  ...
  const earliestAssignmentDate = assignments.reduce<string | undefined>((earliest, assignment) => (
    !earliest || assignment.effective_from < earliest ? assignment.effective_from : earliest
  ), undefined)
  ...
  return <Surface className="p-5">
    ...
    {assignments.length > 0 ? <div className="mt-5 divide-y divide-subtle border-y border-subtle">{[...assignments].sort((left, right) => { const leftStatus = left.effective_from > today ? 2 : left.effective_to && left.effective_to < today ? 1 : 0; const rightStatus = right.effective_from > today ? 2 : right.effective_to && right.effective_to < today ? 1 : 0; if (leftStatus !== rightStatus) return leftStatus - rightStatus; return right.effective_from.localeCompare(left.effective_from) }).map((assignment) => { const location = locations.find((option) => option.id === assignment.location_id); const status = assignment.effective_from > today ? 'future' : assignment.effective_to && assignment.effective_to < today ? 'history' : 'current'; const statusLabel = status === 'current' ? labels.current : status === 'future' ? labels.future ?? labels.current : labels.history; const content = <><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-muted-foreground">{statusLabel}</p><p className="mt-1 font-semibold">{location?.name ?? labels.notRecorded}</p></div><Badge tone={status === 'current' || status === 'future' ? 'info' : 'neutral'}>{assignment.effective_to ?? labels.active}</Badge></div><p className="mt-3 text-sm text-muted-foreground">{labels.effectiveOn}: {assignment.effective_from}</p></>; return canWrite ? <button className="w-full cursor-pointer px-2 py-4 text-left transition-colors hover:bg-surface-subtle focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary" key={assignment.id} onClick={() => openEdit(assignment)} type="button">{content}</button> : <article className="px-2 py-4 text-left" key={assignment.id}>{content}</article> })}</div> : null}
    {canWrite ? <FormDrawer cancelLabel={labels.cancel} closeLabel={labels.cancel} dirty={dirty} dirtyProtection={{ title: labels.discardTitle ?? labels.cancel, description: labels.discardDescription ?? labels.failed, discardLabel: labels.discardConfirm ?? labels.cancel, keepEditingLabel: labels.discardCancel ?? labels.cancel }} onDiscard={close} onOpenChange={(nextOpen) => { if (!nextOpen) close() }} onSubmit={save} open={open} saveLabel={labels.save} saving={saving} title={mode === 'add' ? labels.add : labels.edit}>
      ...
      <FormField label={labels.effectiveOn} required control={<TextInput required min={earliestAssignmentDate} readOnly={mode === 'edit'} type="date" value={effectiveOn} onChange={(event) => setEffectiveOn(event.target.value)} />} />
