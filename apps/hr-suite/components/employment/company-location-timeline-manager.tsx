...
export function CompanyLocationTimelineManager({ company, locations, assignments, employmentId, canWrite, labels }: { company: { name: string; single_location: boolean; address: CompanyAddress | null }; locations: CompanyLocationOption[]; assignments: CompanyLocationAssignment[]; employmentId: string; canWrite: boolean; labels: Labels }) {
  ...
  return <Surface className="p-5">
    ...
    {activeLocations.length === 0 ? <EmptyState className="mt-5" title={labels.noLocations} /> : <div className="mt-5 divide-y divide-subtle border-y border-subtle">{assignments.map((assignment, index) => { const location = locations.find((option) => option.id === assignment.location_id); return <button className="w-full cursor-pointer px-2 py-4 text-left transition-colors hover:bg-surface-subtle focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary" key={assignment.id} onClick={() => openEdit(assignment)} type="button"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-muted-foreground">{index === 0 ? labels.current : labels.history}</p><p className="mt-1 font-semibold">{location?.name ?? labels.notRecorded}</p></div><Badge tone={index === 0 ? 'info' : 'neutral'}>{assignment.effective_to ?? labels.active}</Badge></div><p className="mt-3 text-sm text-muted-foreground">{labels.effectiveOn}: {assignment.effective_from}</p></button> })}</div>}
