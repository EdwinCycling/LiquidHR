...
export function SelectableTimelineList({ items, labels }: {
  items: TimelineItem[]
  labels: { current: string; history: string; future: string; empty: string; close: string }
}) {
  ...
  return <>
    <div className="divide-y divide-subtle border-y border-subtle">
      {items.map((item) => {
        const statusLabel = item.status === 'current' ? labels.current : item.status === 'future' ? labels.future : labels.history
        const statusTone = item.status === 'current' ? 'info' : item.status === 'future' ? 'info' : 'neutral'
        return <button type="button" key={item.id} onClick={() => setSelected(item)} className="group w-full cursor-pointer px-2 py-4 text-left transition-colors hover:bg-surface-subtle focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><p className="text-sm font-medium text-muted-foreground">{statusLabel}</p><p className="mt-1 font-semibold">{item.title}</p></div>
          <Badge tone={statusTone}>{item.period}</Badge>
        </div>
