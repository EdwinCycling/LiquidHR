...
export interface EmploymentCreateFormProps {
  ...
  labels: {
    ...
    contractShortWarning: string; startDatePastWarning: string; startDateFutureWarning: string; probationLongWarning: string; probationOutsideContract: string; firstContractStartDateHelp: string; contractStartDateMinimumHelp: string
...
function addToDate(value: string, mode: 'FOUR_WEEKS' | 'ONE_MONTH' | 'TWO_MONTHS'): string {
  ...
  else return addMonths(value, mode === 'ONE_MONTH' ? 1 : 2)
...
function averageDayHours(weeks: Array<Record<DayKey, string>>): Record<DayKey, number> {
  return Object.fromEntries(dayKeys.map((day) => [day, weeks.reduce((sum, week) => sum + (Number(week[day]) || 0), 0) / weeks.length])) as Record<DayKey, number>
}
...
function workPatternDays(weeks: Array<Record<DayKey, string>>): Array<{
  ...
  return weeks.flatMap((week, weekIndex) => dayKeys.map((day, dayIndex) => {
    const scheduledMinutes = Math.round((Number(week[day]) || 0) * 60)
    return {
...
function defaultDraft(options: EmploymentCreationOptions, copyPreviousData = false): Draft {
  ...
  const draft: Draft = {
    ...
    employmentNumber: options.nextEmploymentNumber, employmentType: 'EMPLOYEE', isPrimary: !options.hasActivePrimaryEmployment,
...
export function EmploymentCreateForm({ employeeId, options: initialOptions, showNavigation = true, showPayrollChoice = false, onStepChange, onPayrollChoiceChange, onSaving, onSaveFailed, onSaved, copyPreviousData = false, canScrollDown = false, moreDataAvailable, labels }: EmploymentCreateFormProps) {
  ...
  const rosterWeekTotals = rosterWeeks.map((week) => dayKeys.reduce((sum, day) => sum + (Number(week[day]) || 0), 0))
  const rosterAverage = rosterWeekTotals.reduce((sum, total) => sum + total, 0) / rosterWeekTotals.length
  const rosterMatches = Math.abs(rosterAverage - Number(draft.weeklyHours)) < 0.0001
  const rosterHasNegativeHours = rosterWeeks.some((week) => dayKeys.some((day) => Number(week[day]) < 0))
  const rosterHasInvalidHours = rosterWeeks.some((week) => dayKeys.some((day) => week[day].trim() === '' || !Number.isFinite(Number(week[day])) || Number(week[day]) > 24))
  const contractTooShort = draft.durationType === 'DEFINITE' && Boolean(draft.endsOn) && draft.endsOn < addMonths(draft.startsOn, 1)
  const startDateIsTooFarInPast = Boolean(draft.startsOn) && draft.startsOn < addMonths(todayDateOnly(), -1)
  const startDateIsTooFarInFuture = Boolean(draft.startsOn) && draft.startsOn > addMonths(todayDateOnly(), 2)
  const probationTooLong = draft.probationApplies && Boolean(draft.probationEndsOn) && draft.probationEndsOn > addMonths(draft.startsOn, 3)
  ...
  function valid(key: StepKey): boolean {
    return isEmploymentWizardStepValid(key, draft, {
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const payload = payrollDetails ? {
      ...
      costAllocation: { validFrom: draft.startsOn, allocations: draft.allocations.map((allocation) => ({ costCenterId: allocation.costCenterId, costCarrierId: draft.allocations[0]?.costCarrierId ?? allocation.costCarrierId, percentage: Number(allocation.percentage) })) },
    } : {
      employment: { employmentNumber: draft.employmentNumber, employmentType: draft.employmentType, startsOn: draft.startsOn, seniorityDate: draft.seniorityDate, countryCode: draft.countryCode, isPrimary: draft.isPrimary },
    }
  ...
  return <employmentFieldLabelsContext.Provider value={labels}><form onSubmit={(event) => void submit(event)} className="flex min-h-full min-w-0 flex-col rounded-2xl border bg-surface p-5 shadow-sm">
    ...
    {currentStep === 'contract' && <WizardStep title={labels.stepContract}>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field required label={labels.duration}><select className={inputClass} value={draft.durationType} onChange={(event) => update('durationType', event.target.value as DurationType)}><option value="INDEFINITE">{labels.indefinite}</option><option value="DEFINITE">{labels.definite}</option></select></Field>
        <div className="grid gap-4 sm:col-span-2 sm:grid-cols-2">
          ...
          {draft.durationType === 'DEFINITE' && <Field required label={labels.endDate}><input type="date" min={draft.startsOn} className={inputClass} value={draft.endsOn} onChange={(event) => update('endsOn', event.target.value)} /></Field>}
        </div>
        <Field required label={labels.probation}><select className={inputClass} value={String(draft.probationApplies)} onChange={(event) => update('probationApplies', event.target.value === 'true')}><option value="false">{labels.no}</option><option value="true">{labels.yes}</option></select></Field>
      ...
      <div className="mt-4 space-y-2 text-sm" aria-live="polite">
        ...
        {probationTooLong && <p className="text-warning">{labels.probationLongWarning}</p>}
        {draft.probationApplies && draft.durationType === 'DEFINITE' && draft.probationEndsOn && draft.endsOn && draft.probationEndsOn > draft.endsOn && <p role="alert" className="text-destructive">{labels.probationOutsideContract}</p>}
      </div>
    </WizardStep>}
    ...
    {currentStep === 'schedule' && <WizardStep title={labels.stepSchedule}>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field required label={labels.weeklyHours}><input type="number" min="0" max="50" step="0.01" className={inputClass} value={draft.weeklyHours} onChange={(event) => distributeHours(event.target.value)} /><span className="text-xs font-normal text-muted-foreground">{labels.fulltimeReference}: {selectedFulltimeHours} {labels.hoursPerWeek}</span></Field>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3"><h4 className="font-semibold">{labels.roster}</h4><button type="button" className="button-secondary" onClick={() => setDraft((current) => ({ ...current, twoWeekRoster: !current.twoWeekRoster }))}>{draft.twoWeekRoster ? labels.removeSecondWeek : labels.addSecondWeek}</button></div>
      ...
      <div className="mt-2 space-y-1 text-sm" aria-live="polite">
        {(Number(draft.weeklyHours) > 50 || Number(draft.weeklyHours) < 0) && <p role="alert" className="text-destructive">{labels.weeklyHoursInvalid}</p>}
    ...
    {currentStep === 'review' && <WizardStep title={labels.completeSummary}><p className="text-sm text-muted-foreground">{labels.createHint}</p>{invalidStepKeys.length > 0 && <div role="alert" className="mt-4 rounded-xl border border-warning/40 bg-warning-surface p-4 text-sm"><p className="font-semibold">{labels.reviewMissingFields}</p><div className="mt-2 flex flex-wrap gap-2">{invalidStepKeys.map((key) => <button type="button" className="button-secondary text-xs" key={key} onClick={() => setStep(stepKeys.indexOf(key))}>{labels.reviewEditStep}: {stepLabels[key]}</button>)}</div></div>}<dl className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><Summary label={labels.administration} value={options.administrations.find((item) => item.id === draft.administrationId)?.name ?? ''} /><Summary label={labels.employmentNumber} value={draft.employmentNumber} /><Summary label={labels.workerType} value={draft.employmentType} /><Summary label={labels.startDate} value={draft.startsOn} />{payrollDetails && <><Summary label={labels.laborConditions} value={selectedLaborSet?.name ?? ''} /><Summary label={labels.weeklyHours} value={draft.weeklyHours} /><Summary label={labels.department} value={options.departments.find((item) => item.id === draft.departmentId)?.name ?? ''} /><Summary label={labels.job} value={selectedJob?.name ?? ''} />{options.canWriteSalary && <Summary label={Number(draft.weeklyHours) === selectedFulltimeHours ? labels.fulltimeSalary : labels.parttimeSalary} value={draft.salaryBasis === 'MINIMUM_WAGE' ? (minimumRate ? `€ ${money(minimumRate.hourlyAmount)}` : '') : `€ ${money(Number(draft.weeklyHours) === selectedFulltimeHours ? draft.fulltimeAmount : draft.parttimeAmount)}`} />}</>}</dl></WizardStep>}

    {state === 'failed' && <p role="alert" className="mt-4 text-sm text-destructive">{errorMessage(errorCode, labels)}</p>}{state === 'saved' && <p className="mt-4 text-sm text-success">{labels.saved}</p>}<div className="sticky bottom-0 z-10 mt-auto flex items-center justify-between gap-3 border-t border-border/70 bg-surface/95 py-2.5 backdrop-blur-sm"><button type="button" className="button-secondary shrink-0" disabled={step === 0 || state === 'saving' || optionsLoading} onClick={() => setStep((current) => Math.max(0, current - 1))}>{labels.previous}</button><EmploymentScrollHint label={moreDataAvailable} visible={canScrollDown} />{step < stepKeys.length - 1 ? <button type="button" className="button-primary shrink-0" disabled={state === 'saving' || optionsLoading || payrollChoicePending} onClick={next}>{labels.next}</button> : <button type="submit" className="button-primary shrink-0" disabled={state === 'saving' || optionsLoading}>{labels.submit}</button>}</div>
...
function errorMessage(code: string, labels: EmploymentCreateFormProps['labels']): string {
  ...
  if (code === 'PROBATION_DATE_INVALID' || code === 'CONTRACT_END_DATE_REQUIRED' || code === 'INITIAL_TIMELINE_DATE_MISMATCH' || code === 'EMPLOYMENT_INPUT_INVALID' || code === 'COMPLETE_EMPLOYMENT_PAYLOAD_INVALID') return labels.requiredFields
...
function RosterWeekFields({ title, days, labels, inputClass, onChange }: { title: string; days: Record<DayKey, string>; labels: EmploymentCreateFormProps['labels']; inputClass: string; onChange: (day: DayKey, value: string) => void }) {
  return <section className="mt-4"><h5 className="text-sm font-semibold">{title}</h5><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-7">{dayKeys.map((day) => <Field required key={day} label={labels[day]} labelClassName="min-h-10 items-start"><input type="number" min="0" max="24" step="0.25" className={inputClass} value={days[day]} onChange={(event) => onChange(day, event.target.value)} /></Field>)}</div></section>
}
