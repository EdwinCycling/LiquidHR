...
import { hasOrganizationChartResultFilter } from '@/lib/organization-chart/page-state'
...
function OrganizationChartExplorerState({ graph, query, labels, defaultDate }: OrganizationChartExplorerProps) {
  ...
  const hasResultFilter = hasOrganizationChartResultFilter(query)
  ...
  return (
    <>
      <section className="space-y-4">
        <PageToolbar
          end={hasResultFilter ? <p aria-live="polite" className="text-sm font-semibold text-muted-foreground">{matchText}</p> : undefined}
      ...
      {!hasStructure ? (
        ...
        <div className="mt-6">
          {hasFiltering && graph.metadata.matchCount === 0 ? <div className="mb-4 rounded-2xl border border-warning/25 bg-warning-surface px-4 py-3"><h2 className="text-sm font-semibold text-warning">{labels.noMatchesTitle}</h2><p className="mt-0.5 text-xs leading-5 text-warning">{labels.noMatchesBody}</p></div> : null}
