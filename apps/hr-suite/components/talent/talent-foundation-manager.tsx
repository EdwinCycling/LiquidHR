  ...
  return <div className="space-y-5">
    <SettingsAccordion initialOpen="models" sections={[
      ...
      { id: 'seniority-add', title: labels.addSeniority, children: (
        <section className="rounded-2xl border bg-surface p-5 shadow-sm"><p className="text-sm text-muted-foreground">{labels.seniorities}</p>{canManage ? <button className="button-secondary mt-4 inline-flex items-center gap-2" onClick={() => setModal({ kind: 'seniority', id: null, code: '', name: '', description: '', sortOrder: String(foundation.seniorities.length + 1), status: 'ACTIVE' })} type="button"><Plus size={16} />{labels.addSeniority}</button> : null}</section>
      ) },
