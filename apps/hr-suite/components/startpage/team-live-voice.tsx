...
export function pauseLiveOutput(audio: Pick<HTMLAudioElement, 'pause'> | null, active: boolean): void {
...
export function TeamLiveVoice({ departmentId, contextName, enabled = true, labels, locale }: { departmentId?: string; contextName: string; enabled?: boolean; labels: TeamLiveVoiceLabels; locale: string }): ReactElement {
  ...
  async function start(): Promise<void> {
    ...
    function interrupt(): void {
      if (!live()) return
      // GPT-Live is full-duplex and handles barge-in on the media transport.
      // The response.cancel/output_audio_buffer.clear commands belong to the
      // legacy Realtime event contract and are invalid Live client events.
      pauseLiveOutput(run.audio ?? null, true)
    }
