        ...
        <div className="mt-4 max-h-[32rem] space-y-2 overflow-y-auto pr-1">
          {filteredRoles.map((role) => {
            ...
            return <button aria-pressed={selectedRoleId === role.id} className={`w-full rounded-md border px-3 py-3 text-left transition ${selectedRoleId === role.id ? 'border-primary bg-accent text-accent-foreground shadow-sm' : 'bg-background text-foreground hover:border-primary/40'}`} key={role.id} onClick={() => selectRole(role.id)} type="button">
              <span className="flex items-start justify-between gap-3"><span><span className="block font-medium">{role.name}</span><span className="mt-0.5 block text-xs text-muted-foreground">{role.code} Â· {role.is_system ? labels.systemRole : labels.tenantRole}</span></span><span className="rounded-full bg-muted px-2 py-0.5 text-xs font-semibold text-muted-foreground">{count}</span></span>
...
function AuthorizationHeatmap({ labels, onInspectCoverage, permissions, rolePermissions, roles }: { labels: AuthorizationLabels; onInspectCoverage: (roleId: string, category: string) => void; permissions: Permission[]; rolePermissions: RolePermission[]; roles: Role[] }) {
  ...
  return <section className="rounded-xl border bg-surface p-4 sm:p-6"><header className="border-b pb-5"><h2 className="text-xl font-semibold text-foreground">{labels.overviewTitle}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{labels.overviewSubtitle}</p></header><div className="mt-5 rounded-lg border bg-accent p-4"><p className="font-semibold text-accent-foreground">{labels.scopeNoticeTitle}</p><p className="mt-1 text-sm leading-6 text-muted-foreground">{labels.scopeNotice}</p></div>
    ...
    <div className="mt-6 space-y-4 lg:hidden">{roles.map((role) => <article className="rounded-xl border bg-background p-4" key={role.id}><div><span className="font-semibold text-foreground">{role.name}</span><span className="ml-2 text-xs text-muted-foreground">{role.code}</span></div><div className="mt-4 space-y-3">{groups.map(([category, items]) => { const coverage = permissionCoverage(rolePermissionSet(role.id, rolePermissions), items.map((item) => item.id)); return <button className="block w-full text-left" key={category} onClick={() => onInspectCoverage(role.id, category)} type="button"><span className="flex items-center justify-between gap-3 text-xs"><span className="font-medium text-foreground">{category}</span><span className="text-muted-foreground">{coverage.assigned}/{coverage.total} Â· {coverage.percentage}%</span></span><span className="mt-1.5 block h-2 overflow-hidden rounded-full bg-muted"><span className="block h-full rounded-full bg-primary" style={{ width: `${coverage.percentage}%` }} /></span></button> })}</div></article>)}</div>
  </section>
}
