...
function FilterDropdown({ label, value, children }: { label: string; value: string; children: ReactNode }) {
  ...
  return <div className="relative min-w-52" ref={ref}><span className="text-xs font-semibold uppercase tracking-[.08em] text-muted-foreground">{label}</span><button aria-expanded={open} className="dropdown-trigger mt-2 text-sm" onClick={() => setOpen((current) => !current)} type="button"><span className="truncate">{value}</span><ChevronDown aria-hidden="true" className={`size-4 text-muted-foreground transition ${open ? 'rotate-180' : ''}`} /></button>{open ? <div className="dropdown-menu absolute left-0 top-full z-40 mt-2 w-full min-w-64 p-3">{children}</div> : null}</div>
}

export function UpcomingEventsReportView({ report, query, labels, locale, dateFormat, returnTo }: { report: UpcomingEventsReport; query: { types: UpcomingEventType[]; periodDays: PeriodDays; departmentIds: string[] }; labels: Labels; locale: string; dateFormat: DateFormat; returnTo: string }) {
  const router = useRouter(); const searchParams = useSearchParams(); const [departmentSearch, setDepartmentSearch] = useState('')
  ...
  function update(next: Partial<typeof query>): void { const nextQuery = { types: next.types ?? query.types, periodDays: next.periodDays ?? query.periodDays, departmentIds: next.departmentIds ?? query.departmentIds }; router.push(buildInsightApplyHref(searchParams, upcomingEventsQueryParams(nextQuery)), { scroll: false }) }
