...
import { Bell, CalendarDays, Clock3, X } from "lucide-react";
import {
  buildMonthDays,
...
export function HrMonthCalendar({
  ...
  const selectedWorkDay =
    selection?.type === "employee"
      ...
      : undefined;
  const selectedHoliday = selection
    ? holidayByDate.get(selection.date)
    : undefined;
  return (
    <div className="relative">
      <div className="overflow-auto rounded-2xl border bg-surface shadow-sm">
        <div className="min-w-max">
          <div
            className="sticky top-0 z-20 grid bg-surface"
            style={{
          ...
          >
            ...
            {days.map((day) => {
              ...
              return (
                <button
                  className={`border-b border-r p-1.5 text-center text-[11px] font-semibold hover:bg-accent ${holiday ? "bg-accent text-accent-foreground" : ""}`}
                  key={day}
          ...
          {employees.map((employee) => (
            ...
            >
              ...
              {days.map((day) => {
                ...
                return (
                  <button
                    className={`relative flex min-h-14 flex-col items-center justify-center gap-1 border-b border-r p-1 hover:bg-accent/70 ${holiday ? "bg-accent/35" : ""}`}
                    key={day}
                    onClick={() =>
      ...
      {selection ? (
        <aside className="fixed inset-y-0 right-0 z-50 w-[min(28rem,calc(100vw-2rem))] overflow-y-auto border-l bg-surface p-5 shadow-2xl">
          ...
          {selection.type === "employee" ? (
            <>
              ...
              <button
                className="mt-6 w-full rounded-xl border border-dashed p-4 text-sm font-semibold text-muted-foreground"
                disabled
                type="button"
              >
                {labels.actionsLater}
            ...
            <div className="mt-5 space-y-2">
              {generalReminders
                .filter((reminder) => reminder.date === selection.date)
                .map((reminder) => (
                  <div
                    className="flex items-center gap-2 rounded-xl bg-muted p-3 text-sm"
                    key={reminder.id}
              ...
              {events
                .filter((event) => event.eventDate === selection.date)
                .map((event) => (
                  <Link
                    className="flex items-center gap-2 rounded-xl bg-muted p-3 text-sm"
                    href={event.sourceHref}
