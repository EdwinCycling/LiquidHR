...
export function DocumentCategoryManager({ categories, labels }: { categories: DocumentCategory[]; labels: Record<string, string> }) {
  ...
  async function remove(item: DocumentCategory) { if (!window.confirm(labels.deleteConfirm)) return; const response = await fetch(`/api/master-data/document-categories/${item.id}`, { method: 'DELETE' }); if (!response.ok) { const body = await response.json() as { error?: string }; window.alert(body.error === 'DOCUMENT_CATEGORY_IN_USE' ? labels.inUse : labels.deleteFailed); return }; router.refresh() }
  return <div className="space-y-4"><form className="grid gap-3 rounded-xl border p-4 sm:grid-cols-[10rem_minmax(0,1fr)_minmax(0,1.2fr)_auto]" onSubmit={(event) => void create(event)}><input className="form-field" name="code" placeholder={labels.code} required /><input className="form-field" name="name" placeholder={labels.name} required /><input className="form-field" name="description" placeholder={labels.description} /><button className="button-primary" disabled={saving} type="submit">{saving ? labels.saving : labels.add}</button></form><ul className="grid gap-2">{categories.map((item) => <li className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4" key={item.id}><span><strong>{item.name}</strong><span className="ml-2 text-xs text-muted-foreground">{item.code}</span>{item.description && <span className="mt-1 block text-sm text-muted-foreground">{item.description}</span>}</span><div className="flex flex-wrap gap-2"><button className="button-secondary" onClick={() => void toggle(item)} type="button">{item.is_active ? labels.deactivate : labels.activate}</button><button className="button-secondary" onClick={() => void remove(item)} type="button">{labels.delete}</button></div></li>)}</ul></div>
}
