  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border bg-surface p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><button className="button-secondary" disabled={saving} onClick={() => setModal({ kind: 'group', mode: 'create' })} type="button"><Plus size={16} />{labels.addNew} {labels.groups.toLocaleLowerCase()}</button><button className="button-primary" disabled={saving || activeGroups.length === 0} onClick={() => setModal({ kind: 'job', mode: 'create' })} type="button"><Plus size={16} />{labels.addNew} {labels.jobs.toLocaleLowerCase()}</button></div>
      </div>
      <div className="mt-6 grid gap-3 lg:grid-cols-[minmax(0,1fr)_14rem_14rem]">
