...
interface Group { id: string; code: string; name: string; description: string | null; is_active: boolean }
interface JobRevision { id: string; name: string; description: string | null; valid_from: string; valid_until: string | null }
interface Job { id: string; code: string; job_group_id: string; job_group_ids: string[]; is_active: boolean; job_revisions: JobRevision[] }
interface Labels {
  ...
  filters: string; graphTitle: string; graphSubtitle: string; jobsInGroup: string; noJobsInGroup: string
}
...
export function JobCatalogManager({ groups, jobs, labels }: { groups: Group[]; jobs: Job[]; labels: Labels }) {
  ...
  const filteredGroups = useMemo(() => {
    ...
    return [...groups].filter((group) => `${group.code} ${group.name} ${group.description ?? ''}`.toLocaleLowerCase('nl-NL').includes(query)).sort((left, right) => compareItems(left.code, right.code, sortMode, left.name, right.name, left.is_active, right.is_active, sortAscending))
  }, [groups, search, sortAscending, sortMode])
  const filteredJobs = useMemo(() => {
    const query = search.trim().toLocaleLowerCase('nl-NL')
    return [...jobs].filter((job) => {
      const revision = latestRevision(job)
      const matchesGroup = !groupFilter || job.job_group_ids.includes(groupFilter)
      return matchesGroup && `${job.code} ${revision?.name ?? ''} ${revision?.description ?? ''}`.toLocaleLowerCase('nl-NL').includes(query)
    }).sort((left, right) => {
  ...
  }, [groupFilter, jobs, search, sortAscending, sortMode])
  ...
  async function submitGroup(event: FormEvent<HTMLFormElement>, groupId?: string): Promise<void> {
    ...
    const body = { code: form.get('code'), name: form.get('name'), description: form.get('description') || null }
    const success = await request(groupId ? `/api/master-data/job-groups/${groupId}` : '/api/master-data/job-groups', groupId ? 'PATCH' : 'POST', body)
    if (success) { formElement.reset(); setModal(null) }
  }
  ...
  async function submitJob(event: FormEvent<HTMLFormElement>, jobId?: string): Promise<void> {
    ...
    const body = { code: form.get('code'), name: form.get('name'), description: form.get('description') || null, jobGroupIds }
    const success = await request(jobId ? `/api/master-data/jobs/${jobId}` : '/api/master-data/jobs', jobId ? 'PATCH' : 'POST', body)
    if (success) { formElement.reset(); setModal(null) }
  }
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border bg-surface p-5 shadow-sm sm:p-6">
      ...
      <details className="group mt-6 rounded-xl border bg-muted/20">
        ...
        <div className="border-t p-4">
          ...
          <button className="mt-3 inline-flex items-center gap-2 text-xs font-semibold text-muted-foreground hover:text-foreground" onClick={() => setSortAscending((current) => !current)} type="button"><ArrowDownUp size={14} />{labels.sortBy}: {sortAscending ? 'A–Z' : 'Z–A'}</button>
        </div>
      </details>
    </section>
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <CatalogList title={labels.groups} count={filteredGroups.length} empty={search || groupFilter ? labels.noResults : labels.empty}>
        {filteredGroups.map((group) => <div className="flex flex-col gap-3 border-b py-4 last:border-b-0 sm:flex-row sm:items-center sm:justify-between" key={group.id}>
          <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong>{group.code}</strong><span className="truncate">{group.name}</span><StatusPill active={group.is_active} labels={labels} /></div><p className="mt-1 text-xs text-muted-foreground">{jobs.filter((job) => job.job_group_ids.includes(group.id)).length} {labels.relatedJobs}{group.description ? ` · ${group.description}` : ''}</p></div>
