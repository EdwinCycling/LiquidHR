...
interface Labels {
  ...
  employeePortalMode: string; managerPortalMode: string; focusOnly: string; focusAndFull: string
...
export function HrGroupManager({ activeGroup, canWrite, labels }: { activeGroup: HrGroupContextOption; canWrite: boolean; labels: Labels }) {
  ...
  const [employeePortalMode, setEmployeePortalMode] = useState(activeGroup.employeePortalMode ?? 'FOCUS_AND_FULL')
  const [managerPortalMode, setManagerPortalMode] = useState(activeGroup.managerPortalMode ?? 'FOCUS_AND_FULL')
  ...
  const groupDirty = groupName !== activeGroup.name || groupDescription !== (activeGroup.description ?? '') || employeePortalMode !== (activeGroup.employeePortalMode ?? 'FOCUS_AND_FULL') || managerPortalMode !== (activeGroup.managerPortalMode ?? 'FOCUS_AND_FULL')
  ...
  async function saveGroup(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try { const response = await fetch('/api/hr-groups', { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: groupName, description: groupDescription, employeePortalMode, managerPortalMode }) }); if (!response.ok) throw new Error(labels.failed); setGroupDrawerOpen(false); setSaved(true); router.refresh() } catch (saveError) { setError(saveError instanceof Error ? saveError.message : labels.failed) } finally { setSaving(false) }
  }
  ...
  return <section className="space-y-6">
    ...
    <FormDrawer cancelLabel={labels.cancel} closeLabel={labels.close} description={labels.subtitle} dirty={groupDirty} dirtyProtection={{ description: labels.failed, discardLabel: labels.cancel, keepEditingLabel: labels.cancel, title: labels.cancel }} onDiscard={() => { setGroupName(activeGroup.name); setGroupDescription(activeGroup.description ?? ''); setEmployeePortalMode(activeGroup.employeePortalMode ?? 'FOCUS_AND_FULL'); setManagerPortalMode(activeGroup.managerPortalMode ?? 'FOCUS_AND_FULL') }} onOpenChange={setGroupDrawerOpen} onSubmit={(event) => void saveGroup(event)} open={groupDrawerOpen} saveLabel={labels.saveGroup} saving={saving} title={labels.groupName}><FormField control={<TextInput disabled value={activeGroup.code} />} label={labels.groupCode} /><FormField control={<TextInput onChange={(event) => setGroupName(event.target.value)} required value={groupName} />} label={labels.groupName} required /><FormField control={<Textarea maxLength={1000} onChange={(event) => setGroupDescription(event.target.value)} value={groupDescription} />} label={labels.groupDescription} /><FormField control={<DropdownSelect aria-label={labels.employeePortalMode} onChange={(event) => setEmployeePortalMode(event.target.value as 'FOCUS_ONLY' | 'FOCUS_AND_FULL')} value={employeePortalMode}><option value="FOCUS_ONLY">{labels.focusOnly}</option><option value="FOCUS_AND_FULL">{labels.focusAndFull}</option></DropdownSelect>} label={labels.employeePortalMode} /><FormField control={<DropdownSelect aria-label={labels.managerPortalMode} onChange={(event) => setManagerPortalMode(event.target.value as 'FOCUS_ONLY' | 'FOCUS_AND_FULL')} value={managerPortalMode}><option value="FOCUS_ONLY">{labels.focusOnly}</option><option value="FOCUS_AND_FULL">{labels.focusAndFull}</option></DropdownSelect>} label={labels.managerPortalMode} />{error ? <p className="text-sm text-destructive" role="alert">{error}</p> : null}</FormDrawer>
