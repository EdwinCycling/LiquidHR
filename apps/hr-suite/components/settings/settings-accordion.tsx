...
export function SettingsAccordion({ sections, initialOpen, alwaysOpen = false, onOpenChange }: { sections: SettingsAccordionSection[]; initialOpen?: string; alwaysOpen?: boolean; onOpenChange?: (sectionId: string | null) => void }) {
  ...
  return <div className="space-y-3">{sections.map((section) => { const isOpen = open === section.id; return <section className="scroll-mt-6 rounded-xl border bg-surface shadow-sm" id={`settings-section-${section.id}`} key={section.id}><button aria-controls={`settings-panel-${section.id}`} aria-expanded={isOpen} className="flex w-full items-center justify-between gap-4 p-5 text-left" onClick={() => { const nextOpen = isOpen && !alwaysOpen ? null : section.id; setOpen(nextOpen); onOpenChange?.(nextOpen) }} type="button"><span className="text-base font-semibold">{section.title}</span><ChevronDown className={`shrink-0 transition-transform ${isOpen ? 'rotate-180' : ''}`} size={18} /></button>{isOpen ? <div className="border-t p-5" id={`settings-panel-${section.id}`}>{section.children}</div> : null}</section> })}</div>
}
