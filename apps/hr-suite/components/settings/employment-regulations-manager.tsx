...
function RegulationDialog({ state, labels, onCancel, onSave }: { state: DialogState; labels: Labels; onCancel: () => void; onSave: (input: { name: string; validFrom: string; standardHoursPerWeek: number; probationMaximumMonths: 1 | 2 }) => Promise<void> }) {
  ...
  return <div aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/30 p-4" role="dialog">
    <div className="w-full max-w-lg rounded-2xl border bg-background p-5 shadow-xl">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        <label className="grid gap-1 text-sm font-medium sm:col-span-2">{labels.probationMaximum}<select className="form-field" value={probationMaximumMonths} onChange={(event) => setProbationMaximumMonths(event.target.value)}><option value="1">{labels.probationOneMonth}</option><option value="2">{labels.probationTwoMonths}</option></select></label>
      </div>
