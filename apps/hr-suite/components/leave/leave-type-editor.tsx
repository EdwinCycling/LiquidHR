...
type Labels = {
  ...
  leaveSettings?: { title: string; family: string; vacationFamily: string; otherFamily: string; allowLimitOverrun: string; pinInCalendar: string; requiresManagerApproval: string; notifyManagerOnRequest: string; requiresManagerApprovalOnCancellation: string }
...
export function LeaveTypeEditor({ mode, existing, catalog, labels, workHourSettings, initialTab = 'base', initialCategory = 'REGULAR_WORK', backHref = '/settings/leave-accrual', initialProfileId }: { mode: 'leave' | 'work'; existing?: LeaveType | WorkHourType; catalog?: LeaveCatalog; labels: Labels; workHourSettings?: { initial: import('@/lib/leave/leave-service').OvertimeSettingsPageData; labels: OvertimePanelLabels; mode: OvertimePanelMode }; initialTab?: Tab; initialCategory?: WorkHourType['category']; backHref?: string; initialProfileId?: string }) {
  ...
  const [family, setFamily] = useState<LeaveType['family']>(leave?.family ?? 'OTHER')
  ...
  const setting = (label: string, checked: boolean, onChange: (value: boolean) => void, disabled = readOnly) => <Checkbox checked={checked} disabled={disabled} label={label} onChange={(event) => onChange(event.target.checked)} />
  ...
  return <div className="space-y-5">
     <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex gap-1 overflow-x-auto border-b">{(Object.keys(labels.tabs) as Tab[]).map((item) => <Button aria-current={tab === item ? 'page' : undefined} className="rounded-b-none" key={item} onClick={() => setTab(item)} size="sm" type="button" variant={tab === item ? 'primary' : 'ghost'}>{tab === item ? '✓ ' : ''}{labels.tabs[item]}</Button>)}</div><div className="flex gap-2">{!existing ? <Button onClick={() => router.push(backHref)} type="button" variant="secondary">{labels.cancel}</Button> : null}<Button disabled={saveBlocked} onClick={() => void save()} type="button">{state === 'saving' ? labels.saving : labels.save}</Button>{existing ? <Button disabled={state === 'saving'} onClick={() => setArchiveOpen(true)} type="button" variant="danger">{labels.archive}</Button> : null}</div></div>

     {tab === 'base' ? <Surface className="p-6"><div className="grid gap-5 md:grid-cols-2"><FormField control={<TextInput disabled={readOnly} maxLength={160} onChange={(event) => setName(event.target.value)} required value={name} />} label={labels.name} required /><div className="grid gap-2 text-sm font-medium"><span>{labels.color}</span><div className="flex flex-wrap gap-2" role="radiogroup" aria-label={labels.color}>{LEAVE_COLOR_OPTIONS.map((option) => <button aria-label={labels.colorOptions[option.labelKey] ?? option.labelKey} aria-pressed={colorCode === option.value} className={`size-8 rounded-full border-2 transition disabled:cursor-not-allowed disabled:opacity-60 ${colorCode === option.value ? 'border-foreground ring-2 ring-primary/30' : 'border-transparent'}`} disabled={readOnly} key={option.value} onClick={() => setColorCode(option.value)} style={{ backgroundColor: option.value }} type="button" />)}</div><span className="inline-flex items-center gap-2 text-xs text-muted-foreground"><span aria-hidden="true" className="size-3 rounded-full" style={{ backgroundColor: colorCodeToCssValue(colorCode) }} />{labels.colorOptions[LEAVE_COLOR_OPTIONS.find((option) => option.value === colorCode)?.labelKey ?? colorCode] ?? colorCode}</span></div></div>
      {isLeave && labels.leaveSettings ? <div className="mt-7 border-t pt-6"><h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">{labels.leaveSettings.title}</h2><div className="mt-4 grid gap-4 md:grid-cols-2"><FormField control={<DropdownSelect aria-label={labels.leaveSettings.family} onChange={(event) => setFamily(event.target.value as LeaveType['family'])} value={family}><option value="VACATION">{labels.leaveSettings.vacationFamily}</option><option value="OTHER">{labels.leaveSettings.otherFamily}</option></DropdownSelect>} label={labels.leaveSettings.family} required />{setting(labels.selfService, isSelfService, setIsSelfService)}{setting(labels.leaveSettings.allowLimitOverrun, allowLimitOverrun, setAllowLimitOverrun)}{setting(labels.leaveSettings.pinInCalendar, pinInCalendar, setPinInCalendar)}{setting(labels.leaveSettings.requiresManagerApproval, requiresManagerApproval, setRequiresManagerApproval)}{setting(labels.leaveSettings.notifyManagerOnRequest, notifyManagerOnRequest, setNotifyManagerOnRequest)}{setting(labels.leaveSettings.requiresManagerApprovalOnCancellation, requiresManagerApprovalOnCancellation, setRequiresManagerApprovalOnCancellation)}{setting(labels.activeLabel, isActive, setIsActive)}</div></div> : null}{!isLeave && labels.workSettings ? <div className="mt-7 border-t pt-6"><h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">{labels.workSettings.title}</h2><div className="mt-4 grid gap-4 md:grid-cols-2">{setting(labels.requiresManagerApproval, workRequiresManagerApproval, setWorkRequiresManagerApproval)}{setting(labels.notifyManagerOnEntry, notifyManagerOnEntry, setNotifyManagerOnEntry)}{setting(labels.activeLabel, isActive, setIsActive)}{setting(labels.selfService, workSelfService, setWorkSelfService)}</div></div> : null}
     </Surface> : null}
    ...
    {tab === 'limits' && isLeave ? <div className="space-y-5">
      <section className="rounded-2xl border bg-surface p-6 shadow-sm">
        <div className="grid gap-5 md:max-w-2xl">
           <FormField control={<DropdownSelect aria-label={labels.entitlement} onChange={(event) => { setEntitlementMode(event.target.value as LeaveType['entitlement_mode']); setRuleEditorOpen(event.target.value === 'ACCRUAL') }} value={entitlementMode}><option value="ACCRUAL">{labels.accrual}</option><option value="UNLIMITED">{labels.unlimited}</option><option value="ANNUAL_HOURS_CAP">{labels.annualCap}</option><option value="ANNUAL_HOURS_FTE_CAP">{labels.annualFteCap}</option><option value="OVERTIME_HOURS">{labels.overtimeCap}</option></DropdownSelect>} label={labels.entitlement} required />
           {entitlementMode === 'ANNUAL_HOURS_CAP' ? <FormField control={<TextInput className="w-32" min="0" onChange={(event) => setAnnualHoursCap(event.target.value)} step="0.01" type="number" value={annualHoursCap} />} label={labels.annualCapValue} /> : null}
           {entitlementMode === 'ANNUAL_HOURS_FTE_CAP' ? <FormField control={<TextInput className="w-32" min="0" onChange={(event) => setAnnualHoursFteCap(event.target.value)} step="0.01" type="number" value={annualHoursFteCap} />} label={labels.annualFteCapValue} /> : null}
           {entitlementMode === 'OVERTIME_HOURS' ? <fieldset className="rounded-xl border bg-muted/20 p-4"><legend className="px-1 text-sm font-semibold">{labels.overtimeTypes}</legend><p className="mt-1 text-xs text-muted-foreground">{labels.overtimeTypesHelp}</p><TextInput className="mt-3" onChange={(event) => setOvertimeSearch(event.target.value)} placeholder={labels.search} type="search" value={overtimeSearch} /><div className="mt-3 max-h-56 space-y-2 overflow-y-auto">{visibleOvertimeTypes.map((item) => <Checkbox checked={overtimeWorkHourTypeIds.includes(item.id)} key={item.id} label={item.name} onChange={() => toggleOvertimeType(item.id)} />)}{visibleOvertimeTypes.length === 0 ? <p className="text-sm text-muted-foreground">{labels.noOvertimeTypes}</p> : null}</div><p className="mt-2 text-xs text-muted-foreground">{labels.selectedCount.replace('{count}', String(overtimeWorkHourTypeIds.length))}</p></fieldset> : null}
        </div>
      </section>
      {entitlementMode === 'ACCRUAL' && catalog && labels.ruleEditor && ruleEditorOpen ? <AccrualRuleEditor catalog={catalog} copyFromRuleId={copyFromRuleId} ensureLeaveTypeId={() => save(false, true)} initialProfileId={initialProfileId} labels={labels.ruleEditor} leaveTypeId={existing?.id ?? createdId ?? undefined} onCancel={() => { setEditingRuleId(undefined); setCopyFromRuleId(undefined); setRuleEditorOpen(false) }} onSaved={() => { setEditingRuleId(undefined); setCopyFromRuleId(undefined); setRuleEditorOpen(false) }} ruleId={editingRuleId} /> : null}
      {entitlementMode === 'ACCRUAL' && catalog && existing && labels.ruleList && !ruleEditorOpen ? <LeaveAccrualRuleList catalog={catalog} leaveTypeId={existing.id} profileId={initialProfileId} labels={labels.ruleList} onAdd={() => { setEditingRuleId(undefined); setCopyFromRuleId(latestAccrualRuleId); setRuleEditorOpen(true) }} onSelect={(ruleId) => { setEditingRuleId(ruleId); setCopyFromRuleId(undefined); setRuleEditorOpen(true) }} /> : null}
    ...
    {state === 'saved' ? <p className="text-sm text-success">{labels.saved}</p> : state === 'failed' ? <p className="text-sm text-destructive">{labels.failed}</p> : null}
     <ConfirmDialog cancelLabel={labels.archiveCancel} confirmLabel={labels.archiveConfirm} destructive description={labels.archiveDescription} onConfirm={() => save(true).then(() => undefined)} onOpenChange={setArchiveOpen} open={archiveOpen} pending={state === 'saving'} title={labels.archiveTitle} />
  </div>
}
