  ...
  return <>
    ...
    {open ? <div aria-modal="true" className="fixed inset-0 z-40 grid place-items-center bg-foreground/30 p-4" role="dialog"><section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl border bg-surface p-6 shadow-xl"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">{labels.title}</p><h2 className="mt-1 text-xl font-semibold">{labels.add}</h2></div><button aria-label={labels.cancel} className="button-secondary px-3" onClick={reset} type="button"><X size={17} /></button></div>
      <div className="mt-5 grid gap-4 md:grid-cols-2"><fieldset className="grid gap-2 text-sm font-medium md:col-span-2"><legend>{labels.selectionMode}</legend><div className="flex flex-wrap gap-2"><button className={`rounded-lg border px-3 py-2 ${selectionMode === 'one' ? 'border-primary bg-primary/10 text-primary' : ''}`} onClick={() => { setSelectionMode('one'); setSelectedEmploymentIds(selectedEmploymentIds.slice(0, 1)) }} type="button">{labels.onePerson}</button><button className={`rounded-lg border px-3 py-2 ${selectionMode === 'multiple' ? 'border-primary bg-primary/10 text-primary' : ''}`} onClick={() => setSelectionMode('multiple')} type="button">{labels.multiplePeople}</button></div></fieldset>
        ...
        <label className="inline-flex items-center gap-3 text-sm font-medium md:col-span-2"><input checked={noAccrual} className="size-4 accent-primary" onChange={(event) => { const checked = event.target.checked; setNoAccrual(checked); if (checked) setExpiryMonths(''); else if (!expiryMonths) setExpiryMonths('6') }} type="checkbox" />{labels.noAccrual}</label>
        ...
        {!noAccrual ? <label className="grid gap-1.5 text-sm font-medium"><span>{labels.expiry}</span><input className="form-field" min="0" max="120" onChange={(event) => setExpiryMonths(event.target.value)} type="number" value={expiryMonths} /></label> : null}<label className="grid gap-1.5 text-sm font-medium md:col-span-2"><span>{labels.reason}</span><textarea className="form-field min-h-20" maxLength={500} onChange={(event) => setReason(event.target.value)} value={reason} /></label>
      </div><div className="mt-5 rounded-xl bg-muted/50 p-4 text-sm"><p className="font-semibold">{labels.summary}</p><p className="mt-1 text-muted-foreground">{selectedEmploymentIds.length} {labels.summaryPeople}. {labels.summaryStart} {validFrom}. {noAccrual ? labels.noAccrual : `${labels.summaryAmount} ${toHours(amountHours, amountMinutes)} ${labels.hoursUnit}. ${labels.summaryExpiry} ${expiryMonths} ${labels.months}.`} {reason.trim() ? `${labels.summaryReason} ${reason.trim()}.` : ''}</p></div><div className="mt-6 flex justify-end gap-2"><button className="button-secondary" onClick={reset} type="button">{labels.cancel}</button><button className="button-primary" disabled={status === 'saving'} onClick={() => void save()} type="button">{status === 'saving' ? labels.saving : labels.save}</button></div>{status === 'saved' ? <p className="mt-3 text-sm text-success">{labels.saved}</p> : status === 'failed' ? <p className="mt-3 text-sm text-destructive">{labels.failed}</p> : null}</section></div> : null}
  </>
}
