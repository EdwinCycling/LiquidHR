...
export function LeaveAccrualExceptionsPanel({ catalog, leaveTypeId, labels }: { catalog: LeaveCatalog; leaveTypeId: string; labels: Labels }) {
  ...
  const visibleEmployees = useMemo(() => catalog.leaveExceptionEmployees.filter((employee) => employee.employee_name.toLowerCase().includes(search.toLowerCase()) || employee.employment_number.toLowerCase().includes(search.toLowerCase())), [catalog.leaveExceptionEmployees, search])
  ...
  const save = async () => {
    if (selectedEmployeeIds.length === 0 || !reason.trim() || (!noAccrual && !amountHours)) { setStatus('failed'); return }
    setStatus('saving')
    try {
      const response = await fetch('/api/leave/catalog', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'ACCRUAL_EXCEPTION', employeeIds: selectedEmployeeIds, leaveTypeId, validFrom, validUntil: validUntil || null, noAccrual, accrualAmount: noAccrual ? null : toHours(amountHours, amountMinutes), expirationMonths: expiryMonths ? Number(expiryMonths) : null, reason: reason.trim() }) })
  ...
  return <>
    ...
    {open ? <div aria-modal="true" className="fixed inset-0 z-40 grid place-items-center bg-foreground/30 p-4" role="dialog"><section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl border bg-surface p-6 shadow-xl"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">{labels.title}</p><h2 className="mt-1 text-xl font-semibold">{labels.add}</h2></div><button aria-label={labels.cancel} className="button-secondary px-3" onClick={reset} type="button"><X size={17} /></button></div>
      <div className="mt-5 grid gap-4 md:grid-cols-2"><fieldset className="grid gap-2 text-sm font-medium md:col-span-2"><legend>{labels.selectionMode}</legend><div className="flex flex-wrap gap-2"><button className={`rounded-lg border px-3 py-2 ${selectionMode === 'one' ? 'border-primary bg-primary/10 text-primary' : ''}`} onClick={() => { setSelectionMode('one'); setSelectedEmployeeIds(selectedEmployeeIds.slice(0, 1)) }} type="button">{labels.onePerson}</button><button className={`rounded-lg border px-3 py-2 ${selectionMode === 'multiple' ? 'border-primary bg-primary/10 text-primary' : ''}`} onClick={() => setSelectionMode('multiple')} type="button">{labels.multiplePeople}</button></div></fieldset>
        {selectionMode === 'one' ? <label className="grid gap-1.5 text-sm font-medium md:col-span-2">{labels.selectPerson}<select className="form-field" onChange={(event) => setSelectedEmployeeIds(event.target.value ? [event.target.value] : [])} value={selectedEmployeeIds[0] ?? ''}><option value="" />{catalog.leaveExceptionEmployees.map((employee) => <option key={employee.employee_id} value={employee.employee_id}>{employee.employee_name} · {employee.employment_number}</option>)}</select></label> : <div className="rounded-xl border p-4 md:col-span-2"><div className="flex flex-wrap items-center justify-between gap-3"><label className="relative min-w-48 flex-1"><Search className="absolute left-3 top-3 size-4 text-muted-foreground" /><input className="form-field pl-9" onChange={(event) => setSearch(event.target.value)} placeholder={labels.search} value={search} /></label><button className="button-secondary" onClick={() => setSelectedEmployeeIds(visibleEmployees.map((employee) => employee.employee_id))} type="button">{labels.selectVisible}</button></div><div className="mt-3 max-h-56 divide-y overflow-y-auto">{visibleEmployees.map((employee) => <label className="flex items-center gap-3 px-2 py-2 text-sm" key={employee.employee_id}><input checked={selectedEmployeeIds.includes(employee.employee_id)} className="size-4 accent-primary" onChange={() => toggleEmployee(employee.employee_id)} type="checkbox" />{employee.employee_name} · {employee.employment_number}</label>)}</div><p className="mt-2 text-xs text-muted-foreground">{labels.selected.replace('{count}', String(selectedEmployeeIds.length))}</p></div>}
