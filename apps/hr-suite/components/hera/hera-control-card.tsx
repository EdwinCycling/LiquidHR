...
export function HeRaControlCard({
  ...
  const values = draft
    ? actionValues(draft.controlPayload)
    : memoryProposal?.operation === 'UPDATE'
      ? { oldValue: memoryProposal.currentContent, newValue: memoryProposal.content }
      : null
  const memoryTitle = memoryProposal?.operation === 'UPDATE'
    ? labels.updateMemoryProposal
    : memoryProposal?.operation === 'DELETE'
      ? labels.deleteMemoryProposal
      : labels.rememberProposal
  const memoryConfirm = memoryProposal?.operation === 'UPDATE'
    ? labels.updateMemory
    : memoryProposal?.operation === 'DELETE'
      ? labels.deleteMemory
      : labels.remember
    ...
    <section className="mx-auto mt-5 max-w-3xl rounded-2xl border border-warning/30 bg-warning-surface/70 p-4 shadow-sm" aria-live="polite">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-foreground">{isMemory ? memoryTitle : labels.confirmAction}</p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button className="button-primary gap-2" disabled={busy} onClick={onConfirm} type="button"><Check aria-hidden="true" size={15} />{isMemory ? memoryConfirm : labels.confirmAction}</button>
