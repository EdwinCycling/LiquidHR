...
import { LeaveBalanceHistoryPanel, type LeaveBalanceHistoryPanelLabels } from '@/components/leave/leave-balance-history-panel'
...
export interface EmployeeDashboardLabels extends EmployeeDashboardSummaryLabels {
  ...
  leaveHistory?: LeaveBalanceHistoryPanelLabels
}
...
export function EmployeeDashboard({ detail, customFields, documents, reminders, activity, canWriteActivity, canUseEmployeeAi, canUseEmployeeVoice, employeeName, aiActionsLabels, liveVoiceLabels, initialLayout, compact, locale, dateFormat, timeFormat, labels, canManageEmployments, absence, absenceEmploymentOptions = [], selfReportAbsence = false, canReportAbsence = true, canRecoverAbsence = true, canChangeAbsenceCapacity = true, processWork, canReadProcesses, canStartProcess, journeys, journeyLabels, today }: EmployeeDashboardProps) {
  ...
  const leaveEmploymentOptions = detail.employments
    .filter((employment) => employment.record_status === 'CONFIRMED' && employment.starts_on <= today && (employment.ends_on === null || employment.ends_on >= today))
    .map((employment) => ({ id: employment.id, employmentNumber: employment.employment_number ?? labels.notRecorded }))
  const selectedLeaveEmploymentId = leaveEmploymentOptions.find((employment) => detail.employments.find((candidate) => candidate.id === employment.id)?.is_primary)?.id ?? leaveEmploymentOptions[0]?.id
  ...
  const narrow = [
    ...
    { id: 'reminders' as const, node: <DashboardCard icon={<CalendarDays className="h-4 w-4" />} title={labels.reminders} actionHref="?tab=reminders" actionLabel={labels.viewReminders} compact>{reminders.length ? <ul className="divide-y divide-border/70">{reminders.slice(0, 4).map((item) => <li className="py-3 first:pt-0 last:pb-0" key={item.recipientId}><p className="text-sm font-medium">{item.title}</p><time className="mt-1 block text-xs text-muted-foreground" dateTime={item.remindAt}>{formatDateTime(item.remindAt, { locale, dateFormat, timeFormat, timeZone: 'UTC' })}</time></li>)}</ul> : <EmptyInline>{labels.remindersEmpty}</EmptyInline>}</DashboardCard> },
    ...
    {employments.length === 0 ? <EmptyState className="items-start p-4 text-left" title={labels.employmentEmpty} actions={canManageEmployments ? <Link href={`/employees/${employeeId}/employments/new`} className={buttonClasses({ variant: 'primary', className: 'mt-4' })}><Plus aria-hidden="true" className="h-4 w-4" />{labels.employmentAdd}</Link> : undefined} /> : <div className="space-y-3">
      ...
      {employments.map((employment) => {
        ...
        return <Link key={employment.id} prefetch={false} href={`/employees/${employeeId}/employments/${employment.id}?fromTab=overview`} className="group block rounded-[var(--radius-surface)] border border-border/70 bg-background p-4 transition-colors hover:border-primary/45 hover:bg-accent/25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50">
          ...
          <span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-primary opacity-80 transition-opacity group-hover:opacity-100"><ArrowUpRight aria-hidden="true" className="h-3.5 w-3.5" />{labels.viewEmployment}</span>
        </Link>
      })}
    </div>}
  </div>
