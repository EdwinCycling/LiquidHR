...
function AbsenceStatusCard({ absence, labels }: { absence: AbsenceCaseSummary; labels: EmployeeDashboardLabels }) {
  const isCurrentlySick = absence.status === 'ACTIVE'
  const spell = absence.spells[0]
  return <div className={`rounded-xl border p-4 text-sm ${isCurrentlySick ? 'border-destructive/30 bg-destructive-surface/70 text-destructive' : 'border-success/25 bg-success-surface/60 text-success'}`}>
    <div className="flex items-center gap-2 font-semibold">{isCurrentlySick ? <HeartPulse aria-hidden="true" className="h-5 w-5" /> : <CheckCircle2 aria-hidden="true" className="h-5 w-5" />}{isCurrentlySick ? labels.absenceNowSick : labels.absenceNowNotSick}</div>
