...
export function EmployeeReminders({ employeeId, reminders, locale, dateFormat, timeFormat, labels }: { employeeId: string; reminders: ReminderItem[]; locale: string; dateFormat: DateFormat; timeFormat: TimeFormat; labels: Labels }) {
  ...
  function startAdd(): void { setEditing(null); setDateTime(initialDateTime()); setOpen(true); setFeedback(null) }
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const response = await fetch(editing ? `/api/reminders/${editing.reminderId}` : '/api/reminders', { method: editing ? 'PATCH' : 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(editing ? body : { type: 'HR', ...body, targetType: 'EMPLOYEES', targetIds: [employeeId] }) })
    if (!response.ok) { setFeedback(labels.failed); return }
    if (!editing) { const result = await response.json() as { data?: { id?: string } }; if (result.data?.id) await fetch(`/api/reminders/${result.data.id}/publish`, { method: 'POST' }) }
  ...
  return <section className="mt-8 space-y-5"><header className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow text-primary">{labels.title}</p><h2 className="mt-1 flex items-center gap-2 text-2xl font-semibold"><BellRing aria-hidden="true" size={21} />{labels.title}</h2></div></header>
    ...
    {reminders.length === 0 ? <p className="rounded-2xl border border-dashed bg-surface p-6 text-center text-sm text-muted-foreground">{labels.empty}</p> : <ol className="space-y-3">{reminders.map((item) => <li key={item.recipientId} className="rounded-2xl border bg-surface p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h3 className="font-semibold">{item.title}</h3>{item.description && <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{item.description}</p>}<time className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground" dateTime={item.remindAt}><CalendarDays aria-hidden="true" size={14} />{formatDateTime(item.remindAt, { locale, dateFormat, timeFormat })}</time></div><div className="flex gap-2"><button type="button" className="button-secondary inline-flex items-center gap-1.5" onClick={() => startEdit(item)}><Pencil aria-hidden="true" size={14} />{labels.edit}</button><button type="button" className="button-secondary inline-flex items-center gap-1.5 text-destructive" onClick={() => void remove(item)}><Trash2 aria-hidden="true" size={14} />{labels.remove}</button></div></div></li>)}</ol>}
    <div><button className="button-primary inline-flex items-center gap-2" onClick={() => open ? setOpen(false) : startAdd()} type="button"><Plus aria-hidden="true" size={16} />{labels.add}</button></div>
