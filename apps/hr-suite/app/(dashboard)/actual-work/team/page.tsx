...
export default async function ActualWorkTeamPage({ searchParams }: Props) {
  ...
  return <PageShell className="space-y-6 py-6 sm:py-8" width="wide">
    <PageHeader actions={<div className="flex flex-wrap gap-2"><Link className="inline-flex min-h-10 items-center rounded-[var(--radius-control)] border border-border bg-surface px-4 py-2 text-sm font-medium hover:bg-surface-raised" href={'/actual-work/bulk?month=' + month}>{t('actualWorkBulkTitle')}</Link><Link className="inline-flex min-h-10 items-center rounded-[var(--radius-control)] border border-border bg-surface px-4 py-2 text-sm font-medium hover:bg-surface-raised" href="/employees">{t('actualWorkBack')}</Link></div>} description={t('actualWorkDescription')} title={t('actualWorkTitle') + ' · ' + month} />
