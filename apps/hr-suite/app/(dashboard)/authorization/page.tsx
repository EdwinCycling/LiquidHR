...
export default async function AuthorizationPage() {
  ...
  const [matrix, t, settings, employeeResult, departmentResult] = await Promise.all([
    listAuthorizationMatrix(), getTranslator('organization'), getTranslator('settings'),
  ...
  return <section className="mx-auto w-full max-w-[96rem] px-5 py-8 sm:px-8 sm:py-10 lg:px-10"><AdminSettingsPageHeader backLabel={settings('admin.backToOverview')} eyebrow={t('eyebrow')} subtitle={t('subtitle')} title={t('title')} /><AuthorizationManager {...matrix} employees={employeeResult.data.map((employee) => ({ id: employee.id, name: `${employee.employee_number} · ${employee.first_name} ${employee.birth_name}` }))} departments={departmentResult.data.map((department) => ({ id: department.id, name: `${department.code} · ${department.name}` }))} labels={labels} /></section>
}
