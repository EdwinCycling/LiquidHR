...
export default async function ProductUpdatesSettingsPage() {
  let management: Awaited<ReturnType<typeof listManagedProductUpdates>>
  try {
    management = await listManagedProductUpdates()
  } catch (error) {
    if (error instanceof AuthorizationError) redirect('/geen-toegang')
    throw error
  }
  const t = await getTranslator('productUpdates')
