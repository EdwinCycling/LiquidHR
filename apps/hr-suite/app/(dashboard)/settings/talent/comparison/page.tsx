...
export default async function TalentComparisonSettingsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (loadFailed || !initial) return <section className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-8 lg:px-10"><AdminSettingsPageHeader backHref="/settings/talent" backLabel={t('backToTalent')} title={t('comparisonTitle')} subtitle={t('comparisonHrSubtitle')} /><EmptyState className="mt-6" title={t('comparisonLoadFailed')} description={t('comparisonLoadFailedDescription')} /></section>
  return <section className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-8 lg:px-10"><AdminSettingsPageHeader backHref="/settings/talent" backLabel={t('backToTalent')} title={t('comparisonTitle')} subtitle={t('comparisonHrSubtitle')} /><TalentComparisonWorkspace key={`${initial.selectedEmployeeId ?? ''}:${initial.selectedProfileVersionId ?? ''}`} action="/settings/talent/comparison" initial={initial} labels={labels} profileHref="/settings/talent" /></section>
}
