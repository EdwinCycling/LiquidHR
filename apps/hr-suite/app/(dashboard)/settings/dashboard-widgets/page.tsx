...
export default async function DashboardWidgetSettingsPage() {
  ...
  return <PageShell className="py-8 lg:py-10"><AdminSettingsPageHeader backLabel={messages('admin.backToOverview')} eyebrow={messages('admin.title')} subtitle={messages('dashboardWidgets.subtitle')} title={messages('dashboardWidgets.title')} /><DashboardWidgetSettingsForm labels={{ saving: messages('saving'), saved: messages('dashboardWidgets.saved'), failed: messages('dashboardWidgets.failed'), enabled: messages('dashboardWidgets.enabled'), active: messages('dashboardWidgets.active'), inactive: messages('dashboardWidgets.inactive'), roles: messages('dashboardWidgets.roles'), allRoles: messages('dashboardWidgets.allRoles'), noRoles: messages('dashboardWidgets.noRoles'), categories: Object.fromEntries(categoryKeys.map((key) => [key, messages(`dashboardWidgets.categories.${key}`)])) }} roles={roles} widgets={items} /></PageShell>
}
