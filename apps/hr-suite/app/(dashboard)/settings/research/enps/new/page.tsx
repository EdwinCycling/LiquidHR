...
import { getEnpsDraft, listResearchSettingsData } from '@/lib/research/admin-service'
import { ResearchError } from '@/lib/research/errors'

export default async function NewEnpsPage({ searchParams }: { searchParams: Promise<{ campaignId?: string }> }) {
  let data
  ...
  let draft
  if (campaignId) {
    try { draft = await getEnpsDraft(campaignId) } catch (error) {
