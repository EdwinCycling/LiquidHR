...
export default async function PersonalSettingsPage() {
  ...
  const labels: SettingsModalLabels = {
    open: settings('open'), close: settings('close'), title: settings('title'), subtitle: settings('subtitle'),
  ...
  return <div className="mx-auto w-full max-w-6xl px-5 py-8 lg:px-10"><header className="mb-7"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">{settings('personalEyebrow')}</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">{labels.title}</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">{labels.subtitle}</p></header><PersonalSettingsForm labels={labels} preferences={preferences} /></div>
}
