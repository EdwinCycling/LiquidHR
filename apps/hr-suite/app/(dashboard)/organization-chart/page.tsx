...
export default async function OrganizationChartPage({ searchParams }: OrganizationChartPageProps) {
  ...
  const labels: OrganizationChartExplorerLabels = {
    ...
    employees: translate('employees'), manager: translate('manager'), managerInherited: translate('managerInherited'), managerNone: translate('managerNone'), managerAmbiguous: translate('managerAmbiguous'),
    ...
    <section className="mx-auto w-full max-w-[96rem] px-4 py-7 sm:px-8 sm:py-9 lg:px-10">
      <header className="mb-6 flex flex-col gap-5 border-b pb-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="grid gap-2 text-xs sm:grid-cols-3">
          ...
          <div className="flex items-center gap-2 rounded-xl border bg-surface px-3 py-2.5 text-muted-foreground"><UsersRound aria-hidden="true" size={16} /><span>{translate('departmentCount', { count: graph.metadata.visibleDepartmentCount })} · {translate('employeeCount', { count: graph.metadata.visibleEmployeeCount })}</span></div>
        </div>
      </header>
