...
export default async function RemindersPage() {
  ...
  const labels: ReminderCenterLabels = {
    ...
    openManagement: t('openManagement'), openAction: t('openAction'),
    ...
    sortSoonest: t('sortSoonest'), sortLatest: t('sortLatest'), sortTitle: t('sortTitle'), search: t('search'), selectAll: t('selectAll'), clearSelection: t('clearSelection'), bulkComplete: t('bulkComplete'), moreInfo: t('moreInfo'), employee: t('employee'), noResults: t('noResults'), selectedCount: t('selectedCount'), visibleCount: t('visibleCount'), close: t('close'), cancel: t('cancel'), discardTitle: t('discardTitle'), discardDescription: t('discardDescription'), discardConfirm: t('discardConfirm'), keepEditing: t('keepEditing'),
  }
  return <main className="mx-auto w-full max-w-7xl px-4 py-7 sm:px-6 lg:px-8 lg:py-10"><h1 className="text-3xl font-semibold tracking-tight">{t('title')}</h1><ReminderCenter canManageHr={canManageHr} initialManaged={managed} initialReminders={reminders} initialNowIso={new Date().toISOString()} labels={labels} locale={locale} dateFormat={preferences.dateFormat} timeFormat={preferences.timeFormat} targetOptions={targetOptions} /></main>
}
