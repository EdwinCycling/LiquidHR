...
export default async function MyTalentPage() {
  ...
  return <section className="mx-auto w-full max-w-4xl px-5 py-8 sm:px-8 lg:px-10"><AdminSettingsPageHeader backHref="/dashboard/start" backLabel={t('backToDashboard')} eyebrow={t('myTitle')} subtitle={t('mySubtitle')} title={t('myTitle')} /><nav aria-label={t('assessmentTitle')} className="mt-5 flex flex-wrap gap-2"><Link className="button-secondary" href="/my-talent/assessments">{t('assessmentTitle')}</Link><Link className="button-secondary" href="/my-talent/role-explorer">{t('roleExplorerTitle')}</Link><Link className="button-secondary" href="/my-talent/goals">{t('goalTitle')}</Link><Link className="button-secondary" href="/my-talent/reports">{t('reportTitle')}</Link></nav><TalentNotificationPanel labels={{ title: t('notificationTitle'), intro: t('notificationIntro'), empty: t('notificationEmpty'), markRead: t('notificationMarkRead'), complete: t('notificationComplete'), dismiss: t('notificationDismiss'), saved: t('notificationSaved'), failed: t('notificationFailed') }} />{readOnlyProfile ? <div className="mt-6"><TalentReadOnlyProfile labels={labels} profile={readOnlyProfile} /></div> : <p className="mt-6 rounded-2xl border border-dashed p-6 text-sm text-muted-foreground">{t('myProfileEmpty')}</p>}<TalentEmployeeCapabilityRecords mode="self" initial={records} options={recordOptions} labels={recordLabels} /></section>
}
