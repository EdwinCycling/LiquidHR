...
import { evaluateAiSettingsAccess, getAiGroupSettingsForContext } from '@/lib/ai/settings-service'
  ...
  try {
    ...
    const [canReadDocuments, canWriteDocuments, canDeleteDocuments] = tab === 'documents'
      ? await performance.measure('documents.permissions', () => Promise.all([
        permissionAllowed('document:read', employeeId, dependencies.supabase), permissionAllowed('document:write', employeeId, dependencies.supabase), permissionAllowed('document:delete', employeeId, dependencies.supabase),
      ]))
      : [false, false, false]
    ...
    const [canWriteNotes, canDeleteNotes, canUseAi, notes] = canReadNotes
      ...
      : tab === 'overview'
        ? [false, false, await permissionAllowed('ai:use', employeeId), []]
      : [false, false, false, []]
...
async function permissionAllowed(permissionCode: string, employeeId: string, supabase?: SupabaseServerClient): Promise<boolean> {
  try {
    await requirePermission(permissionCode, employeeId)
    if (permissionCode === 'document:read' || permissionCode === 'document:write' || permissionCode === 'document:delete') {
      const client = supabase ?? await createClient()
      const { data, error } = await client.rpc('can_access_employee_dossier', { requested_employee_id: employeeId, requested_permission: permissionCode })
...
export default async function EmployeeDetailPage({ params, searchParams }: EmployeeDetailPageProps) {
  ...
  const canConfirmAbsence = authContext.employeeId !== employeeId && authContext.permissions.includes('absence:write')
  const canChangeAbsenceCapacity = authContext.permissions.includes('absence:write')
  const compact = view === 'compact'
  const today = new Date().toISOString().slice(0, 10)
  const aiSettings = (tab === 'overview' || tab === 'notes') && canUseAi
    ? await getAiGroupSettingsForContext(authContext).catch(() => null)
    : null
  const settingsAllow = (featureCode: string, origin: 'UI' | 'VOICE' = 'UI'): boolean => aiSettings !== null && evaluateAiSettingsAccess({
    settings: aiSettings,
  ...
  const canUseEmployeeAi = canUseAi && isAiImproveAvailable() && settingsAllow('EMPLOYEE_SUMMARY') && settingsAllow('CONVERSATION_PREPARATION')
  const canUseEmployeeNoteAi = canUseAi && isAiImproveAvailable() && settingsAllow('improve-existing-hr-text')
  const canUseEmployeeVoice = canUseAi && isAiImproveAvailable() && isRealtimeVoiceEnabled() && settingsAllow('VOICE', 'VOICE')
  ...
  return (
      <PageShell width="standard" className="py-7 lg:py-10">
        ...
        {tab === 'overview' && <EmployeeDashboard journeys={journeys} journeyLabels={{ journeys: tEmployees('journeys'), journeysDescription: tEmployees('journeysDescription'), journeysEmpty: tEmployees('journeysEmpty'), journeysOpen: tEmployees('journeysOpen'), journeyProgress: tEmployees('journeyProgress') }} selfReportAbsence={selfReport} canReportAbsence={canReportAbsence} canRecoverAbsence={canRecoverAbsence} canChangeAbsenceCapacity={canChangeAbsenceCapacity} canManageEmployments={canManageEmployments} absence={absenceOverview} absenceEmploymentOptions={reportableAbsenceEmploymentOptions} detail={detail} customFields={customFields} documents={dashboardDocuments} reminders={reminders} activity={dashboardActivity} canWriteActivity={canWriteActivity} canUseEmployeeAi={canUseEmployeeAi} canUseEmployeeVoice={canUseEmployeeVoice} employeeName={`${detail.employee.firstName} ${detail.employee.birthName}`} aiActionsLabels={{
          ...
          working: tEmployees('aiWorking'), reviewTitle: tEmployees('aiReviewTitle'), cancel: tEmployees('aiCancel'), copy: tEmployees('aiCopy'), copied: tEmployees('aiCopied'), retry: tEmployees('aiRetry'), failed: tEmployees('aiFailed'), saveToLogbook: tEmployees('aiSaveToLogbook'), savingToLogbook: tEmployees('aiSavingToLogbook'), savedToLogbook: tEmployees('aiSavedToLogbook'), saveToLogbookFailed: tEmployees('aiSaveToLogbookFailed'),
        }} liveVoiceLabels={{
          ...
          title: tEmployees('aiVoiceTitle'), description: tEmployees('aiVoiceDescription'), start: tEmployees('aiVoiceStart'), stop: tEmployees('aiVoiceStop'), connecting: tEmployees('aiVoiceConnecting'), listening: tEmployees('aiVoiceListening'), speaking: tEmployees('aiVoiceSpeaking'), processing: tEmployees('aiVoiceProcessing'), microphoneDenied: tEmployees('aiVoiceMicrophoneDenied'), connectionFailed: tEmployees('aiVoiceConnectionFailed'), disabled: tEmployees('aiVoiceDisabled'), toolFailed: tEmployees('aiVoiceToolFailed'), elapsed: tEmployees('aiVoiceElapsed'), paused: tEmployees('aiVoicePaused'), inputLevel: tEmployees('aiVoiceInputLevel'), inputMuted: tEmployees('aiVoiceInputMuted'),
        }} initialLayout={dashboardLayout} compact={compact} locale={locale} dateFormat={preferences.dateFormat} timeFormat={preferences.timeFormat} processWork={processWork} canReadProcesses={canReadProcesses} canStartProcess={canStartProcess} today={today} labels={{
          ...
          salaryHidden: tEmployees('salaryRevealHelp'), salaryNotAvailable: tEmployees('dashboardSalaryNotAvailable'), salaryMonthly: tEmployees('salaryMonthlySuffix'), salaryHourly: tEmployees('salaryHourlySuffix'), salaryLoading: tEmployees('dashboardSalaryLoading'), salaryFailed: tEmployees('dashboardSalaryFailed'), leave: tEmployees('dashboardLeave'), leaveDescription: tEmployees('dashboardLeaveDescription'), leaveHistory: { employment: tEmployees('dashboardLeaveHistoryEmployment'), employmentPlaceholder: tEmployees('dashboardLeaveHistoryEmploymentPlaceholder'), employmentSearch: tEmployees('dashboardLeaveHistoryEmploymentSearch'), currentBalance: tEmployees('dashboardLeaveHistoryCurrentBalance'), beginningBalance: tEmployees('dashboardLeaveHistoryBeginningBalance'), taken: tEmployees('dashboardLeaveHistoryTaken'), planned: tEmployees('dashboardLeaveHistoryPlanned'), projectedEnd: tEmployees('dashboardLeaveHistoryProjectedEnd'), projectedContractEnd: tEmployees('dashboardLeaveHistoryProjectedContractEnd'), openingBalance: tEmployees('dashboardLeaveHistoryOpeningBalance'), accrual: tEmployees('dashboardLeaveHistoryAccrual'), year: tEmployees('dashboardLeaveHistoryYear'), yearSelect: tEmployees('dashboardLeaveHistoryYearSelect'), viewOverview: tEmployees('dashboardLeaveHistoryViewOverview'), unlimited: tEmployees('dashboardLeaveHistoryUnlimited'), history: tEmployees('dashboardLeaveHistoryTitle'), leaveType: tEmployees('dashboardLeaveHistoryLeaveType'), amount: tEmployees('dashboardLeaveHistoryAmount'), effectiveDate: tEmployees('dashboardLeaveHistoryEffectiveDate'), reason: tEmployees('dashboardLeaveHistoryReason'), actor: tEmployees('dashboardLeaveHistoryActor'), createdAt: tEmployees('dashboardLeaveHistoryCreatedAt'), empty: tEmployees('dashboardLeaveHistoryEmpty'), loading: tEmployees('dashboardLeaveHistoryLoading'), failed: tEmployees('dashboardLeaveHistoryFailed'), notRecorded: tEmployees('dashboardLeaveHistoryNotRecorded') },
