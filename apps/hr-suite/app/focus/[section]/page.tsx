...
import { getFocusDocuments, getFocusDirectory, getFocusHoursOverview, getFocusLeaveOverview, getFocusProfileProjection, loadFocusSectionContext } from '@/lib/focus/section-service'
...
export default async function FocusSectionPage({ params, searchParams = Promise.resolve({}) }: { params: Promise<{ section: string }>; searchParams?: Promise<{ actAs?: string; month?: string; day?: string; request?: string; edit?: string; reportEmployee?: string; q?: string }> }) {
  ...
  if (kind === 'journey') {
    ...
    content = <>{reportMember && calendar.canReportAbsence ? <FocusAbsenceForm employeeId={reportMember.employeeId} employmentId={reportMember.employmentId} endpoint="/api/absence/report" labels={{ title: t('absence.managerTitle'), description: t('absence.managerDescription'), startDate: t('absence.startDate'), submit: t('absence.submit'), submitting: t('absence.submitting'), success: t('absence.success'), failed: t('absence.failed') }} today={props.today} /> : null}<FocusTeamCalendarView actAsToken={data.actAs?.token ?? query.actAs} calendar={calendar} labels={{ month: t('team.month'), previous: t('team.previous'), next: t('team.next'), today: t('team.today'), employee: t('team.employee'), status: t('team.status'), present: t('team.present'), absent: t('team.absent'), available: t('team.available'), off: t('team.off'), leave: t('team.leave'), hours: t('team.hours'), selectedDay: t('team.selectedDay'), reportAbsence: t('team.reportAbsence'), actAs: t('team.actAs'), acting: t('team.acting') }} locale={locale} /></>
  } else if (kind === 'absence') {
    content = <FocusAbsenceForm employeeId={employeeId} labels={{ title: t('absence.title'), description: t('absence.description'), startDate: t('absence.startDate'), submit: t('absence.submit'), submitting: t('absence.submitting'), success: t('absence.success'), failed: t('absence.failed') }} today={props.today} token={data.actAs?.token ?? query.actAs} />
