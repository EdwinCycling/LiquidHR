...
export async function POST(request: Request) {
  try {
    ...
    const selection = parseHrGroupSelection(body)
    const context = await loadActiveContext()
    const hrGroup = validateHrGroupSelection(context, selection.hrGroupId)
    ...
    cookieStore.set(ACTIVE_HR_GROUP_COOKIE, hrGroup.id, cookieOptions)
    cookieStore.delete(ACTIVE_ADMINISTRATION_COOKIE)

    return NextResponse.json({ data: { hrGroup } })
  } catch (error) {
