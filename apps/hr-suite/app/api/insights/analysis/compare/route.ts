...
import { executeAnalysisComparison } from '@/lib/insights/analysis-comparison'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'ANALYSIS_COMPARISON_INVALID_REQUEST' }, { headers: noStoreHeaders, status: 400 })
  }

  try {
    const data = await executeAnalysisComparison(body)
