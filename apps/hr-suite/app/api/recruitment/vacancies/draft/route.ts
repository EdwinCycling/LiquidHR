...
import { runVacancyDraft, vacancyDraftRequestSchema } from '@/lib/recruitment/vacancy-ai'
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const parsed = vacancyDraftRequestSchema.safeParse(await request.json().catch(() => null) as unknown)
  if (!parsed.success) return NextResponse.json({ error: 'AI_EVERYWHERE_INPUT_INVALID' }, { status: 400 })
  try {
    const proposal = await runVacancyDraft({ request: parsed.data, idempotencyKey })
