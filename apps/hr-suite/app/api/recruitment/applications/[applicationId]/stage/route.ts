...
import { transitionRecruitmentApplication } from '@/lib/recruitment/application-service'
...
export async function POST(request: Request, { params }: { params: Promise<{ applicationId: string }> }): Promise<NextResponse> {
  try {
    ...
    if (!stageId.success || expectedVersion === null) return NextResponse.json({ code: 'RECRUITMENT_STAGE_INPUT_INVALID' }, { status: 400 })
    const data = await transitionRecruitmentApplication(applicationId, stageId.data, expectedVersion, idempotencyKey, await createClient())
