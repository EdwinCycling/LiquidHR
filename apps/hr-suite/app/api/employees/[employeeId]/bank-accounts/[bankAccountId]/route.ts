...
import { bankAccountUpdateSchema } from '@/lib/employees/schemas'
...
export async function PATCH(request: Request, context: Context): Promise<NextResponse> { try { const input = bankAccountUpdateSchema.safeParse(await request.json()); if (!input.success) return NextResponse.json({ error: 'BANK_ACCOUNT_INPUT_INVALID' }, { status: 400 }); const ids = await context.params; await updateEmployeeBankAccount(ids.employeeId, ids.bankAccountId, input.data); return NextResponse.json({ data: { updated: true } }) } catch (error) { return fail(error) } }
