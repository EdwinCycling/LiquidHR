...
import { CONVERSATION_PREPARATION_FEATURE } from '@/lib/ai/feature-registry'
...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  try {
    const proposal = await runEmployeeAi({ employeeId: (await context.params).employeeId, feature: CONVERSATION_PREPARATION_FEATURE, request: parsed.data, idempotencyKey })
