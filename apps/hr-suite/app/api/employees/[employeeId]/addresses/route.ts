...
import { createEmployeeAddress } from '@/lib/employees/employee-service'
...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: 'ADDRESS_INPUT_INVALID' }, { status: 400 })
    return NextResponse.json({ data: { id: await createEmployeeAddress(employeeId, parsed.data) } }, { status: 201 })
  } catch (error) {
