-- De transactionele publicatie-RPC en de salary-wrapper moeten een audit
-- change-set kunnen schrijven zonder de publieke change-set RLS-projectie te
-- verbreden. Deze helper blijft actor-, HR-groep-, administratie-, medewerker-,
-- dienstverband- en datumgebonden.
create or replace function internal_security.insert_employment_change_set(
...
begin
  if actor_user_id is null
     or requested_created_by_user_id is distinct from actor_user_id then
    raise exception 'EMPLOYMENT_CHANGE_SET_ACTOR_INVALID';
  end if;
  if requested_status <> 'APPLIED'
     or requested_reason not in ('EMPLOYMENT_CREATED', 'EMPLOYMENT_CREATED_SALARY') then
    raise exception 'EMPLOYMENT_CHANGE_SET_PAYLOAD_INVALID';
  end if;
  if requested_reason = 'EMPLOYMENT_CREATED'
     and (requested_domains is null or requested_domains[1] <> 'EMPLOYMENT') then
    raise exception 'EMPLOYMENT_CHANGE_SET_PAYLOAD_INVALID';
  end if;
  if requested_reason = 'EMPLOYMENT_CREATED_SALARY'
     and requested_domains is distinct from array['SALARY']::text[] then
    raise exception 'EMPLOYMENT_CHANGE_SET_PAYLOAD_INVALID';
  end if;
  if not exists (
    select 1
    ...
    raise exception 'EMPLOYMENT_CHANGE_SET_SCOPE_INVALID';
  end if;
  ...
  return created_change_set_id;
end;
$$;

revoke all on function internal_security.insert_employment_change_set(uuid, uuid, uuid, uuid, date, text, text[], text, uuid) from public, anon, authenticated;
grant execute on function internal_security.insert_employment_change_set(uuid, uuid, uuid, uuid, date, text, text[], text, uuid) to authenticated;

-- De tijdelijke policy/helper zijn niet meer nodig nadat beide publicatie-RPC's
-- via de gecontroleerde security-definer helper schrijven.
drop policy if exists employment_change_sets_insert_for_employment_creation on public.employment_change_sets;
drop function if exists internal_security.can_insert_employment_creation_change_set(uuid, uuid, uuid, uuid, date, text, text, uuid);
...
begin
  ...
  if not internal_security.employee_has_administration_assignment(
    requested_tenant_id,
  ...
  created_change_set_id := internal_security.insert_employment_change_set(
    requested_tenant_id,
    requested_administration_id,
    requested_employee_id,
    created_employment_id,
    requested_starts_on,
    'EMPLOYMENT_CREATED',
...
begin
  ...
  salary_change_set_id := internal_security.insert_employment_change_set(
    employee_row.tenant_id,
    requested_administration_id,
    requested_employee_id,
    created_employment_id,
    employment_row.starts_on,
    'EMPLOYMENT_CREATED_SALARY',
    array['SALARY'],
    'APPLIED',
    (select auth.uid())
  );
