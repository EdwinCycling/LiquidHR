-- DEV repair: the employment insert policy already authorizes the canonical
-- HR-group permission. Keep the contract exception on that same scope while
-- retaining exact tenant, group, administration, employee and parent checks.
...
for all
to authenticated
using ((select internal_security.has_hr_group_access(
  employment_contracts.tenant_id,
  employment_contracts.hr_group_id
)))
with check (
  (select internal_security.has_hr_group_access(
    employment_contracts.tenant_id,
    employment_contracts.hr_group_id
  ))
