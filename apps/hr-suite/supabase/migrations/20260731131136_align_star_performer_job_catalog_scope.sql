create or replace function public.upsert_star_performer_assessment(
...
begin
  ...
  if subject_job_id is not null and not exists (
    ...
    where job.id = subject_job_id and job.tenant_id = context_tenant_id
  ) then
  ...
  if subject_job_group_id is not null and not exists (
    ...
    where job_group.id = subject_job_group_id and job_group.tenant_id = context_tenant_id
  ) then
    raise exception 'STAR_PERFORMER_JOB_GROUP_NOT_FOUND' using errcode = 'P0001';
  end if;

  select coalesce(array_agg(distinct tag_id), '{}') into requested_tag_ids
  from (
  ...
  if coalesce(array_length(requested_tag_ids, 1), 0) > 0 then
    select count(*) into valid_tag_count
  ...
  if subject_job_id is not null then
    select assessment.id into assessment_record_id
  ...
  if assessment_record_id is null then
    insert into public.star_performer_assessments (
      tenant_id, administration_id, employee_id, job_id, job_group_id, criticality_level
    ) values (
      context_tenant_id, requested_administration_id, target_employee_id,
      subject_job_id, subject_job_group_id, target_criticality_level
  ...
  delete from public.star_performer_assessment_tags where assessment_id = assessment_record_id;
  if coalesce(array_length(requested_tag_ids, 1), 0) > 0 then
