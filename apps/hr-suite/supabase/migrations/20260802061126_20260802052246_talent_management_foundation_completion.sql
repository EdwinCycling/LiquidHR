...
as $$
begin
  new.normalized_name := internal_security.normalize_catalog_name(new.name);
  return new;
end;
$$;
...
for each row execute function internal_security.audit_hr_change('talent_capability_tags');
