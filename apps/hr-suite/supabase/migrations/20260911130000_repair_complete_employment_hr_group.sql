-- DEV repair: refresh the complete-employment RPC after the observed employment_contracts RLS failure.
-- The function keeps SECURITY INVOKER and propagates hr_group_id to every grouped row.

-- De publicatie-RPC draait als security invoker en moet deze ene
-- autorisatiecontrole buiten de RLS-projectie kunnen uitvoeren. De helper
-- retourneert alleen een boolean en controleert de volledige tenant-, HR-groep-,
-- administratie-, medewerker- en datumscope.
create or replace function internal_security.employee_has_administration_assignment(
...
revoke all on function internal_security.employee_has_administration_assignment(uuid, uuid, uuid, uuid, date) from public, anon, authenticated;
grant execute on function internal_security.employee_has_administration_assignment(uuid, uuid, uuid, uuid, date) to authenticated;
