...
create or replace function public.create_hr_reminder(
  requested_tenant_id uuid, requested_administration_id uuid, requested_title text,
  requested_description text, requested_remind_at timestamptz,
...
begin
  ...
  ) then raise exception 'REMINDER_FORBIDDEN' using errcode = '42501'; end if;
  ...
  if requested_target_type = 'DEPARTMENTS' then
    insert into public.reminder_targets (tenant_id, administration_id, reminder_id, department_id)
...
create or replace function public.manage_employment_organization_timeline(
  requested_employment_id uuid, requested_placement_id uuid, requested_effective_on date,
  requested_department_id uuid, requested_job_id uuid
)
returns uuid language plpgsql security invoker
...
begin
  ...
  where employment.id = requested_employment_id and employment.deleted_at is null
  for update;
  if employment_row.id is null then raise exception 'EMPLOYMENT_NOT_FOUND'; end if;
  if not internal_security.current_user_has_permission(
    employment_row.tenant_id, employment_row.administration_id, 'organization-placement:write'
  ...
  join public.job_revisions revision
    on revision.job_id = job.id and revision.tenant_id = job.tenant_id
  ...
  if requested_placement_id is not null then
    ...
    set department_id = requested_department_id, job_id = requested_job_id, job_title = job_name
    where id = requested_placement_id and employment_id = requested_employment_id
  ...
  update public.employee_organizations set effective_to = requested_effective_on - 1 where id = placement_row.id;
  insert into public.employee_organizations (
    tenant_id, administration_id, employee_id, employment_id, department_id, job_id, job_title,
    direct_manager_id, direct_manager_deputy_id, effective_from, effective_to
  ) values (
    employment_row.tenant_id, employment_row.administration_id, employment_row.employee_id, employment_row.id,
...
create or replace function internal_security.can_manage_employee(
  target_employee_id uuid, requested_permission_code text
)
returns boolean language sql stable security definer set search_path = ''
as $$
  with recursive target_placements as (
    select organization.tenant_id, organization.administration_id, organization.department_id, organization.direct_manager_id
  ...
  target_department_tree as (
    select department.id as department_id, department.parent_id,
           placement.tenant_id, placement.administration_id
    from target_placements placement
    join public.departments department
      on department.id = placement.department_id and department.tenant_id = placement.tenant_id
    union
    select parent.id, parent.parent_id, tree.tenant_id, tree.administration_id
  ...
  actors as (
    ...
    from public.employees employee
    where employee.auth_user_id = (select auth.uid()) and employee.deleted_at is null
  )
  select exists (
    select 1 from target_placements placement
    where internal_security.current_user_has_permission(
      placement.tenant_id, placement.administration_id, requested_permission_code
    )
  )
  or exists (
    select 1
    from target_placements placement
    join actors actor on actor.id = placement.direct_manager_id and actor.tenant_id = placement.tenant_id
    join public.management_roles role on role.code = 'DIRECT_MANAGER' and role.tenant_id is null
    join public.role_permissions role_permission on role_permission.management_role_id = role.id
    join public.permissions permission on permission.id = role_permission.permission_id and permission.code = requested_permission_code
    where internal_security.has_administration_access(placement.tenant_id, placement.administration_id)
  )
  or exists (
    select 1
    from public.department_management assignment
    join actors actor on actor.id = assignment.employee_id and actor.tenant_id = assignment.tenant_id
    join public.role_permissions role_permission on role_permission.management_role_id = assignment.management_role_id
    join public.permissions permission on permission.id = role_permission.permission_id and permission.code = requested_permission_code
    ...
    where assignment.effective_from <= current_date
      ...
      and internal_security.has_administration_access(tree.tenant_id, tree.administration_id)
  );
$$;
...
create or replace function public.list_employee_overviews(
  requested_tenant_id uuid, requested_administration_id uuid,
  requested_as_of date default current_date, requested_archive_filter text default 'active'
)
returns table (
  id uuid, employee_number text, first_name text, birth_name_prefix text, birth_name text,
  work_email text, avatar_url text, is_archived boolean, employment_history jsonb,
  department_name text, job_title text
)
language sql stable security invoker set search_path = public
as $$
  with scoped_employees as (
    select distinct on (assignment.employee_id) assignment.employee_id
  ...
  select employee.id, employee.employee_number, employee.first_name, employee.birth_name_prefix,
    employee.birth_name, employee.work_email, employee.avatar_url, employee.is_archived,
    coalesce(employment_history.periods, '[]'::jsonb) as employment_history,
    placement.department_name, placement.job_title
  from scoped_employees scoped
  join public.employees employee
    on employee.tenant_id = requested_tenant_id and employee.id = scoped.employee_id and employee.deleted_at is null
  left join lateral (
    select jsonb_agg(
      jsonb_build_object('starts_on', employment.starts_on, 'ends_on', employment.ends_on, 'record_status', employment.record_status)
      order by employment.starts_on
    ) as periods
    from public.employments employment
    where employment.tenant_id = requested_tenant_id and employment.employee_id = employee.id and employment.deleted_at is null
  ...
  left join lateral (
    ...
    left join public.departments department
      on department.tenant_id = organization.tenant_id and department.id = organization.department_id
