-- Reminderdoelen mogen volgens het bestaande contract alleen aan een draft
-- worden toegevoegd; publiceer daarom pas nadat targets en recipients bestaan.
create or replace function internal_security.create_journey_reminders_internal(requested_journey_id uuid)
returns void language plpgsql security definer set search_path = pg_catalog
as $$
declare
  journey_row public.journeys%rowtype; item record; reminder_id uuid;
  recipient_user_id uuid; remind_at_value timestamptz; administration_id_value uuid;
begin
  ...
  if auth.uid() is null or not internal_security.current_user_has_hr_group_permission(journey_row.tenant_id, journey_row.hr_group_id, 'journey:write') then raise exception 'JOURNEY_FORBIDDEN' using errcode = '42501'; end if;
  ...
  if administration_id_value is null then
    select employment.administration_id into administration_id_value from public.employments employment
    where employment.tenant_id=journey_row.tenant_id and employment.hr_group_id=journey_row.hr_group_id
      ...
      and employment.starts_on<=journey_row.anchor_date and (employment.ends_on is null or employment.ends_on>=journey_row.anchor_date)
  ...
  for item in
    select distinct moment.id moment_id,moment.available_on,participant.id participant_id,participant.employee_id
    from public.journey_moments moment join public.journey_topics topic on topic.moment_id=moment.id
  ...
  loop
    ...
    values(journey_row.tenant_id,administration_id_value,journey_row.created_by_user_id,'HR','EMPLOYEES','Journey: '||coalesce(journey_row.template_name->>'nl','Journey'),'Open /journeys/'||journey_row.id::text||'#moment-'||item.moment_id::text,remind_at_value,'DRAFT',null) returning id into reminder_id;
