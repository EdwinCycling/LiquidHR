...
create type public.document_studio_asset_status as enum ('APPROVED', 'RETIRED');
...
create table public.document_studio_assets (
  ...
  status public.document_studio_asset_status not null default 'APPROVED',
  ...
  check ((status = 'APPROVED' and retired_at is null) or (status = 'RETIRED' and retired_at is not null))
);
...
begin
  ...
  if jsonb_typeof(requested_diagnostics) <> 'array' then
    raise exception 'DOCUMENT_TEMPLATE_VALIDATION_INVALID' using errcode = '22023';
  end if;
  update public.document_studio_template_versions
  set content_hash = requested_hash, validation_state = case when jsonb_array_length(requested_diagnostics) = 0 then 'VALID' else 'INVALID' end,
      validation_diagnostics = requested_diagnostics, updated_by_user_id = actor_id
  where id = draft_row.id;
  perform internal_security.document_studio_audit(
    draft_row.tenant_id,
    draft_row.hr_group_id,
    ...
    jsonb_build_object(
      ...
      'valid', jsonb_array_length(requested_diagnostics) = 0,
      'diagnosticsCount', jsonb_array_length(requested_diagnostics)
    )
  );
  return jsonb_build_object('draftId', draft_row.id, 'valid', jsonb_array_length(requested_diagnostics) = 0);
end;
$$;
...
create or replace function internal_security.create_document_studio_asset(
  requested_tenant_id uuid,
  requested_hr_group_id uuid,
...
begin
  ...
  if not internal_security.current_user_has_hr_group_permission(requested_tenant_id, requested_hr_group_id, 'document-asset:write') then
  ...
  insert into public.document_studio_assets (
    ...
    requested_asset_id, requested_tenant_id, requested_hr_group_id, 'APPROVED', requested_filename,
    requested_mime, requested_byte_size, requested_width, requested_height, requested_pixel_count,
    requested_sha256, requested_storage_key, actor_id
  ...
  perform internal_security.document_studio_audit(
    ...
    'CREATE',
    jsonb_build_object(
      'normalizedMime', asset_row.normalized_mime,
...
create or replace function public.create_document_studio_asset(
  requested_tenant_id uuid,
  requested_hr_group_id uuid,
  requested_asset_id uuid,
  requested_filename text,
...
set search_path = pg_catalog
as $$
  select internal_security.create_document_studio_asset(
    ...
    requested_pixel_count, requested_sha256, requested_storage_key
  );
$$;
...
revoke all on function public.create_document_studio_asset(
  uuid, uuid, uuid, text, text, integer, integer, integer, bigint, text, text
) from public, anon;
...
revoke all on function internal_security.create_document_studio_asset(uuid, uuid, uuid, text, text, integer, integer, integer, bigint, text, text) from public, anon, authenticated;
...
grant execute on function public.create_document_studio_asset(uuid, uuid, uuid, text, text, integer, integer, integer, bigint, text, text) to authenticated;
...
grant update on public.document_studio_templates to authenticated;
