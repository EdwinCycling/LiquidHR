...
begin
  ...
  effective_slug := coalesce(requested_slug, 'vacancy-' || left(vacancy.id::text, 8));
  ...
  if not found then
    ...
    values (vacancy.tenant_id, vacancy.hr_group_id, vacancy.id, effective_slug, requested_status, vacancy.title, vacancy.location_label, coalesce(requested_payload, '{}'::jsonb), case when requested_status = 'OPEN' then timezone('utc', now()) else null end, case when requested_status = 'CLOSED' then timezone('utc', now()) else null end, case when requested_status = 'ARCHIVED' then timezone('utc', now()) else null end)
    returning id into publication_id;
  else
    effective_slug := coalesce(requested_slug, publication.slug);
    update public.recruitment_publications set
      slug = effective_slug, status = requested_status, published_title = vacancy.title,
  ...
  return jsonb_build_object('id', publication_id, 'status', requested_status, 'slug', effective_slug);
end;
$$;
