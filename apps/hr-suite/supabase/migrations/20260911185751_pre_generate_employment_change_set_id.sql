-- Generate the change-set identifier before INSERT so its RETURNING clause does not require premature visibility of the incomplete employment graph.
do $$
declare
...
begin
  ...
  if function_definition is null or position(old_change_set_block in function_definition) = 0 or position('returning id into created_change_set_id' in lower(function_definition)) = 0 or position('created_change_set_id := gen_random_uuid()' in function_definition) > 0 then
