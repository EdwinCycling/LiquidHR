-- Generate the contract identifier before INSERT so INSERT ... RETURNING is not needed while the subsequent organization placement is still absent.
do $$
declare
...
begin
  ...
  if function_definition is null or position(old_contract_block in function_definition) = 0 or position('created_contract_id := gen_random_uuid()' in function_definition) > 0 then
