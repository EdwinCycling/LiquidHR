...
begin
  ...
  candidate_normalized_email := nullif(lower(btrim(requested_private_email)), '');
  ...
  values (vacancy.tenant_id, vacancy.hr_group_id, btrim(requested_first_name), btrim(requested_last_name), candidate_normalized_email, candidate_normalized_email, nullif(btrim(requested_phone), ''), duplicate_signal)
