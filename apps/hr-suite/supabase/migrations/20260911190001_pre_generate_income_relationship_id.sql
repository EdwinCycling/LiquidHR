-- Generate the income-relationship identifier before INSERT so its RETURNING clause does not require premature visibility of the incomplete graph.
do $$
declare
...
begin
  ...
  if function_definition is null or position(old_income_block in function_definition) = 0 or position('returning id into created_income_relationship_id' in lower(function_definition)) = 0 or position('created_income_relationship_id := gen_random_uuid()' in function_definition) > 0 then
