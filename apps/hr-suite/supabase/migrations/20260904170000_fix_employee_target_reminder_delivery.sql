-- An employee record does not necessarily have a login account yet. An HR
-- reminder for that record must still be publishable and actionable for the
-- HR author and, when available, the employee's active direct manager.
...
begin
  ...
  if not internal_security.current_user_has_permission(
    ...
    reminder_row.administration_id,
    'reminder:write'
  ) then
    raise exception 'REMINDER_FORBIDDEN' using errcode = '42501';
  end if;
  if reminder_row.status <> 'DRAFT' then
    raise exception 'REMINDER_NOT_DRAFT' using errcode = 'P0001';
  end if;
  if reminder_row.remind_at <= timezone('utc', now()) then
    raise exception 'REMINDER_IN_PAST' using errcode = '22023';
  end if;
  if reminder_row.target_type in ('DEPARTMENTS', 'EMPLOYEES')
    and not exists (
      select 1
  ...
  if reminder_row.target_type = 'EVERYONE' then
      ...
      and (
        ...
        or exists (
          ...
          where organization.employee_id = employee.id
            ...
            and (
              organization.effective_to is null
              or organization.effective_to >= reminder_row.remind_at::date
            )
        )
      );
    get diagnostics inserted_count = row_count;
    recipient_count := recipient_count + inserted_count;
    ...
    join public.employee_organizations organization
      on organization.department_id = target.department_id
     and organization.effective_from <= reminder_row.remind_at::date
     and (
       organization.effective_to is null
       or organization.effective_to >= reminder_row.remind_at::date
    ...
    join public.employees employee
      on employee.id = organization.employee_id
    ...
    -- Keep the target employee as the employee recipient when an account is
    -- linked. For preboarding/unlinked records, notify the responsible HR
    -- author and active direct manager instead of failing with zero recipients.
    insert into public.reminder_recipients (
      tenant_id, reminder_id, user_id, employee_id, effective_remind_at
    )
    select
    ...
    join public.employees employee
     ...
     and employee.deleted_at is null
    left join lateral (
      select manager.auth_user_id as user_id
      from public.employee_organizations organization
      join public.employees manager
        on manager.id = organization.direct_manager_id
       and manager.auth_user_id is not null
       and manager.is_active
       and manager.deleted_at is null
      where organization.employee_id = employee.id
        and organization.tenant_id = reminder_row.tenant_id
        and (
          reminder_row.administration_id is null
          or organization.administration_id = reminder_row.administration_id
        )
        and organization.effective_from <= reminder_row.remind_at::date
        and (
          organization.effective_to is null
      ...
      limit 1
    ) manager_fallback on true
    cross join lateral (
      values (reminder_row.created_by_user_id), (manager_fallback.user_id)
    ) fallback(user_id)
    where target.reminder_id = reminder_row.id
      and fallback.user_id is not null;
    get diagnostics inserted_count = row_count;
    recipient_count := recipient_count + inserted_count;
  end if;

  if recipient_count = 0 then
    raise exception 'REMINDER_NO_RECIPIENTS' using errcode = 'P0001';
  end if;
