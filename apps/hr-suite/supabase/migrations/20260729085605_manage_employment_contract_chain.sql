create function public.manage_employment_contract(
...
begin
  ...
  if requested_contract_id is null then
    ...
    update public.employment_labor_conditions
    set
      condition_group = labor_condition_name,
      valid_from = requested_starts_on,
      valid_until = case
        when requested_ends_on is null then null else requested_ends_on + 1
      end
    where employment_contract_id = current_contract.id;
  end if;
