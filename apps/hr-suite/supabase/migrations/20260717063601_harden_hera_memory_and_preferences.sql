...
begin
  ...
  if not exists (
    ...
    raise exception using errcode = '23514', message = 'HERA_MEMORY_SOURCE_SCOPE_INVALID';
  end if;
...
alter table public.ai_action_drafts
  add column action_type text not null default 'PERSONAL_REMINDER'
    check (action_type in ('PERSONAL_REMINDER','EMPLOYEE_ADDRESS_CHANGE','EMPLOYMENT_SALARY_CHANGE','EMPLOYMENT_SCHEDULE_CHANGE','ORGANIZATION_PLACEMENT_CHANGE')),
  add column version integer not null default 1 check (version > 0),
  add column control_payload jsonb not null default '{}'::jsonb;

alter table public.ai_action_drafts drop constraint ai_action_drafts_tool_name_check;
alter table public.ai_action_drafts
  add constraint ai_action_drafts_tool_name_check
  check (tool_name in ('draft_personal_reminder','draft_employee_address_change','draft_employment_salary_change','draft_employment_schedule_change','draft_organization_placement_change'));