...
declare tenant uuid; document_id uuid; reminder_id uuid; audience jsonb; target jsonb; actor_employee uuid;
begin
  ...
  for audience in select value from jsonb_array_elements(requested_payload->'audiences') loop
    ...
    values (tenant, requested_administration_id, document_id, (audience->>'type')::public.document_target_type,
  ...
  if actor_employee is not null then insert into public.document_audiences (tenant_id, administration_id, document_id, target_type, target_employee_id) values (tenant, requested_administration_id, document_id, 'EMPLOYEE', actor_employee) on conflict do nothing; end if;
  if requested_payload->'reminder' is not null and requested_payload->'reminder' <> 'null'::jsonb then
    ...
    values (tenant, requested_administration_id, auth.uid(), 'HR', 'EMPLOYEES', requested_payload->>'title', 'Document verloopt op ' || (requested_payload->>'expiresOn'), (requested_payload#>>'{reminder,remindAt}')::timestamptz, 'PUBLISHED', timezone('utc', now())) returning id into reminder_id;
    for target in select value from jsonb_array_elements(requested_payload#>'{reminder,targets}') loop
      ...
      values (tenant, requested_administration_id, reminder_id, (target->>'type')::public.document_target_type,
    ...
    select distinct tenant, reminder_id, employee.auth_user_id, employee.id, (requested_payload#>>'{reminder,remindAt}')::timestamptz
    from public.employees employee where employee.tenant_id = tenant and employee.auth_user_id is not null and employee.deleted_at is null and (
      exists (select 1 from public.reminder_target_rules rule where rule.reminder_id = reminder_id and rule.target_type='EMPLOYEE' and rule.target_employee_id=employee.id)
      or exists (select 1 from public.reminder_target_rules rule join public.user_access access on access.management_role_id=rule.target_management_role_id and access.user_id=employee.auth_user_id and access.is_active where rule.reminder_id=reminder_id and rule.target_type='MANAGEMENT_ROLE')
      or exists (with recursive branch as (select rule.target_department_id id from public.reminder_target_rules rule where rule.reminder_id=reminder_id and rule.target_type='DEPARTMENT_BRANCH' union all select department.id from public.departments department join branch on department.parent_id=branch.id) select 1 from public.employee_organizations organization where organization.employee_id=employee.id and organization.department_id in (select id from branch) and organization.effective_from<=current_date and (organization.effective_to is null or organization.effective_to>=current_date))
    ) on conflict (reminder_id, user_id) do nothing;
    update public.employee_documents set expiry_reminder_id = reminder_id where id = document_id;
  end if;
  return document_id;
end; $$;
