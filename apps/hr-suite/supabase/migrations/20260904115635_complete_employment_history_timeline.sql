...
select 'employment:'||employment.id||':start' event_id, employment.tenant_id, employment.administration_id, employment.employee_id, employment.id employment_id,
  ...
  jsonb_build_object('number',employment.employment_number,'contractType',employment.contract_type,'validUntil',employment.ends_on) title_values,
  '/employees/'||employment.employee_id||'/employments/'||employment.id||'?tab=overview' source_href, 'INFO'::text severity
...
select 'employment:'||employment.id||':end', employment.tenant_id, employment.administration_id, employment.employee_id, employment.id,
  employment.ends_on, 'EMPLOYMENT_ENDED', 'employment.event.ended', jsonb_build_object('number',employment.employment_number,'validUntil',employment.ends_on),
  '/employees/'||employment.employee_id||'/employments/'||employment.id||'?tab=overview', 'ATTENTION'
...
select 'ikv:'||link.id, link.tenant_id, link.administration_id, link.employee_id, link.employment_id, link.valid_from,
  'INCOME_RELATIONSHIP_CHANGED', 'employment.event.incomeRelationship', jsonb_build_object('number',relationship.ikv_number,'validUntil',link.valid_until),
  '/employees/'||link.employee_id||'/employments/'||link.employment_id||'?tab=overview', 'INFO'
...
union all
select 'contract:'||contract.id, contract.tenant_id, contract.administration_id, contract.employee_id, contract.employment_id,
  contract.starts_on, 'CONTRACT_CHANGED', 'employment.event.contract', jsonb_build_object('number',contract.sequence_number,'contractType',contract.duration_type,'workerType',contract.worker_type,'startsOn',contract.starts_on,'validUntil',case when contract.ends_on is null then null else contract.ends_on + 1 end),
  '/employees/'||contract.employee_id||'/employments/'||contract.employment_id||'?tab=overview', 'INFO'
...
select 'organization:'||organization.id, organization.tenant_id, organization.administration_id, organization.employee_id, organization.employment_id,
  organization.effective_from, 'ORGANIZATION_CHANGED', 'employment.event.organization', jsonb_build_object('jobTitle',organization.job_title,'department',department.name,'validUntil',organization.effective_to),
...
select 'labor:'||labor.id, labor.tenant_id, labor.administration_id, labor.employee_id, labor.employment_id, labor.valid_from,
  'LABOR_CONDITIONS_CHANGED', 'employment.event.labor', jsonb_build_object('conditionGroup',labor.condition_group,'validUntil',labor.valid_until),
  '/employees/'||labor.employee_id||'/employments/'||labor.employment_id||'?tab=overview', 'INFO'
...
select 'schedule:'||schedule.id, schedule.tenant_id, schedule.administration_id, schedule.employee_id, schedule.employment_id, schedule.valid_from,
  'SCHEDULE_CHANGED', 'employment.event.schedule', jsonb_build_object('hours',schedule.average_hours_per_week,'factor',schedule.part_time_factor,'validUntil',schedule.valid_until),
...
select 'salary:'||salary.id, salary.tenant_id, salary.administration_id, salary.employee_id, salary.employment_id, salary.valid_from,
  'SALARY_CHANGED', 'employment.event.salary', jsonb_build_object('amount',salary.fulltime_amount,'currency',salary.currency_code,'validUntil',salary.valid_until),
  '/employees/'||salary.employee_id||'/employments/'||salary.employment_id||'?tab=salary', 'ATTENTION'
from public.employment_salaries salary
union all
select 'cost:'||allocation.id, allocation.tenant_id, allocation.administration_id, allocation.employee_id, allocation.employment_id,
  allocation.valid_from, 'COST_ALLOCATION_CHANGED', 'employment.event.costAllocation', jsonb_build_object('percentage',allocation.percentage,'costCenter',center.name,'validUntil',allocation.valid_until),
