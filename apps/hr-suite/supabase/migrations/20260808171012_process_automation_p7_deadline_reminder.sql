...
begin
  ...
  select subject.employee_id, employee.auth_user_id
  into subject_employee_id, subject_auth_user_id
  from public.process_employee_subjects subject
  join public.employees employee
    on employee.tenant_id = subject.tenant_id
   and employee.hr_group_id = subject.hr_group_id
   and employee.id = subject.employee_id
  where subject.tenant_id = job_row.tenant_id
    and subject.hr_group_id = job_row.hr_group_id
    and subject.process_instance_id = job_row.process_instance_id
  limit 1;
  ...
  insert into public.reminders (
    ...
    reminder_at, 'PUBLISHED'::public.reminder_status, timezone('utc', now())
