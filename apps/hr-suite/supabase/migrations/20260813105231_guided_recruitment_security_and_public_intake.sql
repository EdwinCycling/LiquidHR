...
begin
  ...
  if exists (select 1 from public.recruitment_events event where event.application_id = application.id and event.idempotency_key = requested_idempotency_key) then
    return jsonb_build_object('id', application.id, 'version', application.version, 'idempotentReplay', true);
  end if;
  ...
  values (application.tenant_id, application.hr_group_id, application.id, 'APPLICATION_STAGE_CHANGED', requested_idempotency_key, (select auth.uid()), jsonb_build_object('version', application.version));
  return jsonb_build_object('id', application.id, 'version', application.version, 'idempotentReplay', false);
end;
$$;
...
begin
  ...
  if exists (select 1 from public.recruitment_events event where event.application_id = application.id and event.idempotency_key = requested_idempotency_key) then
    return jsonb_build_object('id', application.id, 'version', application.version, 'outcome', application.terminal_outcome, 'idempotentReplay', true);
  end if;
  if application.version <> expected_version then raise exception 'RECRUITMENT_VERSION_CONFLICT' using errcode = '40001'; end if;
  if application.terminal_outcome is not null then raise exception 'RECRUITMENT_APPLICATION_TERMINAL' using errcode = 'P0001'; end if;
  ...
  insert into public.recruitment_events (tenant_id, hr_group_id, application_id, event_type, idempotency_key, actor_user_id, payload)
  values (application.tenant_id, application.hr_group_id, application.id, 'APPLICATION_' || requested_outcome, requested_idempotency_key, (select auth.uid()), jsonb_build_object('version', application.version));
  return jsonb_build_object('id', application.id, 'version', application.version, 'outcome', requested_outcome, 'idempotentReplay', false);
end;
$$;
...
begin
  ...
  if requested_idempotency_key is null or char_length(requested_idempotency_key) < 8 then raise exception 'RECRUITMENT_IDEMPOTENCY_REQUIRED' using errcode = '22023'; end if;
  if exists (select 1 from public.recruitment_events event where event.application_id = application.id and event.idempotency_key = requested_idempotency_key) then
    return jsonb_build_object('id', application.id, 'version', application.version, 'idempotentReplay', true, 'participantsRestored', false);
  end if;
  if application.version <> expected_version then raise exception 'RECRUITMENT_VERSION_CONFLICT' using errcode = '40001'; end if;
  ...
  where id = application.id returning * into application;
  insert into public.recruitment_events (tenant_id, hr_group_id, application_id, event_type, idempotency_key, actor_user_id, payload)
  values (application.tenant_id, application.hr_group_id, application.id, 'APPLICATION_REOPENED', requested_idempotency_key, (select auth.uid()), jsonb_build_object('version', application.version, 'participantsRestored', false));
  return jsonb_build_object('id', application.id, 'version', application.version, 'idempotentReplay', false, 'participantsRestored', false);
end;
$$;
...
begin
  ...
  normalized_email := nullif(lower(btrim(requested_payload->>'email')), '');
  ...
  values (publication.tenant_id, publication.hr_group_id, btrim(requested_payload->>'firstName'), btrim(requested_payload->>'lastName'), normalized_email, normalized_email, nullif(btrim(requested_payload->>'phone'), ''), duplicate_signal)
