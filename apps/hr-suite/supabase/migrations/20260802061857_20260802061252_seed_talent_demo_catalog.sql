...
with capability_seed(capability_type, code, name, description, category_code, status, language_code, language_cefr, language_is_native, certificate_issuing_body, certificate_validity_months, certificate_is_permanent, certificate_code, certificate_renewal_required) as (
  values
    ...
    ('COMPETENCY', 'SAFETY_AWARENESS', 'Veiligheidsbewustzijn', 'Herkent risico�u���l�w^~)�vs en handelt volgens veilige werkwijzen.', 'DELIVERY', 'ACTIVE', null, null, false, null, null, false, null, false),
...
set status = 'ACTIVE',
    ...
    tasks = case job.code
      ...
      when 'TEST-PLANNER' then '["planning maken", "capaciteitsrisico�w^~)�v��y��y�s signaleren", "scenario�u���l�w^~)�vs vergelijken"]'::jsonb
...
with requirement_seed(job_code, capability_code, requirement_type, target_level_code, language_level, certificate_details, rationale, sort_order) as (
  values
    ...
    ('TEST-SUPERVISOR', 'SAFETY_AWARENESS', 'REQUIRED', 'L3', null, null, 'Risico�w^~)�v��y��y�s herkennen en opvolgen.', 2),
