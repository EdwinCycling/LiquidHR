...
describe('test role switch', () => {
  ...
  it('vereist de expliciete flag en de canonical DEV Supabase-projectref', () => {
    expect(isTestRoleSwitchEnabled({ nodeEnv: 'development', explicitFlag: 'true', supabaseUrl: DEV_SUPABASE_URL })).toBe(true)
    expect(isTestRoleSwitchEnabled({ nodeEnv: 'development', explicitFlag: 'false', supabaseUrl: DEV_SUPABASE_URL })).toBe(false)
  ...
  it('resolveert alleen een kale Supabase-project-URL naar een ref', () => {
    expect(resolveSupabaseProjectRef(DEV_SUPABASE_URL)).toBe('wnpfloqpjvaacobppbpk')
  ...
  it('kan op de Vercel production-target met flag en canonical DEV-ref werken', () => {
    expect(isTestRoleSwitchEnabled({ nodeEnv: 'production', vercelEnv: 'production', explicitFlag: 'true', supabaseUrl: DEV_SUPABASE_URL })).toBe(true)
    expect(isTestRoleSwitchEnabled({ nodeEnv: 'production', vercelEnv: 'production', explicitFlag: 'false', supabaseUrl: DEV_SUPABASE_URL })).toBe(false)
  ...
  it('kan in Vercel Preview alleen met dezelfde twee gates werken', () => {
    expect(isTestRoleSwitchEnabled({ nodeEnv: 'production', vercelEnv: 'preview', explicitFlag: 'true', supabaseUrl: DEV_SUPABASE_URL })).toBe(true)
    expect(isTestRoleSwitchEnabled({ nodeEnv: 'production', vercelEnv: 'preview', explicitFlag: ' TRUE ', supabaseUrl: DEV_SUPABASE_URL })).toBe(true)
