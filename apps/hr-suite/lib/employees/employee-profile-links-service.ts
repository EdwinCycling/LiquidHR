...
export async function createEmployeeProfileLink(employeeId: string, input: ProfileLinkInput) {
  ...
  const { data: employee, error: employeeError } = await supabase.from('employees').select('id').eq('id', employeeId).eq('tenant_id', context.tenantId).eq('hr_group_id', requireHrGroupId(context)).is('deleted_at', null).maybeSingle()
