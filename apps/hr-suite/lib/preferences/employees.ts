...
export async function getStoredEmployeesFilterPanelOpen(): Promise<boolean> {
  ...
  if (!isRecord(data?.ui_state) || !isRecord(data.ui_state.employeesList)) return true
  return data.ui_state.employeesList.filterPanelOpen !== false
}
