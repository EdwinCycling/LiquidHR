...
import { getNormalizedDocumentV1 } from '@/lib/document-studio/service'
...
const inputSchema = z.object({ templateVersionId: z.string().uuid(), employeeId: z.string().uuid(), freeInputs: z.record(z.string(), z.string()).default({}), temporalInputs: z.record(z.string(), z.string()).default({}) })
...
export async function createGenerationPreview(value: unknown): Promise<{ id: string; status: 'PREVIEW'; freeKeys: string[]; resolvedDocumentHash: string }> {
  ...
  const inserted = await table(client, 'document_generation_snapshots').insert(payload).select('id').single(); if (inserted.error) throw new DocumentGenerationError('DOCUMENT_GENERATION_CREATE_FAILED', 500)
  await table(client, 'document_generation_audit').insert({ tenant_id: tenantId, hr_group_id: hrGroupId, snapshot_id: id, action: 'PREVIEW_CREATED', actor_user_id: userId, metadata: { templateVersionId: normalized.templateVersionId, resolvedDocumentHash: snapshot.resolvedDocumentHash } })
  return { id, status: 'PREVIEW', freeKeys, resolvedDocumentHash: snapshot.resolvedDocumentHash }
}

export async function finalizeGeneration(snapshotId: string): Promise<{ id: string; status: 'FINAL'; pdfHash: string }> {
  const { client, tenantId, hrGroupId, userId } = await authz(); const snapshot = await row<Record<string, unknown>>(table(client, 'document_generation_snapshots').select('*').eq('tenant_id', tenantId).eq('hr_group_id', hrGroupId).eq('id', snapshotId).maybeSingle())
  ...
  const update = await table(client, 'document_generation_snapshots').update({ status: 'FINAL', final_pdf_hash: hash, final_storage_key: key, finalized_at: new Date().toISOString(), finalized_by_user_id: userId }).eq('tenant_id', tenantId).eq('hr_group_id', hrGroupId).eq('id', snapshotId).eq('status', 'PREVIEW').select('id').maybeSingle(); if (update.error || !update.data) throw new DocumentGenerationError('DOCUMENT_GENERATION_ALREADY_FINALIZED', 409)
  await table(client, 'document_generation_audit').insert({ tenant_id: tenantId, hr_group_id: hrGroupId, snapshot_id: snapshotId, action: 'FINALIZED', actor_user_id: userId, metadata: { finalPdfHash: hash, rendererVersion: DG1_RENDERER_VERSION } })
  return { id: snapshotId, status: 'FINAL', pdfHash: hash }
}
