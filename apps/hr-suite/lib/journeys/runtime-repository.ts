...
export const supabaseJourneyRuntimeRepository: JourneyRuntimeRepository = {
  ...
  async get(tenantId, hrGroupId, journeyId) {
    ...
    return {
      ...
      topics: (topics.data ?? []).map((topic) => ({ id: topic.id, momentId: topic.moment_id, key: topic.key, title: localized(topic.title), body: localized(topic.body), topicType: topic.topic_type, isRequired: topic.is_required, status: topic.status, actionUrl: topic.action_url, ownerRoleKey: topic.owner_role_key, ownerNames: (assignments.data ?? []).filter((assignment) => assignment.topic_id === topic.id && assignment.is_owner).map((assignment) => names.get(participantById.get(assignment.participant_id)?.employee_id ?? '') ?? '') .filter(Boolean) })),
