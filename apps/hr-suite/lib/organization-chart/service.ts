...
export async function getOrganizationChart(query: OrganizationChartQuery): Promise<OrganizationChartGraph> {
  ...
  return projectOrganizationChart({
    ...
    managementAssignments: (managementResult.data ?? []).flatMap((assignment) => {
      ...
      return role && role.is_organization_scoped && assignment.department_id ? [{ id: assignment.id, departmentId: assignment.department_id, employeeId: assignment.employee_id, roleCode: role.code, roleName: role.name, effectiveFrom: assignment.effective_from, effectiveTo: assignment.effective_to }] : []
    }),
