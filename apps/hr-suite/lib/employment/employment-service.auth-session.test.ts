...
function query(result: { data: unknown; error: null | Error }) {
  const builder = {
    select: vi.fn(), eq: vi.fn(), is: vi.fn(), lte: vi.fn(), or: vi.fn(), limit: vi.fn(), maybeSingle: vi.fn().mockResolvedValue(result),
  }
  builder.select.mockReturnValue(builder); builder.eq.mockReturnValue(builder); builder.is.mockReturnValue(builder); builder.lte.mockReturnValue(builder); builder.or.mockReturnValue(builder); builder.limit.mockReturnValue(builder)
  return builder
}
...
function createClient(userResult: { data: { user: { id: string } | null }; error: null | Error }) {
  ...
  return {
    auth: { getUser: vi.fn().mockResolvedValue(userResult) }, rpc,
    from: vi.fn((table: string) => {
      if (table === 'labor_condition_sets') return query({ data: { probation_maximum_months: 1 }, error: null })
      if (table === 'employee_administration_assignments') return query({ data: { id: 'assignment-1' }, error: null })
      throw new Error(`Onverwachte tabel in test: ${table}`)
    }),
  }
}

function completeInput(withSalary = false): CompleteEmploymentCreateInput {
  return {
    employment: { employmentNumber: '100015', employmentType: 'EMPLOYEE', startsOn: '2026-08-01', seniorityDate: '2026-08-01', countryCode: 'NL', isPrimary: true },
    contract: { workerType: 'EMPLOYEE', laborConditionSetId: '00000000-0000-0000-0000-000000000001', durationType: 'DEFINITE', startsOn: '2026-08-01', endsOn: '2026-10-01', probationApplies: true, probationEndsOn: '2026-09-01' },
    ...(withSalary ? { salary: { paymentType: 'PERIODIC_FIXED' as const, paymentFrequency: 'MONTHLY' as const, salaryBasis: 'MANUAL' as const, salaryRoute: 'MANUAL' as const, fulltimeAmount: 2000, parttimeAmount: 1600, currencyCode: 'EUR', salaryFrequencyId: '00000000-0000-0000-0000-000000000002', validFrom: '2026-08-01' } } : {}),
  }
}

describe('complete employment request session verification', () => {
  beforeEach(() => { mocks.getRequestAuthorizationContext.mockReset(); mocks.requirePermission.mockReset(); mocks.requirePermission.mockResolvedValue(context) })
  ...
  it.each([
    ...
    mocks.getRequestAuthorizationContext.mockResolvedValue({ context, supabase: client, activeContext: { administrationsInActiveHrGroup: [{ id: 'admin-1' }] } })
  ...
  it.each([
    ['getUser-fout', { data: { user: { id: 'user-1' } }, error: new Error('sessie error') }],
