...
export const documentMetadataSchema = z.object({
  ...
  audiences: z.array(target).min(1).max(100).transform((items) => [...new Map(items.map((item) => [`${item.type}:${item.targetId}`, item])).values()]),
  reminder: z.object({ remindAt: z.string().datetime({ offset: true }), targets: z.array(target).min(1).max(100) }).strict().nullable(),
}).strict().superRefine((value, context) => {
  if (value.reminder && !value.expiresOn) context.addIssue({ code: 'custom', path: ['expiresOn'], message: 'EXPIRY_REQUIRED' })
})
