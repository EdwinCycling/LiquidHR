...
export async function listJobCatalog() {
  ...
  const [groups, jobs, relations, families, seniorities] = await Promise.all([
    supabase.from('job_groups').select('id, code, name, description, is_active, job_family_id').eq('tenant_id', auth.tenantId).order('code').limit(500),
    supabase.from('jobs').select('id, code, job_group_id, seniority_id, is_active, job_revisions(id, name, description, valid_from, valid_until)').eq('tenant_id', auth.tenantId).order('code').limit(500),
    supabase.from('job_group_jobs').select('job_id, job_group_id').eq('tenant_id', auth.tenantId).limit(5000),
...
export async function createJobGroup(input: JobGroupCreateInput) {
  ...
  const { data, error } = await supabase.from('job_groups').insert({
    tenant_id: auth.tenantId, code: input.code.toUpperCase(),
...
export async function createJob(input: JobCreateInput) {
  ...
  const { data, error } = await supabase.rpc('create_job_with_revision', {
    requested_tenant_id: auth.tenantId, requested_payload: input as Json,
  })
  if (error || !data) databaseError(error?.message ?? 'JOB_CREATE_FAILED')
  return data
}

async function assertActiveJobGroups(supabase: Awaited<ReturnType<typeof createClient>>, tenantId: string, groupIds: string[]): Promise<void> {
  const { data, error } = await supabase.from('job_groups').select('id').eq('tenant_id', tenantId).eq('is_active', true).in('id', groupIds)
  if (error) databaseError(error.message)
...
export async function updateJobGroup(groupId: string, input: JobGroupUpdateInput): Promise<void> {
  ...
  const { data, error } = await supabase.from('job_groups').update(payload).eq('tenant_id', auth.tenantId).eq('id', groupId).select('id').maybeSingle()
  if (error) databaseError(error.message)
...
export async function deleteJobGroup(groupId: string): Promise<void> {
  const auth = await tenantContext('job-catalog:write')
  const supabase = await createClient()
  const { count, error: relationError } = await supabase.from('job_group_jobs').select('job_id', { count: 'exact', head: true }).eq('tenant_id', auth.tenantId).eq('job_group_id', groupId)
  if (relationError) databaseError(relationError.message)
  if ((count ?? 0) > 0) databaseError('JOB_GROUP_IN_USE')
  const { data, error } = await supabase.from('job_groups').delete().eq('tenant_id', auth.tenantId).eq('id', groupId).select('id').maybeSingle()
  if (error) databaseError(error.message)
  if (!data) databaseError('JOB_GROUP_NOT_FOUND')
}
...
export async function updateJob(jobId: string, input: JobUpdateInput): Promise<void> {
  const auth = await tenantContext('job-catalog:write')
  const supabase = await createClient()
  if (input.jobGroupIds) await assertActiveJobGroups(supabase, auth.tenantId, input.jobGroupIds)
  ...
  const { data: job, error: jobError } = await supabase.from('jobs').update(jobPayload).eq('tenant_id', auth.tenantId).eq('id', jobId).select('id').maybeSingle()
  if (jobError) databaseError(jobError.message)
  if (!job) databaseError('JOB_NOT_FOUND')
  if (input.jobGroupIds) {
    const { error: deleteRelationsError } = await supabase.from('job_group_jobs').delete().eq('tenant_id', auth.tenantId).eq('job_id', jobId)
    if (deleteRelationsError) databaseError(deleteRelationsError.message)
    const { error: relationError } = await supabase.from('job_group_jobs').insert(input.jobGroupIds.map((jobGroupId) => ({ tenant_id: auth.tenantId, job_group_id: jobGroupId, job_id: jobId })))
    if (relationError) databaseError(relationError.message)
  }
  if (input.name !== undefined || input.description !== undefined) {
    const { data: revision, error: revisionError } = await supabase.from('job_revisions').select('id, name, description').eq('tenant_id', auth.tenantId).eq('job_id', jobId).order('valid_from', { ascending: false }).limit(1).maybeSingle()
    if (revisionError) databaseError(revisionError.message)
    if (!revision) databaseError('JOB_REVISION_NOT_FOUND')
    const { error } = await supabase.from('job_revisions').update({ name: input.name ?? revision.name, description: input.description === undefined ? revision.description : input.description }).eq('tenant_id', auth.tenantId).eq('id', revision.id)
    if (error) databaseError(error.message)
  }
}
...
export async function deleteJob(jobId: string): Promise<void> {
  const auth = await tenantContext('job-catalog:write')
  const supabase = await createClient()
  const { count, error: usageError } = await supabase.from('employee_organizations').select('id', { count: 'exact', head: true }).eq('tenant_id', auth.tenantId).eq('job_id', jobId)
  if (usageError) databaseError(usageError.message)
  if ((count ?? 0) > 0) databaseError('JOB_IN_USE')
  const { data, error } = await supabase.from('jobs').delete().eq('tenant_id', auth.tenantId).eq('id', jobId).select('id').maybeSingle()
  if (error) databaseError(error.message)
