...
export type DocumentCategory = Pick<Database['public']['Tables']['document_categories']['Row'], 'id' | 'code' | 'name' | 'description' | 'requires_salary_permission' | 'is_active'>
...
export async function listDocumentCategories() {
  ...
  const { data, error } = await supabase.from('document_categories').select('id, code, name, description, requires_salary_permission, is_active').eq('tenant_id', context.tenantId).eq('administration_id', context.administrationId).order('code').limit(300)
