...
| HELP-03 | HR Admin onboarding sidepanel | initial visibility, close/reopen, checklist state and contextual hide | HR Admin | mobile fullscreen/usable | TESTED | Setup Assistant initial visibility, open/close/reopen, `Niet meer tonen` action and 390x844 layout exercised. |
...
| HELP-05 | Checklist lifecycle | incomplete/completed detection, refresh/relogin persistence | HR Admin actor/tenant scope | state remains usable | TESTED | First checklist item marked complete, persisted across refresh and relogin, then restored incomplete; contextual hide/re-enable did not reset the incomplete checklist state and no business row was created. |
| HELP-06 | Literal `Niet meer tonen` control | exact label/control requested by spec | HR Admin | control reachable | TESTED | Sidepanel button with the exact NL label hid the assistant through the canonical setting; trigger stayed hidden after refresh and relogin, while Settings showed disabled. |
| HELP-06A | Setup Assistant visibility toggle | administrative/user re-enable and persisted visibility | HR Admin/settings write | control usable at 390x844 | TESTED | Settings `Setup Assistent tonen` re-enabled the same canonical setting and restored the floating trigger; refreshed Settings showed checked/enabled and the mobile sidepanel remained usable. |
