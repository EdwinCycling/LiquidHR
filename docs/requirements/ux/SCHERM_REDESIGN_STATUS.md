...
## Final integration status — 2026-08-21

De vier UX v1.2 correction batches zijn geïntegreerd op `work/ux-v1-2-integration` vanaf `origin/main` `a65d1daaa9444602f4be52c2d32933cffd285dd0`. De gecombineerde acceptance is **SCHEMA APPLY REQUIRED / niet volledig bewezen**: employee personal/notes/addresses/avatar, startpage responsive gedrag, theme-switching en negatieve Manager/Employee-toegang zijn gecontroleerd; document upload faalt met `DOCUMENT_METADATA_FAILED` (400) voor PDF en TXT. De echte RPC-fout is `42501` door `RETURNING id` vóór audience-creatie onder de bestaande RLS SELECT-policy. Absence, employment real mutation, HR authorization create/coverage en process publish/retire blijven open totdat de TEST-functie via expliciete schema/function apply is gecorrigeerd. Storage cleanup is gecontroleerd met 0 resterende orphans op de DEMO-035-prefix. Zie [`delivery/CURRENT_CONTEXT.md`](../../delivery/CURRENT_CONTEXT.md) en [`delivery/TEST_ACCEPTANCE_MATRIX.md`](../../delivery/TEST_ACCEPTANCE_MATRIX.md).
