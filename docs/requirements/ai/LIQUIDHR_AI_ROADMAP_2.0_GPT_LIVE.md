...
De eerste slice start vanuit een medewerkerprofiel voor een manager, HR-adviseur, HR Admin of tenant-admin. De browser maakt een WebRTC-offer en stuurt dat naar de serverroute. De server valideert de medewerkercontext en `ai:use`, voegt een beperkte Live toolconfiguratie toe en gebruikt de permanente OpenAI-key uitsluitend server-side voor `POST /v1/live/sessions` met een JSON-body `{ session, transport: { type: 'webrtc', sdp } }`. De sessieconfiguratie bevat `model = gpt-live-1`; het legacy `session.type`-veld en de Realtime-callvorm (`/v1/realtime/calls` met `type: realtime`) worden niet gebruikt. De browser ontvangt alleen het SDP-answer en wacht op `session.started` voordat client-events worden verstuurd.
