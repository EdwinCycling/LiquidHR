...
De canonical readback faalde eenmaal op `FIXTURE_UNEXPECTED_FAILURE: fetch failed` omdat `NEXT_PUBLIC_APP_URL` naar de niet-luisterende lokale poort `3107` wees. Supabase zelf was bereikbaar: DNS resolveerde, `/auth/v1/settings` gaf HTTP 200 en read-only sign-in slaagde voor HR, Manager en Employee. Met de vers gebouwde integratie op `127.0.0.1:3003` bereikte de canonical setup de bestaande TEST-contractlaag, maar de vaste key `r5-test:claimed-work-item` gaf `HTTP 409 IDEMPOTENCY_KEY_REUSED`; er is geen nieuwe succesvolle fixturecreatie of eigen probe-record aangetoond. Cleanup is daarom niet uitgevoerd. `migrations: NO` blijft bevestigd.
