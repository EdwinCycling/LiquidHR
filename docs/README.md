...
De canonical readback faalde eenmaal op `FIXTURE_UNEXPECTED_FAILURE: fetch failed` omdat `NEXT_PUBLIC_APP_URL` naar de niet-luisterende lokale poort `3107` wees. Supabase zelf was bereikbaar: DNS resolveerde, `/auth/v1/settings` gaf HTTP 200 en read-only sign-in slaagde voor HR, Manager en Employee. Met de vers gebouwde integratie op `127.0.0.1:3003` bereikte de canonical setup de bestaande TEST-contractlaag, maar de vaste key `r5-test:claimed-work-item` gaf `HTTP 409 IDEMPOTENCY_KEY_REUSED`. Read-only inspectie bewees één fixture-eigen record in `r5-test-internal-transfer`, met de canonical tenant/hr-group, subject employee/employment en correlation-id; de scoped inventory bevatte 5 definitions en 7 instances. De bestaande canonical cleanup bereikte de API maar faalde bij `supabase db query` door ontbrekende CLI-auth (`LegacyPlatformAuthRequiredError`); een beschikbare SQL-verbinding kon `session_replication_role` niet instellen en de append-only trigger rolde een tweede poging volledig terug. De residual readback bevestigde daarna 1 conflicterende key, 5 definitions en 7 instances. Setup/readback zijn daardoor niet opnieuw uitgevoerd en `migrations: NO` blijft bevestigd.

De eerste `agent-browser`-route faalde tweemaal met `CDP response channel closed`. De bestaande Playwright CLI/runtime-route werkte vervolgens wel. Authenticated HR, Manager en Employee zijn op `/work`, een UUID-workdetail en `/process-runtime/<workItemId>` gecontroleerd op desktop `1440x900` en mobiel `390x844`; de routes gaven HTTP 200, URL-state en terugcontext bleven behouden, overflow bleef gelijk aan de viewport, keyboard-focus was na Tab bereikbaar en de relevante browserconsole bleef fout- en waarschuwingvrij. HR kreeg assignment-options HTTP 200; Manager en Employee HTTP 403. HR kreeg `/settings/process-automation` HTTP 200; Manager en Employee werden naar `/geen-toegang` geweerd. De resterende `net::ERR_ABORTED`-events waren afgebroken Next-prefetches bij navigatie en geen console- of productfouten. De authenticated browsergate is daarmee implementationeel groen; volledige fixture/readback-acceptance blijft door lokale endpointconfiguratie en bestaande TEST-idempotentiestatus geblokkeerd.
