...
## Process Automation P2/P3-gate 2026-08-08: datamodel, resolver en work-item kernel
...
De implementatie-, gedrags- en cleanup-gate is bereikt. De twee tijdelijke concurrency-fixtures (`p3-concurrency-contract` en `p3-concurrency-contract-2`) zijn na de expliciete bevestiging transactioneel verwijderd; de gecontroleerde cascade-relaties rapporteren nul rijen en de append-only/immutable-triggers staan weer actief. De definitieve P2/P3-handoff is daarmee 100% afgerond. P4 en P5 zijn nog niet gestart.

## Dienstverbandwizard 2026-08-08

De dienstverbandwizard gebruikt een vaste sticky onderbalk per tab, een scrollbaar middenstuk en een linker stapnavigatie die binnen de viewport blijft. De administratie-tab toont geen lege scrollhint. Contractdatums en proeftijd worden onderling gecontroleerd met waarschuwingen voor korte contracten, verre startdatums en lange proeftijden. Roosteruren zijn niet-negatief, maximaal 50 uur per week en ondersteunen een tweede week met exacte gemiddeldecontrole. De fulltime-norm komt uit de gekozen CAO/bedrijfsregeling, salarisfrequenties volgen de administratie-inrichting, leidinggevenden worden uit afdeling en HR-inrichting afgeleid en kostenplaatsen kunnen met één gedeelde kostendrager worden gesplitst. De controle-tab toont het relevante salaris, afdeling en functie en stuurt bij ongeldige opslag gericht terug naar de eerste foutieve stap.
...
| Process Automation | [`requirements/workflows/LIQUID_PROCESS_AUTOMATION_BLUEPRINT.md`](requirements/workflows/LIQUID_PROCESS_AUTOMATION_BLUEPRINT.md) | LEIDEND | P2/P3-IMPLEMENTATIE EN GEDRAGSGATE BEREIKT — schema/RLS/grants, resolver, work-item kernel, API en contracttests lokaal/remote; cleanup van tijdelijke remote testfixtures blokkeert de definitieve 100%-handoff; P4-transition engine/UI niet gestart |
