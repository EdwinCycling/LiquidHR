...
## Niet bereikbaar op huidige TEST-runtime

De fixture markeert deze categorieën als RED en maakt geen kunstmatige status/timestamp:
...
De Employee-read op het Manager approval-item van hetzelfde Employee-subject gaf HTTP `200`; dit is volgens het bestaande self-process-instance contract toegestaan en is geen cross-scope negatieve test. Een tweede strict-UUID Employee-persona met niet-eigen subject kon binnen deze opdracht niet veilig worden aangemaakt. Daarom is de volledige negatieve cross-scope matrix niet bewezen en blijft de eindstatus RED.
...
Eindstatus: **RED — NOT SAFE AS SHARED R5 FIXTURE** totdat HR-queue assignment, deadline materialization, blocked-path en een echte non-self negative persona beschikbaar zijn.
