...
De geïntegreerde production-mode sanity op `http://localhost:3002` is eveneens groen: `/settings`, alle R7-3-representatieve routes, Sidebar v2-secties, Wat-is-nieuw/Gift Drawer, Setup Assistant en Organogram gaven HTTP `200`, zonder overflow. De bestaande private-avatarweergave in Organogram kreeg alleen de noodzakelijke browser-cookiecompatibiliteitsfix; de gerichte Organogram-projectortest is `6/6` groen en de recheck-console is leeg.
...
- Centrale integratie: R7-3 is semantisch geïntegreerd op actuele `main`; salary capability visibility blijft expliciet open.

## Scope safety

Geen migration, schema/RLS/RPC of structurele remote database write. De lokale R7-3-sourcecommit `05ad586b` is semantisch geïntegreerd; deze release bevat de ene centrale version bump en alleen de aantoonbare Organogram-avatarcompatibiliteitsfix. De centrale releasecommit is gereed voor push en Vercel-provenancecontrole.
