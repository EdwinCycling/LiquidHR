...
## Leave profile management UX refinement + approval presentation — 2026-09-12
...
Afgerond: Verlofprofielen is de eerste workspace-tab en de default zonder section-parameter; expliciete deep links blijven werken. Profielen tonen actuele verloftypen met kleur en compacte regelwaarden, lege profielen hebben de HR-admin CTA `Verlofsoort toevoegen`, en toevoegen/wijzigen gebruikt één gedeelde modal-laag rond de bestaande `AccrualRuleEditor`. De Verlofsoorten-catalogus toont profielgebruik en biedt `Bewerken` en `Aan profiel koppelen`; dubbele actuele koppeling wordt in de selector geblokkeerd. Voor `CONTRACT_HOURS` gebruikt de editor `accrual_amount` als `Jaarrecht fulltime`, toont hij de afgeleide jaar/maand/4-wekenweergave en licht hij kalenderdagen naar rato toe; het verwachte engine-contract staat in `docs/requirements/leave/VERLOF_OPBOUW_ENGINE.md`, zonder enginewijziging. De approval-kolom gebruikt nu de expliciete `requires_manager_approval`-waarde uit het bestaande Leave-catalogus-read-model: `Ja`/`Nee` in plaats van een onterechte `Niet ingericht`; alleen een unset waarde blijft niet ingericht. NL/EN-kopie is gelijkgetrokken.

Lokale verificatie: gerichte presentatie-/workspace-tests `5/5`, strict TypeScript, ESLint, i18n-pariteit en Webpack production build zijn groen. De volledige suite heeft `358` geslaagde tests en twee bestaande niet-gerelateerde failures in Document Studio/DG1-PDF. Authenticated browseracceptance gebruikt de normale HR Admin-fixture in Planeten; de huidige Leave-configuratie blijft intact en er wordt geen testconfiguratie aangemaakt of gewijzigd. Er is geen schemawijziging, migration, API-, RLS-, resolver- of enginewijziging.

De release-evidence voor commit, push en de GitHub-sourced Preview hoort bij deze afsluitende run. Protected `.env.local` is alleen op bestaan gecontroleerd en niet gelezen of gewijzigd.
