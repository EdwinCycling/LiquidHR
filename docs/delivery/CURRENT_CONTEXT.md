...
Releasevoorbereiding: alle huidige slices staan op branch `agent/employee-dashboard-release` als commit `22af0f3` (`feat: release employee dashboard and reporting updates`) en zijn naar `https://github.com/EdwinCycling/LiquidHR.git` gepusht. De lokale releasegate is groen: 95 Vitest-bestanden/347 tests, ESLint, strict TypeScript, i18n-pariteit en productiebuild met 85 pagina's. De gekoppelde Vercel-preview `dpl_DQ8xRhJzKmymSKQybV5bfFoBRcmY` is `READY` op `https://liquidhr-git-agent-employee-dashboard-release-edwinitsolutions.vercel.app` en verwijst exact naar commit `22af0f3ddeb9421fe9ae9b79cd0dff0bad142809`. Productie volgt nog steeds `main`; deze featurebranch is niet naar productie gepromoveerd.
