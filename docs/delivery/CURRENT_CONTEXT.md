...
- Open RED-evidence: document upload faalt met een toegestaan TXT-bestand en een minimale PDF op `POST /api/employees/<employeeId>/documents` `400`, response-code `DOCUMENT_METADATA_FAILED`. De echte authenticated RPC-diagnose met TEST HR en `DEMO-035` geeft `42501`, message `new row violates row-level security policy for table "employee_documents"`, details/hint `null`. Repo en actieve TEST-database hebben dezelfde RPC-signature `create_employee_document_metadata(uuid, uuid, jsonb) -> uuid` en dezelfde `RETURNING id`-flow. Een authenticated `INSERT` zonder `RETURNING` slaagt; dezelfde insert met `RETURNING id` faalt omdat de SELECT-policy een audience vereist die pas ná de insert wordt aangemaakt. Dit is een `SOURCE_CODE_BUG` in de RPC/migration-definitie; reparatie vereist function-replacement/migration en is niet uitgevoerd. De diagnostic-key en twee eerdere orphaned `acceptance`-objects zijn verwijderd; DEMO-035 Storage-prefix eindigt met `0` orphans.
...
- Next action: expliciete toestemming voor TEST schema/function apply vragen; daarna echte document CRUD en de resterende absence/employment/authorization/process acceptance uitvoeren. Deze integration branch is niet release-ready en niet GREEN.
