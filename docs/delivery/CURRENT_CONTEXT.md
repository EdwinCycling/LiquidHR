...
## Journeys bouwstap 3 — actuele eindstatus
...
- Authenticated browserbewijs: HR Admin Journeylijst/detail, managerparticipant en buddy/participant op desktop en 390x844. Geslaagde routes hebben 0 console-errors; 390x844 meet `documentWidth = 390` en geen horizontale overflow. Een manager zonder concrete participatie kreeg eerder 404/0 console-errors. De gekoppelde devdata bevat geen `PREBOARDING`-Journey; preboardingbrowserbewijs blijft daarom data-open.
- De versie wordt volgens protocol verhoogd van `1.20260812.2` naar `1.20260812.3`. Geen push, merge of deployment.

## LiquidHR Journeys — bouwstap 3 lokaal in uitvoering 2026-08-12
...
- Feature-worktree: `C:\Users\Edwin\Documents\Apps\LiquidHR\.codex-worktrees\journeys-step-1`, branch `feature/journeys`. Dit blijft de enige featurebranch voor bouwstap 1, 2 en 3. De hoofdworkspace is ongewijzigd; er is niet gepusht, gemerged of gedeployed.
...
- Gekoppelde Supabase-testomgeving: `wnpfloqpjvaacobppbpk`. Stap 3 is remote toegepast via `20260812134246` (`journeys_step3_projection_and_outcomes`) en `20260812134832` (`optimize_journeys_step3_outcomes_foreign_keys`). De nieuwe outcomes-tabel en bestaande Journey-runtimetabellen hebben RLS; de publieke wrappers zijn invokerfuncties met alleen `authenticated` execute en de interne kernels zijn voor `anon` afgesloten. Officiële types zijn opnieuw gegenereerd. De performance-advisor meldt alleen ongebruikte Journey-indexen in de nog verse testdataset; de security-advisor meldt geen nieuwe Journey-bevindingen naast bestaande projectbrede waarschuwingen.
- Lokale verificatie: volledige hr-suite-test- en typecheck/lint/i18n/diff-check/build-gates zijn opnieuw groen. Geauthenticeerde HR Admin-, managerparticipant- en buddy/participantflows zijn via de bestaande Journey-route gecontroleerd op desktop en 390x844 met 0 console-errors en zonder horizontale overflow. Een manager zonder concrete Journey-participatie kreeg server-side 404 met 0 console-errors. De globale `DIRECT_MANAGER`-grant voor `journey-participation:*` is niet toegepast; preboardingbewijs blijft data-open omdat de devomgeving geen `PREBOARDING`-Journey bevat.
...
- Zichtbare appversie gaat van `1.20260812.2` naar `1.20260812.3`; er is geen push, merge of deployment uitgevoerd. De remote rolegrant voor directe managers is bewust niet toegepast.
