...
- Local verification: targeted review/security set `23 bestanden / 112 tests` groen, volledige HR-suite `403/404 bestanden` en `1606/1607 tests`; de enige default-suite failure is de bestaande PDF-render test met vaste 5s timeout, inhoudelijk groen met `--testTimeout=30000`. Strict TypeScript, ESLint, i18n parity `38 namespaces`, `git diff --check` en Webpack-productiebuild (`283` static pages) zijn groen. De standaard Turbopack-build blijft geblokkeerd door de bekende ongeldige worktree-`node_modules/next`-symlink.
