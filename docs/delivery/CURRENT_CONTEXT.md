...
- Aanvullende organogrambeslissing verwerkt: `/organization-chart` is read-only exploration; afdelingbeheer staat op `/departments`; alleen `Organisatiestructuur` en `Rapportagelijnen` zijn user-facing; oude `?view=job` valt terug naar `department`; graph connectors/chrome zijn vereenvoudigd met behoud van `@xyflow/react` en onderliggende talentdata.
