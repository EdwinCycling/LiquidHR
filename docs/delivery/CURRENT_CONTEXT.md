...
## QA Run 1 correction pre-flight — employee wizard — 2026-08-31

- Werk uitgevoerd in `.codex-worktrees/employee-create-wizard-qa-1` op `test/employee-create-wizard-qa-1`. `origin/main` is na `git fetch origin` exact `9151248f224fb62a2d18c558c2627e1078c2cf0a`; er is niet gemerged of gerebased.
- QA-EMP-002 is getraceerd van `employment-create-form.tsx` via `POST /api/employees/[employeeId]/employments`, `publishCompleteEmployment`, de twee complete-employment-RPC's en de `employment_contracts`-trigger. TEST had de oude hard-blocking trigger nog actief; de lokale warning-migration ontbrak remote.
- Historische migration `20260812110000_probation_rule_warnings.sql` is hersteld en byte-identiek aan `origin/main`. Nieuwe forward migration `20260831090324_probation_warning_trigger_alignment.sql` bevat alleen de triggerfunctie-alignering: `PROBATION_NOT_ALLOWED` en `PROBATION_MAXIMUM_EXCEEDED` zijn warnings; `PROBATION_DATE_INVALID`, `PROBATION_DATE_NOT_ALLOWED` en `PROBATION_DATE_OUTSIDE_CONTRACT` blijven structurele blockers. De bestaande structurele tabelconstraint blijft ongemoeid. De wizard, schema's en service gebruiken dezelfde classificatie. De trigger blijft `SECURITY INVOKER`, `SET search_path = ''`, met execute-rechten ingetrokken voor `public`, `anon` en `authenticated`.
- Toegevoegd: gerichte SQL-regressietest `apps/hr-suite/supabase/tests/employment_probation_warnings.sql`; deze is niet tegen TEST uitgevoerd. Gerichte Vitest: 39/39 groen; typecheck, lint (0 errors, 14 bestaande warnings), diff-check en productiebuild (229 pagina's) groen. Volledige suite: 290 bestanden groen, 1 bestaande Journey-fixturefailure.
- QA-EMP-001 blijft behouden. Employee 100015 (`f5294880-d34b-45a4-b5a1-fd9c19ead427`) heeft nog geen employment. Scenario 3 blijft open en gebruikt later uitsluitend de oorspronkelijk afgesproken waarden uit de QA-opdracht. Geen vierde employee, remote migration, remote seed/cleanup, merge, push of release uitgevoerd.
