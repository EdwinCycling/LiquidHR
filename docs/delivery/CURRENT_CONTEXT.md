...
- Root cause: canonical `readback` faalde eenmaal met `FIXTURE_UNEXPECTED_FAILURE: fetch failed` omdat `NEXT_PUBLIC_APP_URL` naar lokale poort `3107` wees zonder listener. DNS/Supabase-authsettings en read-only sign-in voor HR, Manager en Employee waren wel groen. Met de integratie-server op `127.0.0.1:3003` bereikte `setup` TEST, maar de bestaande vaste key `r5-test:claimed-work-item` gaf `HTTP 409 IDEMPOTENCY_KEY_REUSED`. Read-only inspectie bewees één fixture-eigen record in de juiste R5-definitie, tenant/hr-group, subject employee/employment en correlation-id; de inventory telde 5 definitions en 7 instances. De canonical cleanup faalde vervolgens bij de bestaande CLI-SQL-stap door ontbrekende `SUPABASE_ACCESS_TOKEN`; de MCP-rol mocht `session_replication_role` niet instellen en de daaropvolgende trigger-fout rolde de poging terug. Residual readback: 1 conflicterende key, 5 definitions en 7 instances. Geen setup/readback-herhaling; `migrations: NO`.
...
- Eindstatus: **R5 INTEGRATION ACCEPTANCE BLOCKED — EXISTING TEST FIXTURE STATE**. Er is geen aangetoonde R5-code-regressie en geen code gewijzigd. Geen main-merge, push, version bump, Vercel, production migration of branch cleanup. De aparte Journey-suitefailure blijft bekende baseline-debt en is niet gewijzigd. Hervatten vereist uitsluitend CLI-auth voor de voorgeschreven canonical cleanup; daarna setup en readback met `NEXT_PUBLIC_APP_URL=http://127.0.0.1:3003`.
