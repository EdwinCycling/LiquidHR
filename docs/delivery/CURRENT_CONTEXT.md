...
Payroll worktree `work/payroll-p0-p1` starts at main `4adceda`. P0 remains **GREEN** on DEV/TEST. P1 base commit `5d57323281b07bc2601c751db384ef3ad517ea6d` is pushed; local closure tip is `a127fdd6f95640253fecb6b26ab55d7d5904d0fd`. DEV migration history now includes `20260908203016 / payroll_p1_nmbrs_connection`, `20260909125131 / payroll_p1_advisor_indexes` and `20260909133103 / payroll_p1_private_rpc_facade`. The façade keeps `payroll_private` non-exposed: five `public` SECURITY DEFINER RPCs use `search_path=pg_catalog`, execute only for `service_role`, and are called only through the `server-only` Payroll persistence adapter. Exact schema, RLS/grants, policies, functions, types, advisors, tests and persona evidence are recorded in [`PAYROLL_P0_P1_EVIDENCE.md`](PAYROLL_P0_P1_EVIDENCE.md).

P1 development acceptance is **BLOCKED** only at the hosted/runtime gate: local targeted Payroll tests `15/15`, full suite `1394/1396` with the two pre-existing unrelated Document Studio failures, strict TypeScript, ESLint, i18n, diff-check and Webpack build are green; remote advisor readback has no Payroll unindexed-FK finding; and the façade replay/security readback is green. Codex in-app browser evidence shows the existing authenticated `hradmin.fixture@liquidhr.test` session on `/payroll` and all four views, without a password; canonical Manager/Employee direct fixture logins end at `/geen-toegang` with API `403`. The existing Vercel Production deployment `dpl_6evf2p7D398NyxbPYL7o8Ev8tEvr` still runs commit `5d5732…` and returns the old `PGRST106`-derived authorize `500`; no second deployment was triggered. A normal push of the closure commits is currently blocked by `SEC_E_NO_CREDENTIALS`. Decisions remain: PAYROLL is toggleable; existing TENANT_ADMIN/HR_ADMIN effective mapping governs administration; dedicated append-only payroll audit follows DG. No Production Supabase mutation or merge occurred, and protected `next-env.d.ts`, `.env.local`, and other worktrees remain untouched.
