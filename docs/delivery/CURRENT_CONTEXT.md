...
- **Root cause:** components/layout/sidebar.tsx en de gedeelde action-row-compositie waren gelijk aan pre-convergence origin/main, work/bugfix-login-context-20260924 en work/bugfix-startpage-layout-20260924; convergence had de actie niet verwijderd. De eerdere lokale browser gebruikte een productiebuild zonder VERCEL_ENV; de bestaande fail-closed isTestRoleSwitchEnabled-gate verbergt de actie dan. Op lokale next dev met de expliciete feature flag en het canonieke project verscheen het bestaande flask-icoon in dezelfde sidebar header/action row naast het cadeau-icoon. Edwin → Test HR Admin → Edwin werkte. Dit was een lokale runtime-gate mismatch, geen verwijderde sidebarcomponent.
- **Security fix:** dashboardweergave en POST /api/auth/test-role-switch vereisen nu beide bestaande TENANT_ADMIN of HR_ADMIN actieve rollen. Manager, Employee en niet-toegestane accounts krijgen in route-tests 403 voordat de admin magic-link client wordt aangemaakt. De bestaande target-allowlist, projectcontrole, originvalidatie en kortlevende HttpOnly/Secure/SameSite-cookie blijven behouden; geen rol of permission is toegevoegd. De codecomment beschrijft Vercel Production als deployment channel van de enige LiquidHR-omgeving.
- **Handmatige browseracceptatie:** /dashboard/start, Full, een werknemersdossier, /hr-calendar, /organization-chart en Focus/Full-navigatie zijn gecontroleerd. Edwin’s eigen Focus laadde; de HR Admin-fixture heeft zelf geen gekoppeld actief dienstverband. De bestaande ACT-AS-flow toonde de actor/subject-banner in Focus en Stop keerde terug naar Edwin. Alleen de bedoelde START/STOP-auditrecords zijn geschreven; er zijn geen HR-bedrijfsgegevens aangepast. Manager- en Employee-browseraanmeldingen zijn weggelaten volgens de instructie om die checks geautomatiseerd te houden.
- **Geautomatiseerde security:** Manager/Employee kunnen de rolwisselroute niet gebruiken om hogere rollen te verkrijgen. Nieuwe ACT-AS-regressietests controleren actor-, tenant-, HR-group-, rol- en capability-binding, scoped employee lookup en weigering van een onbeschikbare subject. Geen querystring-bypass, rolmutatie of RLS-wijziging is gebruikt.
- **Calendar:** de HR Admin-browser kreeg eerst SQLSTATE 57014 bij het laden van work-pattern-days voor alle actieve dienstverbanden. De service haalt nu eerst goedgekeurd verlof op en leest daarna werkpatronen/roosters alleen voor dienstverbanden met overlappend goedgekeurd verlof in de zichtbare maand, met tenant-, administration- en datumfilters. De live kalender laadde; er zijn geen policies of schema gewijzigd.
- **Gates:** volledige hr-suite-run groen: 460 bestanden / 1827 tests. Een eerdere gecombineerde root-workspace-run had één timeout na 5 seconden op de ongewijzigde PDF-render-test (459/460, 1826/1827); Control slaagde met 2/2 bestanden / 7/7 tests. De PDF-test slaagde daarna geïsoleerd in 2.41s en de daaropvolgende volledige hr-suite-run slaagde met 460/460 bestanden en 1827/1827 tests; Edwin accepteert de timeout daarom als niet-blockerend. Post-merge regressies slaagden met 9/9 bestanden en 44/44 tests; versie-regressie met 2/2. Workspace strict TypeScript, changed-file ESLint, hr-suite i18n (39 NL/EN-namespaces), Control i18n (147 sleutels), git diff --check en builds slaagden. Next.js en eslint-config-next zijn 16.3.6; builds genereerden 296 hr-suite-routes en 12 Control-routes. next-env.d.ts is byte-identiek aan SHA256 1B59D4C6B83807DB275D43F3CF2CC8E9323F465FAB764EEA091CC5BECD5BAD37. .env.local is alleen op bestaan gecontroleerd; waarden zijn niet gelezen.
- **Responsive gate:** Edwin bevestigde handmatige Chrome Device Mode acceptance op exact 390×844 voor Startpage, Focus, document/dossier, calendar en org chart. Geen blocking overlap, verdwenen essentiële controls of onbruikbare responsive layout gevonden.
- **Dependency-audit:** `npm audit --omit=dev` rapporteert 0 kwetsbaarheden. De volledige workspace-audit meldt 26 development-dependencybevindingen (24 HIGH, 2 MODERATE, 0 CRITICAL), in ESLint-, TypeScript-, browserslist-, brace-expansion-, js-yaml- en Vitest-toolingpaden. Er is geen `npm audit fix` uitgevoerd.
- **Database readback:** het enige Supabase-project wnpfloqpjvaacobppbpk is ACTIVE_HEALTHY, met 496 migraties (laatste 20260925092114 allow_scoped_document_expiry_reminders). RLS staat aan met bestaande policies op employment_schedules (5), employment_work_pattern_days (5), employment_work_patterns (5) en leave_requests (2). Security Advisor: 11 RLS/no-policy INFO, 4 anonieme en 103 authenticated SECURITY DEFINER WARN en 1 leaked-password-protection WARN. Performance Advisor: 144 unindexed-FK INFO, 395 unused-index INFO en 33 multiple-permissive-policy WARN. Dit zijn projectbrede bevindingen; geen migration, policy, grant of schemawijziging is uitgevoerd.
- De bestaande ABSENCE-nullabilityfixes blijven staan; er is geen ABS01-functionaliteit toegevoegd. App-versie is `1.20260925.1`. Er is één LiquidHR-omgeving: Supabase `wnpfloqpjvaacobppbpk` met het bestaande Vercel-project `liquidhr`. De uiteindelijke Vercel Production deployment is `READY`, heeft beide bestaande aliases en is teruggelezen op exact de definitieve main-SHA. Runtime-smoke slaagde op `/login`, `/dashboard/start`, `/focus`, `/hr-calendar`, `/employees`, `/settings` en een synthetische dossierroute; anonieme beveiligde requests werden naar `/login` gematcht. De deployment-logcontrole vond geen error/fatal events.
- **Cleanup:** remote refs voor D01, login, calendar, OAuth, org chart, startpage en convergence zijn verwijderd; lokale branchrefs D01 en startpage zijn verwijderd. Vijf lokale branch/worktree-checkouts blijven na auto-review weigering van `git worktree remove --force`: die actie zou ignored/untracked inhoud wissen, waaronder worktree `.env.local`-kopieën. De D01-worktree-registratie is losgekoppeld, maar de artifactdirectory blijft staan zolang `agent-browser-win32-x64.exe` PID `13876` deze gebruikt. De canonieke `.env.local` bestaat; inhoud is nooit gelezen. Er is geen veilig bewijsbare obsolete code gevonden en geen code-cleanup uitgevoerd. ABS01 is niet gestart; `ABS01_BASELINE_SHA` staat in het afrondingsrapport.

## D01 Dossier/Documents acceptance — 2026-09-24 to 2026-09-25

**Status: D01 ACCEPTANCE GREEN in de oorspronkelijke acceptatierun.** Die run rapporteerde twee ABSENCE-nullabilityfouten als pre-existing. De latere convergencecontrole stelde vast dat een koude strict typecheck op de toenmalige `origin/main` slaagde en dat de samengevoegde D01 database-types de optionele RPC-argumenten aanscherpten; de convergence-kandidaat bevat een minimale typeveilige compatibiliteitsfix.
...
De consolidatie is geïntegreerd in GitHub `main` via commit
...
de geverifieerde repository-identiteit. Hosted acceptatie staat pending; deze
retry wordt uitgevoerd met repository-local `user.name=Edwin` en
...
- Eric Oproeper is uitsluitend via de bestaande Employee + Employment-wizard
  aangemaakt in canonical DEV `wnpfloqpjvaacobppbpk`: employee-id
  `088e7ee3-e08e-4372-adaa-560338c397ef`, employment-id
  `c1c70315-446f-4975-85bd-f6026457af61`, wizard-generated employee number
  `100021`. Er is geen auth/login-account voor Eric aangemaakt en Production is
  niet gebruikt.
- Wizard readback is groen: Jupiter BV, Employee, primary, 2026-07-01 t/m
  2027-06-30, definite contract via de bestaande +12-maandenactie, on-call en
  on-call obligation `Ja`, 20/40 uur en factor `50%`, job `Monteur`, afdeling
  `Directie`, manager `Lisa Test`. De schedule is `HOURS_AND_AVG_DAYS` met
  gemiddeld 3 dagen per week; er is geen vaste maandag-vrijdagverdeling
  opgeslagen en salaris is overgeslagen.
- Tijdens de wizardflow zijn drie in-scope defecten minimaal hersteld: on-call
  mocht uren/factor niet resetten, payroll overslaan mocht contract/schedule/
  organization niet weggooien en de gekozen manager mocht niet door een
  department-default worden overschreven. Gerichte wizard/state/schema-tests
  dekken deze paden af (`29/29`).
- Via de echte Eric Actual Work-UI zijn `AW_WORK` 8, `AW_ADDITIONAL` 2 en
  `AW_OVERTIME` 2 uur opgeslagen in september 2026; persisted readback toont
  de drie regels en totaal `12,0000` uur. De transparante types zijn niet
  gebruikt.
- De read-only Leave-accrual preview voor 2026 toont voor Eric op
  `2026-09-01 – 2026-10-01` `WORKED_HOURS · MONTHLY`, status `Gereed` en
  `1,00` uur. Met de bestaande rate `0,08333333` is dit
  `12,0000 × 0,08333333 ≈ 1,0000`; een tweede toepassing van FTE `0,5000`
  zou circa `0,5000` geven en is dus niet toegepast. Er is niet geboekt en geen
  jaar afgesloten.
- Gate-readback vóór integratie: strict TypeScript, volledige lint,
  i18n-pariteit (`35` namespaces), `git diff --check`, gerichte
  wizard/schema-tests (`29/29`) en hr-suite Webpack-productiebuild (`270/270`)
  zijn groen. De volledige hr-suite heeft `1480` geslaagde tests met dezelfde
  drie onaangeraakte baseline failures (contract-change-audit migration
  contract, Document Studio DM-1 migration contract en PDF-render-timeout);
  control is `7/7` groen. De root Turbopack-build blijft geblokkeerd door de
  bekende externe `apps/hr-suite/node_modules`-symlink; de gevalideerde
  Webpack-fallback is gebruikt.
...
- Extern evidence-rapport: `C:\Users\Edwin\Documents\Apps\LiquidHR-Test-Evidence\security-review-20260830\SECURITY-WAVE-A.md`. Vercel Production is succesvol op de exacte finale main-SHA; HR Admin smoke, Secure-cookie metadata, disabled role-switch endpoint en canonical hostile-host redirect zijn bevestigd. Open blijven alleen het SEC-004 HttpOnly-sessionbridgebesluit, hostpolicy-review en de TEST-fixturepolicy.
...
- Zichtbare versie: `1.20260829.2`; releasecommit `9d4ba572975251b3182c8f33c7c64de25aad296f` is geïntegreerd en `main == origin/main` (docs-sync commits volgen in `bc9ced007304e75252ba366bb6647dc71619b718` en de laatste contextcorrectie). Vercel production is `READY` met GitHub-SHA gelijk aan `origin/main`. Tijdens de alias-overgang was één verse check tijdelijk geblokkeerd door TEST-auth/Supabase-state `PGRST303: JWT issued at future` bij `GET /dashboard/start`; na settling bevestigde een verse retry de productie-Startpagina met HTTP `200`, hard refresh, `0` page/console errors en `0` warnings. Geen in-scope authwijziging. Dirty root-worktree bleef onaangeraakt. `TALENT-1` blijft geparkeerd; AI Foundation is niet gestart.
