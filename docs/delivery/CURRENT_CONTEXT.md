...
- Verificatie groen: v1.2/reminder gericht 11/11; Reminder real CRUD CREATE 201, PUBLISH 200, PATCH 200 en DELETE 200; dirty-close GREEN; LinkedHR GREEN; volledige hr-suite 209 testbestanden/824 tests; strict TypeScript; i18n 33 gelijke NL/EN-namespaces; lint exit 0 met 6 bestaande warnings buiten deze slice; productiebuild 225 routes; `git diff --check`.
- Test acceptance matrix is actief en bevat nu ook de canonical fixture-auth preflight. `fixtures:talent-auth` laadt `.env.local` expliciet. In deze run zijn `TALENT_HR_ADMIN_PASSWORD`, `TALENT_MANAGER_PASSWORD` en `TALENT_EMPLOYEE_PASSWORD` alle `MISSING`; de bestaande Playwright-session stond op `/login`, dus authenticated 390px acceptance blijft `BLOCKED BY ENVIRONMENT` zonder productdefectclaim.
- Open: canonical fixture-passwords beschikbaar maken in de lokale test-env en daarna in deze worktree de echte TEST HR 390×844 Reminder acceptance uitvoeren; niet naar `main` mergen. Bestaande wijziging in `apps/hr-suite/next-env.d.ts` blijft buiten scope.
