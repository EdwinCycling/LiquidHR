...
## Performance-optimalisatie gepubliceerd naar preview 2026-08-07: startpagina en medewerkerslijst

Op `feature/performance-startpagina-medewerkerslijst` is de request-keten van `/dashboard/start` en `/employees` geoptimaliseerd. Layout en pagina's hergebruiken één request-scoped autorisatiecontext, actieve context en Supabase-client. Onafhankelijke vertaal-, voorkeur-, directory- en databasereads lopen parallel. De startpagina gebruikt voor de medewerkerstelling twee smalle parallelle reads; de medewerkerslijst deelt de directe teamscope-query met de employee-overview-read. Dit verlaagt dubbele auth/client-setup en waterfall-latency zonder de autorisatiegrenzen te wijzigen.

Er is geen database-DDL toegevoegd: de relevante indexes zijn aanwezig en de live query/advisorcontrole gaf geen aanwijzing dat een nieuwe index voor deze routes de hoofdwinst oplevert. De lokale checks zijn groen: 130 testbestanden/481 tests, strict TypeScript, lint, 28 gelijke NL/EN-namespaces, productiebuild met 173 pagina's en `git diff --check`. Commit `3fcfe7068c4ea796a1544576cdadaa218a959be5` is naar GitHub gepusht. De gekoppelde Vercel-preview `liquidhr-rgbpt20os-edwinitsolutions.vercel.app` is `READY` als deployment `dpl_9H348CBcxUkKh8PuEYmCekd7kgdq`; de build duurde 39 seconden en de runtime error/fatal-logscan is leeg.

Authenticated Chrome after-change meting, drie runs per route: HR Admin startpagina mediaan 1.014 -> 916 ms en medewerkerslijst 886 -> 835 ms; Manager startpagina 2.076 -> 979 ms en medewerkerslijst 1.088 -> 973 ms; Employee medewerkerslijst 938 -> 807 ms. De Employee-rol rendert de startpagina niet als dezelfde route maar landt in de eigen medewerkercontext. Dit is een richtinggevende wall-clockvergelijking met normale cold/warm- en netwerkvariatie. Vercel Production blijft op deployment `dpl_5yXSL5Q6ykSaoaLfRP21X3ehpVuK` / commit `445d91c` / versie `1.20260806.2`; merge naar `main` en productiepromotie zijn nog open.

## Performance vervolg uitgevoerd 2026-08-07: meetlaag, startpagina en medewerkerdashboard
...
Lokale gate: 130 testbestanden/481 tests, strict TypeScript, volledige lint, i18n-pariteit met 28 namespaces, productiebuild met 173 routes en `git diff --check` zijn groen. Supabase is read-only gecontroleerd op project `wnpfloqpjvaacobppbpk`: relevante indexes bestaan en de performance-advisor geeft 347 INFO/0 WARN; er is geen database-DDL, RLS- of typewijziging nodig. GitHub-branch `feature/performance-startpagina-medewerkerslijst` bevat de commit. Vercel-preview `dpl_7yQdNXbyww1gh2pkHhnVvJnmzqA6` (`liquidhr-4iossnx68-edwinitsolutions.vercel.app`) staat op READY, exact op deze commit; build- en nieuwe preview runtime error/fatal-scans zijn leeg.

Authenticated Chrome: HR Admin startpagina preview mediaan 1154 ms (runs 2441/1154/952), Production 933 ms (1079/933/822); HR Admin medewerkerdashboard preview 1054 ms (1228/1054/865), Production 1154 ms (1154/1089/1144); de medewerkerslijst als smoke-test preview 840 ms (985/840/838), Production 851 ms (990/766/851). De Manager- en Medewerker-rolwissel op de preview is uitgevoerd en gaf 0 console-errors/warnings. De productie-logscan bevatte daarnaast sessiegebonden authregels (`JWT issued at future`/ongeldige refresh token) uit de vergelijking; een verse productie-dashboardtab had 0 browserconsole-errors. De wall-clockvergelijking blijft gevoelig voor cold/warm-start en netwerkvariatie. `main` en Production zijn niet gewijzigd.
