...
- Zichtbare versie: `1.20260829.2`; releasecommit `9d4ba572975251b3182c8f33c7c64de25aad296f` is geïntegreerd en `main == origin/main` (de daaropvolgende docs-sync staat in `bc9ced007304e75252ba366bb6647dc71619b718`). Vercel production deployment `dpl_HfbHMzMqmivTMBjGunqYMHewcbmT` is `READY` met exact de actuele docs-sync-SHA. De productcode-deployment `dpl_4Gwo6VZyvncQdhbrbkyRyZptWkUq` was browsergroen; de verse check op de actuele alias is nu geblokkeerd door externe TEST-auth/Supabase-state `PGRST303: JWT issued at future` bij `GET /dashboard/start`, zonder in-scope authwijziging. Dirty root-worktree bleef onaangeraakt. `TALENT-1` blijft geparkeerd; AI Foundation is niet gestart.
