...
## LiquidHR Journeys — bouwstap 1 afgerond 2026-08-12
...
- Stap 1 bevat alleen HR-configuratie: zeven tabellen, negen permissions, RLS/grants/audit, moduleactivatie, diepe service/API-laag, lijst-eerst catalogus en full-page Template Designer met immutable publicatie. Runtime-Journeys en alles uit stap 2/3 ontbreken bewust.
- Gekoppelde Supabase-testomgeving: project `wnpfloqpjvaacobppbpk`. Migraties `20260812081325_journeys_step1_foundation` en `20260812100445_optimize_journeys_step1_foreign_keys` zijn gecontroleerd toegepast en geregistreerd. Alle zeven tabellen hebben RLS; `anon` heeft geen tabelrechten, `authenticated` alleen `SELECT`, publieke wrappers zijn alleen voor `authenticated` uitvoerbaar en interne kernels blijven afgesloten. Remote pgTAP is 18/18 groen. Advisors tonen geen Journeys-securitybevinding en na indexering geen ontbrekende foreign-key-index; alleen de verwachte unused-index-info voor negen nieuwe indexen blijft zichtbaar. Officiële types zijn gegenereerd.
- Lokale verificatie: 19/19 gerichte tests; 622/622 volledige tests; strict typecheck; volledige lint; 32 gelijke NL/EN-namespaces; diff-check; Webpack-build met 202 pagina's tegen de gekoppelde testomgeving. De geauthenticeerde HR-adminbrowserroute `/settings/journeys` toont alle vier remote scenario's.
...
- De zichtbare appversie is verhoogd van `1.20260811.1` naar `1.20260812.1`. Open voor stap 2: activatiepreview/-command, gepinde runtime-instance, participantresolutie en -vervanging, HR-livecatalogus/detail, pause/resume/cancel, reminderadapter en bijbehorende HR-browser/RLS-matrix.
