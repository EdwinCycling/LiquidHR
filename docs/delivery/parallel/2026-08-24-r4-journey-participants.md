...
- Root cause: `transition_journey_internal` raised the expected optimistic version conflict as PostgreSQL `40001` (`serialization_failure`). The Supabase Data API treats transient failures as retryable, so the stale business conflict was retried until the generic 500. Forward migration `20260825134000_fix_journey_version_conflict_retry.sql` changes only this lifecycle conflict to non-retryable `P0001`; its SQL is the exact canonical match for the applied TEST registration.
