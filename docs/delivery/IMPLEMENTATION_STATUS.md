...
| Follow-up full suite | GREEN for changed scope / pre-existing unrelated timeout | `414/415` testbestanden en `1658/1659` tests passed; only unchanged `lib/document-generation/pdf.test.ts` timed out at the existing 5-second limit. Focus follow-up tests, strict TypeScript, ESLint and Webpack `289/289` are green. |
...
| Preview navigation UX | GREEN (code/tests) | Read-only nav and quick actions use preview-scoped `?section=` routes; no external employee-dossier links are emitted, and the mobile nav uses the existing hidden-scrollbar hook. Follow-up: 2 files / 10 tests, strict TypeScript, ESLint and Webpack 289/289 green. |
