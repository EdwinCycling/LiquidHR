...
Vanaf exact authoritative `origin/main` SHA `d5dec456db5ced5d47e1ff052ebe738ffbc7aa87` is in dedicated worktree `C:\Users\Edwin\Documents\Apps\LiquidHR\.codex-worktrees\ai-usage-insights-v1` op branch `work/ai-usage-insights-v1` een HR Admin-facing `/insights?report=ai-usage` toegevoegd. De report gebruikt de bestaande canonical `ai_invocations`-read seam en `get_ai_group_credit_balance`-RPC via de server-side service-role read boundary, scoped vanuit `requirePermission('ai:usage-read')`, de actieve tenant en HR-groep. Querydata kiest nooit tenant, actor, gebruiker of administratie buiten de authenticated context.

V1 geeft alleen aggregate operationele metadata door: canonical actuele balance, charged/settled credits, ontvangen invocations, success rate, daily trend en feature/quality/status-breakdowns. `AI requests` betekent `created_at` binnen het geselecteerde HR-group-local period window; `credits used` betekent `charged_credits` van invocations met `finished_at` in dat window; `success rate` is `SUCCEEDED / (SUCCEEDED + FAILED)` onder afgeronde invocations en sluit `REJECTED` en incomplete states uit. Vrijgegeven reserveringen tellen niet als verbruik. Bekende `improve_text` en huidige `improve-existing-hr-text` vallen generiek onder de customer-facing capability `AI verbeteren`/`AI Improve`; onbekende codes vallen veilig terug op `Overige`/`Other` zonder interne code in de UI.

Privacy is begrensd tot aggregates: geen prompts, input/output, employee-/actor-/business-objectidentifiers, provider/model/request-id, raw failuretext of HR-inhoud in DTO of UI. Er is geen Employee Notes-koppeling, shadow counter, analytics ledger of realtime/polling toegevoegd. NL/EN hebben gelijke keys; de UI gebruikt Foundation `Surface`, `Badge`, `EmptyState`, `SectionHeader` en `DropdownSelect`, met daily SVG-trend en scrollbare tabellen voor smalle viewports.

Evidence: AI Usage targeted `6` testbestanden / `20/20` tests groen; relevante Insights-set `29` bestanden / `97/97` tests groen; strict TypeScript groen en ESLint `0 errors / 14 bestaande warnings`; i18n `33` gelijke NL/EN-namespaces; `git diff --check` groen; `next build --webpack` compileerde succesvol en bevat de nieuwe `/api/insights/ai-usage`-route. De unauthenticated browsercheck op de lokale production candidate gaf `/insights?report=ai-usage` correct `/login` en directe API-toegang `401`; authenticated HR Admin desktop/mobile acceptance kon niet worden uitgevoerd omdat geen sessie beschikbaar was. Componenttests dekken KPIs, trend, breakdowns, empty state en privacy. Geen remote TEST migration of andere databasewrite is uitgevoerd. Geen AI Foundation blocker is ontdekt.

Exacte delta: `apps/hr-suite/app/(dashboard)/insights/page.tsx`, `apps/hr-suite/components/insights/insights-workspace.tsx`, `apps/hr-suite/components/insights/ai-usage-report.tsx`, `apps/hr-suite/components/insights/ai-usage-report.test.tsx`, `apps/hr-suite/app/api/insights/ai-usage/route.ts`, `apps/hr-suite/app/api/insights/ai-usage/route.test.ts`, `apps/hr-suite/lib/insights/ai-usage-query.ts`, `apps/hr-suite/lib/insights/ai-usage-query.test.ts`, `apps/hr-suite/lib/insights/ai-usage-report.ts`, `apps/hr-suite/lib/insights/ai-usage-report.test.ts`, `apps/hr-suite/lib/insights/ai-usage-types.ts`, `apps/hr-suite/lib/insights/report-catalog.ts`, `apps/hr-suite/lib/insights/report-catalog.test.ts`, `apps/hr-suite/lib/insights/query-seam.ts`, `apps/hr-suite/lib/insights/query-seam.test.ts`, `apps/hr-suite/messages/nl/insights.json`, `apps/hr-suite/messages/en/insights.json`, `docs/README.md`, `docs/delivery/IMPLEMENTATION_STATUS.md` en `docs/delivery/CURRENT_CONTEXT.md`. Er is geen lokale commit gemaakt; de parallelle AI Improve-worktree is niet gewijzigd.
