...
- Candidate: `C:\Users\Edwin\Documents\Apps\LiquidHR\.codex-worktrees\focus-completion-act-as-20260918`, branch `work/focus-completion-act-as-20260918`, exact base `3bd31746ff40dbfcc2de54f8957379f3a8f23d61`.
- Scope: Focus now exposes native mobile-first routes for home, leave, hours, requests, work, team, profile, documents, directory, onboarding and more. The fixed bottom-right quick-action control includes leave, employee sickness for managers and a disabled expense action. Focus reuses the canonical Leave, Actual Work, Process Automation, Directory, Documents and Absence boundaries; it does not create a second workflow or ledger.
- Privacy and act-as: profile and directory projections remain server-side and safe for employee use. Act-as is short-lived, audited and subject-based; the UI is read-only until the authenticated subject/permission and DEV schema gate are available. Sickness is date-only and excludes medical details.
- Verification: full HR Vitest `415/415 bestanden, 1.658/1.658 tests`, strict TypeScript, ESLint, i18n parity (`39 namespaces`), `git diff --check`, and Next/Turbopack production build (`294/294 static pages`) are green. Authenticated local browser checks on port `3001` covered the Focus route matrix, mobile navigation, team selected-day cards, empty hours state and fixed quick-action positioning. Exact act-as/absence write acceptance is still pending the DEV migration.
- SQL/remote gate: migration `20260918203815_focus_completion_act_as_absence.sql` is pending. `supabase db push --dry-run --project-ref wnpfloqpjvaacobppbk` fails before mutation with `LegacyDbPushMissingLocalError` due to existing remote migration versions missing locally. Do not repair history or run `db pull` without explicit direction; no DEV/Production remote mutation, main merge or deployment was performed.
