...
## LiquidHR Journeys 2026-08-12 — BOUWSTAP 1 EN 2 AFGEROND
...
Gebouwd zijn de configuratieslice uit stap 1 en de HR-runtimeslice uit stap 2: acht nieuwe runtime-tabellen met gepinde template-/datum-/participant-snapshots; write-free activatiepreview; automatische en handmatige teamresolutie; idempotente activatie; HR Live-lijst, detail en nieuwe Journey-wizard; pause/resume/cancel/complete; future-only participantvervanging met historie; audit; en een adapter naar bestaande reminders. Routes/services blijven de schrijfgrens. Self/preboarding, participantprojecties, topicoutcomes en widgets blijven bewust stap 3.

Bewijs: project `wnpfloqpjvaacobppbpk` bevat zes geordende Journey-migraties tot `20260812143500`. Alle 15 Journey-tabellen hebben RLS; op de acht runtime-tabellen heeft `anon` niets en `authenticated` alleen `SELECT`; de drie publieke runtimewrappers zijn security invoker en interne kernels controleren module, groep en `journey:write`. Remote pgTAP is 18/18 plus 12/12 groen. Security/performance-advisors tonen geen actionable Journey-bevinding; nieuwe indexen hebben alleen verwachte unused-index-info. Officiële remote typen zijn gegenereerd en met het nog lokale `company_activities`-contract behouden. Lokaal: 164/164 bestanden en 633/633 Vitest-tests, strict TypeScript via de Webpack-build, volledige ESLint, 32 NL/EN-namespaces, diff-check en 208 gebouwde pagina's groen. De geauthenticeerde HR-adminbrowserflow verifieert lijst, replacementhistorie, blokkerende preview en 390px zonder overflow of console-/serverfout. Zichtbare versie `1.20260812.2`; geen push, merge of deployment.
