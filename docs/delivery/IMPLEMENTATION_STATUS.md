...
- Hr-suite volledig groen: 460 bestanden / 1827 tests. Een gecombineerde workspace-run had eerder één fixed-5s timeout op de ongewijzigde PDF-test; de geïsoleerde PDF-test en daaropvolgende volledige hr-suite-run slaagden, zoals Edwin accepteert. Control slaagde met 2/2 bestanden / 7/7. Post-merge regressies: 9/9 bestanden en 44/44 tests; versie-regressie 2/2. Strict TypeScript, changed-file ESLint, NL/EN i18n (39 namespaces, 147 Control-sleutels), diff check en builds met Next.js 16.3.6 (296 hr-suite-routes, 12 Control-routes) slaagden.
...
- De enige LiquidHR-omgeving gebruikt Supabase `wnpfloqpjvaacobppbpk` en het bestaande Vercel-project `liquidhr`. Read-only database readback vond 496 migraties en ingeschakelde RLS met policies op calendar schedule/pattern/leave-tabellen; er was geen schema/policy-write. De deployment is `READY`, target `production`, heeft beide bestaande aliases en is teruggelezen op exact main-SHA. Runtime-smoke op `/login`, `/dashboard/start`, `/focus`, `/hr-calendar`, `/employees`, `/settings` en een synthetische dossierroute slaagde; anonieme beveiligde routes werden naar `/login` gematcht. Vercel error/fatal-logcontrole vond geen events. Runtime dependency-audit: 0 kwetsbaarheden; volledige audit: 26 development-toolingbevindingen (24 HIGH, 2 MODERATE, 0 CRITICAL).
- Cleanupstatus: remote refs van de zeven geïntegreerde tijdelijke branches verwijderd; lokale D01- en startpage-branchrefs verwijderd. Vijf lokale branch/worktree-checkouts blijven na auto-review weigering van forced removal omdat die ignored/untracked bestanden zou wissen, waaronder worktree `.env.local`-kopieën. De D01-artifactdirectory blijft staan zolang agent-browser PID `13876` het executable gebruikt. De canonieke `.env.local` bestaat en is buiten cleanup gehouden. Er is geen veilig bewijsbare obsolete code gevonden; er is geen code-cleanup uitgevoerd. ABS01 is niet gestart; `ABS01_BASELINE_SHA` is de definitieve origin/main SHA uit het afrondingsrapport.
...
De actuele authoritative baseline is `origin/main` `fad1a115b496c1d4e0c211953930b272dde22e4c`, met zichtbare versie `1.20260831.2`. De candidate verwijdert geen productfeatures en wijzigt geen Production- of remote-database-state. De finale gate is groen: full suite hr-suite `309/309` bestanden en `1201/1201` tests, control `2/2` en `7/7`, strict TypeScript, ESLint `0/0`, i18n `33` gelijke namespaces, diff-check en Webpack (`233/233` en `12/12`). De oude Journey- en lintkwalificaties in historische secties hieronder zijn release-evidence uit eerdere baselines; zij zijn niet langer geldige uitzonderingen voor deze gate. Actuele migration/typegen-drift staat in [`MIGRATION_HISTORY_DRIFT.md`](MIGRATION_HISTORY_DRIFT.md) en [`SUPABASE_TYPEGEN_DRIFT.md`](SUPABASE_TYPEGEN_DRIFT.md).

## Final consolidation release candidate — 2026-08-31

**Status: GREEN — RELEASED**

De releasecandidate is lokaal opgebouwd vanaf exact `origin/main` `8b080b06993e9de290d2756e6bef1c93f5a6095d` in worktree `release/final-consolidation-20260831`. AN-4/5 komt uit accepted candidate `f6a5844`; SEC-001/002 zijn uitsluitend uit `3265b3d1faac0049c331aa951dfa7f11adbbccb9` overgenomen. De Employee Wizard QA-branch en alle probation/salary/wizard/fixture/`getUser`-artefacts zijn niet geïntegreerd. SEC-003/004/005 uit Security Wave A zijn behouden. Candidate `774d233e8f3a9f038850951d1c7ba3ff823e7f67` is non-force gepubliceerd naar `origin/main`; Vercel Production deployment `dpl_86JsxCCa1gFL9bKUWMU2qYQ83oAP` is `READY` op die SHA. Zichtbare versie: `1.20260831.2`.

De targeted releasechecks zijn `25` bestanden / `90/90` tests, strict TypeScript, i18n `33` gelijke namespaces, ESLint `0` errors / `14` bestaande warnings, diff-check exit 0 en Webpack `233/233` routes. De full suite is eenmaal uitgevoerd als `307/309` bestanden en `1199/1201` tests; alleen de bekende ongewijzigde Journey-baseline bleef rood. De versieassertie is na de bump gericht groen gecontroleerd.

Read-only production preflight: Supabase-project `wnpfloqpjvaacobppbpk` (`LiquidHR`) heeft `public.saved_analysis_definitions` al en registreert de AN-migratie als `20260831093310 / 20260830143757_saved_analysis_definitions`. `mcp__codex_apps__supabase_apply_migration` is de beschikbare named single-migrationmechaniek, maar is niet gebruikt omdat de migratie al geregistreerd is. Production smoke is groen: HR Admin login/start, zichtbare versie `1.20260831.2`, exact vier Analysis-states, role-switch `404`, missing employee `404`, en `0` console/page-errors. Er is geen production apply, `db push`, `--include-all` of repair/pull gebruikt; root bleef onaangeraakt.
...
`TYPEGEN SYNC REQUIRED`: officiële remote typegen bevat nu `saved_analysis_definitions`, maar brede bestaande remote/local drift wordt niet als volledige `packages/db/types.ts`-diff vastgelegd. De AN-4/5 code gebruikt de voorbereide narrow typed persistence seam.

Vanaf actuele `origin/main` `9151248f224fb62a2d18c558c2627e1078c2cf0a` is de slice geïntegreerd in dedicated worktree `work/an4-an5-my-analyses-explore-v1` zonder mergeconflict; de candidate HEAD is `6bf9e11b88d258031a129e63d8cfe95933e60627` vóór deze documentatie-update. De zichtbare versie blijft `1.20260830.2`. De hub heeft exact vier opties en activeert alleen de bestaande guided Explore- en Mijn Analyses-paden naast Rapporten. De server bewaart uitsluitend een strikt gevalideerde, versioned `AnalysisSpec`; scope wordt server-side afgeleid uit de authenticated actieve context. De database valideert de gesloten AnalysisSpec V1 opnieuw, terwijl directe authenticated tabeltoegang is ingetrokken omdat RLS de cookie-context niet kan zien. Analyse-uitvoering blijft fresh via de bestaande engine en `LiquidCanvas`; geen snapshots, result cache, employee rows, AI of conversational flow.

De lokale migration `apps/hr-suite/supabase/migrations/20260830143757_saved_analysis_definitions.sql` maakt `public.saved_analysis_definitions` met tenant/HR-group/owner-scope, naam, definition version, de gesloten negen-key JSONB `analysis_spec`-validator, timestamps, index, immutable identity trigger, defense-in-depth RLS en service-role grants voor de bestaande server-only seam. Exact deze SQL is eenmaal toegepast op TEST via `mcp__codex_apps__supabase_apply_migration` voor project `wnpfloqpjvaacobppbpk`; remote registreerde de migration als `20260831093310 / saved_analysis_definitions`. De gecombineerde AN-4/5 + AN-2/3 set is `23` bestanden / `80/80` tests groen; `check:i18n` meldt `33` gelijke NL/EN-namespaces, strict TypeScript, lint, diff-check en Webpack `233/233` zijn groen. De catalogus/security-test is groen en de transactionele pgTAP/RLS-test is `29/29` groen; alle testinserts zijn teruggerold en remote bevat `0` saved-analysis-rows.
De DB-validator vereist expliciet alle negen canonieke top-level keys, waaronder de verplichte nullable `sort`, en weigert unknown keys; de oude foutieve `jsonb_object_keys(candidate) <> 10`-check is verwijderd. Remote bevat `408` unieke migratieversies; de bestaande set-drift (`313` remote-only, `292` local-only) is niet gerepareerd. Readback bevestigt kolommen, restrict-FK's, checks, index, RLS/policies, grants, validator en beide triggers. Advisors tonen geen AN-securityfinding, maar wel twee AN-gerelateerde performance-INFO's plus bestaande baselinebevindingen. Officiële typegen bevat het nieuwe tabletype, maar brede drift vereist `TYPEGEN SYNC REQUIRED`; geen brede types-diff is gecommit. Authenticated browseracceptance is groen: HR Admin login/context `Planeten`, exacte vier hub-states, normale create/open/re-execute/delete-flow, en Manager owner-GET/PATCH/DELETE elk `404 SAVED_ANALYSIS_NOT_FOUND`. Fake tenant/group-queryparameters konden de servercontext niet overrulen. Persistence readback was configuration-only zonder employee/result-data; cleanup liet `0` rows achter. Full suite is niet uitgevoerd.

Exact één named apply-mechanisme is gebruikt: `mcp__codex_apps__supabase_apply_migration` voor project `wnpfloqpjvaacobppbpk`, naam `20260830143757_saved_analysis_definitions` en de volledige lokaal gelijk bevonden SQL-payload. Er is geen `db push`, `--include-all`, repair, pull of handmatige history-edit gebruikt. Zie [`requirements/reports/LIQUID_ANALYSE_AN4_AN5.md`](../requirements/reports/LIQUID_ANALYSE_AN4_AN5.md) en [`delivery/AN4_5_MY_ANALYSES_EXPLORE_V1.md`](AN4_5_MY_ANALYSES_EXPLORE_V1.md) voor contract en evidence.
...
De verlofconfiguratie heeft nu een vierdelige workspace met profielbeheer, medewerkersetbeheer en jaarafsluiting als afzonderlijke tabs. Profielen tonen gekoppelde regels, actieve sets en directe toewijzingen; de UI legt de precedence uit en linkt door naar profielgebonden opbouwregels. De migration `20260912120000_leave_profile_default_management.sql` bewaakt atomic default switching, case-insensitive profielnamen, archiveringsguards en actieve-profielresolutie.

De migration is na expliciete gebruikersautorisatie op DEV `wnpfloqpjvaacobppbpk` toegepast en geregistreerd als `20260912123318`. HR Admin heeft via de normale UI in Planeten twee profielen en één actieve medewerkerset aangemaakt. De configuratieroute vereist nu `leave:write`; Test Manager en Test Medewerker worden server-side naar `/geen-toegang` gestuurd. De beperkte remote readback bevestigt 2 Planeten-profielen, 1 default, 1 set met prioriteit 10, en behoud van `TEST-BOUNDARY` 0/0 en Test BV 0.

Lokale verificatie: 17/17 targeted leave/schema/migration tests, strict TypeScript, ESLint, i18n-pariteit, diff-check en Webpack-productiebuild geslaagd. Volledige suite: 1381/1382; de enige failure is de bestaande, niet-gerelateerde Document Studio migration-contracttest. Supabase typegen en advisors zijn uitgevoerd; advisors tonen alleen bestaande projectbrede waarschuwingen/INFO’s. Een frisse HR Admin-browserrun heeft 0 console errors; Manager/Medewerker negative acceptance is groen. Turbopack blijft in deze worktree geblokkeerd door de bekende invalid `node_modules`-symlink; de Webpack-fallback is groen. Geen synthetic employee is gekoppeld, en er is niet gepusht of gedeployed.

## Focus + Identity / Preboarding — review remediation 2026-09-17

- Status: **LOKAAL GEIMPLEMENTEERD; REVIEW-FIXES OP DEZE BRANCH; REMOTE/BROWSER-GATES OPEN**.
- Exacte baseline: `origin/main` `6f9f61b85d2b488557d066fcabdbb39e60f2b39b`.
- Worktree/branch: `.codex-worktrees/focus-identity-preboarding-20260917` / `work/focus-identity-preboarding-20260917`.
- Implemented: `/focus` compact employee/manager/preboarding screens; browser preference; safe Journey action handling; invitation lifecycle API/UI; employee-context access surface; individual resend/revoke; bulk select/review/result with partial failures; 7-day invite expiry; acceptance redirect; effective-date access state; strict tenant/HR-group-scoped preboarding allowlist in app permission checks and SQL/RLS helpers.
- Review fixes: REVOKED is no longer presented as BLOCKED; normal employee activation uses additive `EMPLOYEE_ACTIVATION` while future-start employees use `PREBOARDING_EMPLOYEE`; invitation review groups by preferred language and shows the fixed canonical mail preview; per-invite editable introduction is explicitly deferred because the current Supabase Auth delivery API cannot safely carry it.
- Trust-boundary fix: `/api/invitations/employee` accepts only a strict `employeeId`; the server resolves the authenticated active tenant + HR-group, canonical private email, confirmed employment timeline, canonical global `EMPLOYEE` role and tenant scope before calling the existing invitation primitive. Bulk employee activation accepts only employee IDs and calls that same primitive per employee; the generic route and business batch remain `BUSINESS_USER`-only. Negative tests cover crafted email/purpose/role/scope substitution, cross-tenant/cross-HR-group selection and already-linked employees.
- Migration proof: `internal_security.current_employee_id(uuid, uuid)` is defined canonically in `supabase/migrations/20260805200000_hr_group_people_organization_roles.sql:205-226`, with both tenant and HR-group predicates and the authenticated grant. The Focus migration consumes that overload; the contract test reads the historical definition instead of proving only a string in the new migration.
- Deferred to integration: ESS/MSS-owned Leave, Actual Work, My Work, My Requests, workflow and any shared employee/work-item adapters. Focus exposes only safe links/seams and does not duplicate those domains.
- Gates: current targeted review/security set `20 bestanden / 107 tests` is groen. De volledige HR-suite is met de passende `--testTimeout=30000` volledig groen: `407/407` bestanden en `1621/1621` tests; de standaard 5s-run raakt alleen de bestaande PDF-renderer-timeout. Strict TypeScript, ESLint, i18n (`38 namespaces`), `git diff --check` en Webpack-productiebuild (`284` static pages) zijn groen. Turbopack blijft geblokkeerd door de bekende ongeldige worktree-`node_modules/next`-symlink; de Webpack-fallback is groen.
- Browser: authenticated persona/responsive acceptance kon niet worden uitgevoerd omdat de beschermde root-`.env.local` niet naar deze worktree is gekopieerd. De anonieme lokale smoke eindigde op `500` door de bestaande Supabase-configuratiebehoefte in middleware; er is geen credential- of fixture-mutatie gedaan.
- SQL: migration contracttests zijn groen. `supabase db lint --local` kon na de lokale telemetry-blocker niet verder omdat Postgres op `127.0.0.1:54322` niet draait; de migrations zijn niet naar lokaal/DEV/Production toegepast, advisors/typegen zijn daarom bewust niet als uitgevoerd geclaimd.
- ESS/MSS Leave, Actual Work, My Work, My Requests, workflow en gedeelde work-item adapters blijven deferred naar de andere run; deze branch voegt daarvoor geen parallel backenddomein toe. Geen remote migration, Production, Vercel of andere-worktree-mutatie.

## Focus access management — 2026-09-18

- Status: **CODE- EN DEV-SCHEMAGATE GROEN; AUTHENTICATED BROWSERGATE ENVIRONMENT-GATED**.
- Worktree/branch/base: `.codex-worktrees/focus-access-management-20260918` / `work/focus-access-management-20260918` / `5ab7496204ffd0ec4defb99a219997908b5fb815`.
- Opgeleverd: complete HR Employee Detail access card; canonical individual/bulk employee invitation flow; separate `employee_ess_access` ACTIVE/BLOCKED lifecycle; canonical Auth `last_sign_in_at`; read-only HR Admin Focus preview; group-level Employee/Manager portal modes; server- en database-enforcement voor Focus-only, Full, PREBOARDING, NO_EMPLOYMENT en blocked ESS. Geen tweede access model, geen Auth-manipulatie en geen parallel Leave/My Work/My Requests-backend.
- DEV migrations: `focus_access_management` en `focus_no_employment_selfservice_hardening` zijn toegepast en read-back gecontroleerd op `wnpfloqpjvaacobppbk`; remote history is respectievelijk `20260918090701` en `20260918094122`. De scoped permission readback bevat tenant-, HR-group-, future-employment- en blocked-access guards. Production, main, Vercel en andere worktrees zijn niet gewijzigd.
- Gates: targeted review/security `12/12 bestanden, 58/58 tests`; full Vitest `410/410 bestanden, 1.634/1.634 tests`; strict TypeScript, ESLint, i18n `38 namespaces`, `git diff --check` en Next/Webpack build `286/286 static pages` groen. Advisors melden alleen bestaande projectbrede waarschuwingen/INFO’s plus de verwachte unused-index INFO voor de nieuwe scope-index.
- Browsergate: de authenticated hosted run is **ENVIRONMENT-GATED** wegens herhaald `CDP response channel closed` in de bestaande agent-browser harness. Er is geen Auth-, invitation-, token-, fixture- of mailmutatie uitgevoerd; de eerder gesloten Preboarding acceptance blijft gesloten.
- Deferred: Leave, Actual Work, My Work, My Requests en gedeelde work-item adapters naar de andere ESS/MSS-run; editable per-invite introduction omdat Supabase `inviteUserByEmail` dit niet veilig per invitation ondersteunt.

## Focus completion + act-as + absence — 2026-09-19

- Status: **LOCAL CODE GATE GREEN; DEV SCHEMA APPLY AND DB-BACKED ABSENCE ACCEPTANCE BLOCKED BY PRE-EXISTING MIGRATION HISTORY DRIFT**.
- Candidate: `C:\Users\Edwin\Documents\Apps\LiquidHR\.codex-worktrees\focus-completion-act-as-20260918`, branch `work/focus-completion-act-as-20260918`, baseline `3bd31746ff40dbfcc2de54f8957379f3a8f23d61`, current HEAD `5d6a165f35f274e400d447f3dec22d511220d879`.
- Scope: Focus reuses the canonical Leave, Actual Work, Process Automation, Directory, Documents and Absence boundaries. Employee sickness self-report is gated by both `self:absence:write` and the HR-group setting; the pending employee case is operationally visible as AFWEZIG in availability/Manager Home but excluded from formal absence KPI/report reads until manager `Bevestigen`. Manager work exposes `Bevestigen` and `Terugsturen voor correctie`, while direct manager/HR reporting remains an active canonical case without Employee-style confirmation. Optional expected recovery maps to `expected_recovery_on` only in manager/HR flows; recovery reuses `/api/absence/recovery`.
- Manager Home: the direct-team scoped Manager-only home adds exactly the `Ziek in mijn team` and `Op vakantie deze week` cards. Sick rows use inclusive calendar days and show pending status; vacation rows use approved allocations with canonical `leave_types.family = VACATION`, NL Monday-Sunday overlap, partial-day modes and full approved ranges. Act-as Employee receives no manager cards or HR controls.
