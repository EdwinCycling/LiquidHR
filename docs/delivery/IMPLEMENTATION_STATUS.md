...
- Status: **LOKAAL GEIMPLEMENTEERD; REVIEW-FIXES OP DEZE BRANCH GECOMMIT; REMOTE/BROWSER-GATES OPEN**.
...
- Gates: targeted review/security set `23 bestanden / 112 tests` groen; volledige HR-suite `403/404 bestanden` en `1606/1607 tests`, met uitsluitend de bestaande PDF-render timeout op de vaste 5s-limiet (de test slaagt met `--testTimeout=30000`); strict TypeScript, ESLint, i18n `38 namespaces`, `git diff --check` en Webpack-productiebuild (`283` static pages) groen. Turbopack blijft geblokkeerd door de bekende ongeldige worktree-`node_modules/next`-symlink; de Webpack-fallback is groen.
